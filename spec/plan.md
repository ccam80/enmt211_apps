...
| Task | Description | Complexity | Key Files |
|------|-------------|------------|-----------|
| 6.1.1 | **R1+R2+R3 + 2-D seam.** Promote `motor-mesh-view.js` to the production 2-D render (`drawMaterial`/`drawFluxLines`/`drawModulusB`/`drawSaturation`/`drawMagnetization`/`drawCurrentDensity`); rewrite `cross-section-render.js` mesh-native, consuming only `runtime.stack.sliceMesh(k)` + `runtime.lastSolve.perSliceField[k]` (Phase 5 D2/D3) and the shared `UM.fieldViz` toggle state; **add the 2-D seam** `UM.registerCrossSection2D({paint(mountCtx, canvases, rctx)})` to `mount.js` (mirrors `registerRender3D`), rewire the 2-D canvases through it, **delete the dead built-in grid rig** `else` branch + its orphan helpers (`drawFeatureSectors2D/3D`, `fillSector2D/3D`, `drawSlotConductors3D`, `drawRing3D`, `ringPoints`, `featureColor`, `KIND_COLORS`, `smoothedMagScale`) in `mount.js`. Saturation viz uses `|B|/Bknee` (no ν duplication; Phase 5 returns no `nuElem`). Field-view toggles are five **independent** checkboxes (no always-on), defaulting to flux-lines on only. **Frame loop rewritten to wall-budgeted stepping**: delete `PHYS_DT`/`STEPS_PER_FRAME`; each frame runs one `runtime.step(orderedStepDt, 30 ms)` (engine seam `step(dt, wallBudgetMs?)` in `lib/motor-run.js` — budget breaks the adaptive solve loop after ≥1 solve; omitted = deterministic for tests) with a `Playback` slider setting `orderedStepDt` and a header slow-motion warning when the budget caps the frame short of the ordered step. | L | `lessons/unified_motor/cross-section-render.js`, `lib/motor-mesh-view.js`, `lessons/unified_motor/mount.js`, `lib/motor-run.js` |
