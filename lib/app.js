...
//        <select> lesson picker) above the per-spec stage. The picker keeps
//        the outer chrome reading as navigation, distinct from the per-spec
//        control header.
//
...
//     //   yMin, yMax  — fixed bounds (locks the axis).
...
(function () {
  ...
  function _mountSpec(host, spec) {
    ...
    // Modes axis is folded inline into the spec header as a button group, so
    // the chrome stays at two visually distinct rows (slim app-nav, primary
    // spec header) rather than a separate sub-tabs strip below.
    ...
    function buildSliderPanels() {
      ...
      // span:full panels are present, append at the end.
      if (spec.description) {
        let lastFull = -1;
    ...
    function renderPlots() {
      ...
      for (const pane of plotPanes) {
        ...
        if (Number.isFinite(+cfg.yMin) && Number.isFinite(+cfg.yMax)) {
          ...
          // pane is readable even when data is briefly small or zero. The
          // axis shrinks back once a brief spike ages out of the window.
