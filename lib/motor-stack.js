    ...
    function solveCoupled(o) {
      ...
      for (var it = 0; it < maxIter; it++) {
        ...
        for (s = 0; s < nSlices; s++) {
          ...
          // One-sided θ-FD: the single θ−h eval must precede the factorize (it
          // re-preps the gap, invalidating the Cholesky factor), then assemble +
          // factor at θ → R(θ)=Rfield[s], T(θ)=Ts, λ(θ)=tmpLam, gapNodal(θ); the
          // post-diff reuses those centers. Net: one residual/flux eval, not θ±h.
          slice.coupledThetaPreEval(A[s], i, thS);
          var Ts = slice.coupledAssemble(A[s], i, thS, Rfield[s]);
          T += Ts;
          var fns = slice.coupledRhsNorm(); if (fns > fNorm) fNorm = fns;
          slice.coupledFluxInto(A[s], thS, tmpLam);
          dTdth += slice.coupledThetaPostDiff(thS, Rfield[s], tmpLam, Ts, dRdth[s], dLamdth[s]);
