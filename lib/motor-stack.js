...
(function () {
  ...
  function create(expanded, opts) {
    ...
    function coenergyTorque(thetaR, currents, coeffs) {
      ...
      // textbook reluctance torque ½iᵀ(dL/dθ)i carries NO Parseval factor. This
      // matches the back-EMF in motor-slice.js, which uses measure-1 on the SAME
      // dL/dθ — energy conservation (Tω=Σeₖiₖ) forbids the EMF being measure-1
      // while the torque is measure-π. The cogging term comes from gap.torque
      // (full Maxwell stress), also measure-1.
      var GAP_MEASURE = 1;
