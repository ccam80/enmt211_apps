...
(function () {
  ...
  function create(expanded, opts) {
    ...
    function coenergyTorque(thetaR, currents, coeffs) {
      ...
      // Gap-coupling energy Parseval measure (2026-05-29 Bug #1). The λ-derived
      // co-energy terms (reluctance/mutual/pm, built from dL/dθ and dλpm/dθ)
      // omit the gap harmonic basis measure ∫₀^{2π}cos²(kθ)dθ = π, so each is
      // π× too small vs the oracle-validated Maxwell-stress (Arkkio) torque.
      // The cogging term already comes from gap.torque (full Maxwell stress) and
      // is correct as-is. λ/L/λpm themselves stay PHYSICAL (unscaled) for the
      // back-EMF/circuit path (motor-run) — verified against an independent
      // Stokes field-side flux-linkage reference (ratio 1.0); the π lives ONLY
      // here in the torque, never on the flux-linkage readout.
