...
(function () {
  ...
  function create(expanded, opts) {
    ...
    function solveCoupled(o) {
      ...
      var tol = o.tol != null ? o.tol : 1e-9;
      var tolField = o.tolField != null ? o.tolField : 1e-9;
      ...
      var lambda = new Float64Array(m), tmpLam = new Float64Array(m);
      ...
      for (var it = 0; it < maxIter; it++) {
        iters = it + 1;
        // ---- per-slice assemble + borders ----
        T = 0; dTdth = 0;
        for (k = 0; k < m; k++) lambda[k] = 0;
        for (s = 0; s < nSlices; s++) {
          var slice = slices[s].slice, thS = th + slices[s].offset;
          // θ-FD borders FIRST (they re-prep the gap at θ±h), then assemble at θ
          // so the factor used by coupledSolveAgainst is fresh.
          dTdth += slice.coupledThetaDerivs(A[s], i, thS, dRdth[s], dLamdth[s]);
          T += slice.coupledAssemble(A[s], i, thS, Rfield[s]);
          slice.coupledFluxInto(A[s], thS, tmpLam);
          for (k = 0; k < m; k++) lambda[k] += tmpLam[k];
          slice.coupledDTdAInto(thS, dTdA[s]);
          for (a = 0; a < nf; a++) slice.coupledUnitRhsInto(freeIdx[a], thS, uRHS[s][a]);
        }
        // ---- reduced residual ----
        for (a = 0; a < nf; a++) {
          k = freeIdx[a];
        ...
        fieldResid = 0;
        for (s = 0; s < nSlices; s++) for (var n = 0; n < nG[s]; n++) { var av = Math.abs(Rfield[s][n]); if (av > fieldResid) fieldResid = av; }
        redResid = 0; for (a = 0; a < nu; a++) { var rv = Math.abs(rRed[a]); if (rv > redResid) redResid = rv; }
        if (fieldResid < tolField && redResid < tol) { converged = true; break; }
        ...
        for (s = 0; s < nSlices; s++) {
          ...
          for (a = 0; a < nf; a++) {
            k = freeIdx[a];
            var uk = uRHS[s][a];
            for (b = 0; b < nf; b++) S[a * nu + b]       -= coef * dot(uk, xcol[s][b], nG[s]);
            S[a * nu + (nf + 1)] -= coef * dot(uk, xcol[s][nf], nG[s]);   // θ col
            rCond[a] -= coef * dot(uk, xr[s], nG[s]);
          }
