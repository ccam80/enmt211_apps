...
(function () {
  ...
  function create(expanded, opts) {
    ...
    function solveCoupled(o) {
      ...
      // column-major nG×(nf+2) = [Rfield | −uRHS_0..−uRHS_{nf-1} | dRdth].
      var rhsBlk = nG.map(function (n) { return new Float64Array(n * (nf + 2)); });
      var xBlk   = nG.map(function (n) { return new Float64Array(n * (nf + 2)); });
      ...
      for (var it = 0; it < maxIter; it++) {
        ...
        for (s = 0; s < nSlices; s++) {
          ...
          sl.coupledSolveMultiAgainst(blk, nf + 2, xb);
