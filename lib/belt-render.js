...
(function () {
  ...
  function layout(L, p, opts) {
    ...
    const drivePulleyCx = L.xToPx(L.xMin);
    const idlerPulleyCx = L.xToPx(L.xMax);
    const r1Px = p.rDrive * L.mToPx;
    const r2Px = p.rIdler * L.mToPx;
    // Motor sits just left of the drive-pulley centre (visually layered
    // behind the pulley body). Matches the original motionapp placement —
    // anchoring outside the pulley would push the motor off-canvas at any
    // sensible drive-pulley radius.
    const motorCx = drivePulleyCx - motorR - 8;
    // Top-tangent y at the drive-pulley side. Lessons that want to
    // hit-test the load body (not the belt centre) target this.
    const loadTopY = cy - r1Px;
    return { cy, drivePulleyCx, idlerPulleyCx, r1Px, r2Px, motorCx, motorR, loadTopY };
  }
  ...
  function drawScene(ctx, L, p, state, opts) {
    ...
    for (const [pcx, pr, label] of [[drivePulleyCx, r1Px, "drive"],
      ...
      const phaseTheta = (label === "drive")
        ? state.theta
        : state.theta * (p.rDrive / Math.max(1e-6, p.rIdler));
      const ind = pr * 0.7;
    ...
    // Phase marks scrolling along the top run with belt motion
    const beltPhase = state.theta * r1Px;
