...
(function () {
  ...
  function backEmf(coeffs, i, omega) {
    ...
    // Gap-coupling energy Parseval measure (2026-05-29 back-EMF energy-balance).
    // The §9 gap DtN operator is pointwise/per-radian; λ itself (a Stokes loop
    // integral of A) is physical webers (no π — verified ratio 1.0), but any
    // WORK-RATE quantity built from the gap-harmonic amplitudes carries the ring
    // measure ∫₀^{2π}cos²(kθ)dθ = π. Back-EMF *power* e·i is a work rate, so e
    // carries the same π that coenergyTorque applies (GAP_MEASURE) — without it,
    // e·i = ωT/π and the electromechanical energy balance e·i = ωT is violated
    // for EVERY machine (the dLdth/dLambdaPmdth coeffs stay raw; π is applied once
    // here at the consumer, mirroring coenergyTorque, so there is no double-count).
  ...
  function stepCurrents({ L, R, V, i, dt, terminalStates, e, Iimp }) {
    ...
    if (mf > 0) {
      ...
      // Assemble reduced system Ar (mf×mf) and rhs (length mf)
      // Ar[a*mf + b] = L[F[a]*m + F[b]] + (a===b ? dt·R[F[a]] : 0)
      // rhs[a] = Σ_{kb∈F} L[F[a]*m + kb]·i[kb] + dt·(Veff[F[a]] − e[F[a]])
      //          + Σ_{kc∈CURRENT} L[F[a]*m + kc]·(i[kc] − Iimp[kc])
      ...
      for (let a = 0; a < mf; a++) {
        ...
        for (let b = 0; b < mf; b++) {
          ...
          Ar[a * mf + b] = Lkakb + (a === b ? dt * R[ka] : 0);
          rhsVal += Lkakb * i[kb];
        }
        // CURRENT contribution moved from LHS to RHS
