...
(function () {
  ...
  function makeSnapshot(epoch, state, bundle, nSlices, nCircuits) {
    const snap = {
      type: "snapshot",   // so the Worker→main message router dispatches it
      epoch: epoch,
      t: state.t,
      theta: state.theta,
      omega: state.omega,
  ...
  function createWorker(opts) {
    ...
    let fellBack = false;
    let inline = null;
    const queued = [];          // commands awaiting "ready", to flush to the worker
    const history = [];         // non-pace commands ever posted, to replay on fallback

    // Rebuild the sim in-process and replay the state-defining command history
    // (init/rebuild/reset/drive/mechanical — not transient pace) so the app keeps
    // running whatever point the worker failed at, even after `queued` was
    // flushed. Transient pace resumes from the next frame's post.
    function fallBack(reason) {
      ...
      for (const cmd of history) inline.post(cmd);
      queued.length = 0;
    ...
    if (worker) {
      ...
      const budget = opts.readyTimeoutMs != null ? opts.readyTimeoutMs : 3000;
    ...
    return {
      post: function (cmd) {
        ...
        if (cmd.type !== "pace") history.push(cmd);   // for fallback replay
