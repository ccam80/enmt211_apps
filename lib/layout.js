...
(function () {
  ...
  function linearTrack(W, H, opts) {
    ...
    // marginLeftPx / marginRightPx: extra inset beyond padX, used by the
    // shell to reserve room for an external decoration (motor disc, sensor
    // probe, end-stop ornament) so the lesson never has to subtract pixel
    // values from xToPx(xMin) by hand. Track usable area shrinks by these.
    const marginLeftPx  = (opts.marginLeftPx  != null) ? +opts.marginLeftPx  : 0;
    const marginRightPx = (opts.marginRightPx != null) ? +opts.marginRightPx : 0;
    const trackFrac = (opts.trackFrac != null) ? +opts.trackFrac : 0.55;
    const axis = (opts.axis === "vertical") ? "vertical" : "horizontal";

    if (axis === "horizontal") {
      const padLeft  = padX + marginLeftPx;
      const padRight = padX + marginRightPx;
      ...
      function xToPx(xm) { return padLeft + (xm - xMin) * mToPx; }
      function pxToX(px) { return (px - padLeft) / mToPx + xMin; }
      return freeze({
        kind: "linearTrack", axis,
        W, H,
        padX, padY, padLeft, padRight,
