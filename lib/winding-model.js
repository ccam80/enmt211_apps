(function () {
  ...
  function conductorFeatures(routing, slotGeom) {
    const { slotTheta, phases } = routing;
    const { rRange, member, angularWidth, flankPlacement } = slotGeom;
    ...
    // Concentrated (tooth-wound) coils: each coil side hugs the flank of the
    // tooth it wraps — the go side against the tooth just above its slot, the
    // return side against the tooth just below — so a slot carries two separated
    // bundles (one per neighbouring tooth) instead of one shared block. Which
    // flank a conductor takes is read from the routing (go vs return), not from
    // the current sign (which only tracks coil polarity). Full radial depth; the
    // angular split keeps each conductor on its own mesh elements.
    if (flankPlacement) {
      const side = new Int8Array(nCircuits * nSlots); // +1 go, -1 return, 0 absent
      let ci = 0;
      for (const phase of phases) {
        for (let bi = 0; bi < phase.branches.length; bi++) {
          for (const coil of phase.branches[bi].coils) {
            side[ci * nSlots + coil.slotGo] = 1;
            if (coil.slotReturn != null) side[ci * nSlots + coil.slotReturn] = -1;
          }
          ci++;
        }
      }
      const bw = 0.42 * angularWidth;                 // bundle width (≈0.32·halfW gap mid-slot)
      for (let s = 0; s < nSlots; s++) {
        const theta = slotTheta[s];
        for (let c = 0; c < nCircuits; c++) {
          const T = turns[c * nSlots + s];
          if (T === 0) continue;
          const sd = side[c * nSlots + s];
          let lo, hi;
          if (sd > 0)      { hi = theta + halfW; lo = hi - bw; }   // against the upper tooth
          else if (sd < 0) { lo = theta - halfW; hi = lo + bw; }   // against the lower tooth
          else             { lo = theta - bw / 2; hi = theta + bw / 2; }
          features.push({ kind: "conductor", member, rRange: rRange, thetaRange: [lo, hi], circuit: c, turns: T });
        }
      }
      return features;
    }
