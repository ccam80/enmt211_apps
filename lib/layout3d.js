...
(function () {
  ...
  function orbital(W, H, opts) {
    ...
    // Orthographic mode: project without the perspective divide. A face plane
    // perpendicular to the view then maps to screen by a pure affine, so a
    // single ctx transform reproduces it exactly (no divergence between a
    // cap drawn via an affine and walls drawn per-vertex). Pixels-per-world
    // defaults to the perspective scale at the origin so the apparent size
    // matches the perspective camera.
    ...
    const orthoScale = (opts.orthoScale != null) ? +opts.orthoScale : (focal / dist);
    ...
    function project(p) {
      ...
      if (ortho) {
        return {
          px:    originPx.x + c.x * orthoScale,
          py:    originPx.y - c.y * orthoScale,
          depth: c.z,
          behind: c.z < nearDepth,
        };
      }
    ...
    return freeze({
      kind: "orbital3d",
      W, H, originPx,
      yaw, pitch, dist, fov, focal, nearDepth, ortho, orthoScale,
      eye,
