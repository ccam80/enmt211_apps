...
(function () {
  ...
  function create(expanded, opts) {
    ...
    var bdfRtol = opts.bdfRtol != null ? opts.bdfRtol : 1e-4;
    ...
    //   λ_k : atol = rtol · (characteristic flux) — PM flux, or L·I_ref if non-PM
    ...
    for (var _k = 0; _k < m0; _k++) {
      ...
      if (_amp / _R > Iref) Iref = _amp / _R;
    }
    if (!(Iref > 0)) Iref = 1;
    var atolVec = new Float64Array(m0 + 2);
    var rtolVec = new Float64Array(m0 + 2);
    for (var _k2 = 0; _k2 < m0; _k2++) {
      var _lamRef = Math.max(Math.abs(_c0.lambdaPm[_k2]), Math.abs(_c0.L[_k2 * m0 + _k2]) * Iref, 1e-12);
    ...
    // Newton convergence is per-equation RELATIVE (see stack.solveCoupled).
    var relTol     = opts.coupleRelTol != null ? opts.coupleRelTol : 1e-6;
