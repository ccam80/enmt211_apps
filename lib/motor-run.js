...
(function () {
  ...
  function create(expanded, opts) {
    ...
    // -------------------------------------------------------------------------
    //  runtime.step(dt, wallBudgetMs?, maxStepDt?) → state
    //
    ...
    //
    //  `maxStepDt` (live UI only) caps the integrator's step to one render
    //  frame's worth of sim-time. In smooth phases this forces one solve per
    //  frame instead of the controller taking a big step and overshooting (which
    //  then freezes the next frames) — uniform scroll. The clamp is always below
    //  the LTE-allowed step there, so it only sub-divides, never over-reaches; in
    //  stiff phases the controller already wants a smaller step and it does
    //  nothing. Omitted (tests) → the controller is untouched.
    // -------------------------------------------------------------------------
    function step(dt, wallBudgetMs, maxStepDt) {
      ...
      while (integ.t < tEnd - 1e-15 && guard++ < 100000) {
        if (maxStepDt != null && integ.dt > maxStepDt) integ.dt = maxStepDt;   // cap to one frame (live UI)
