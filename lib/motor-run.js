...
(function () {
  ...
  function create(expanded, opts) {
    ...
    function step(dt) {
      ...
      // Step 6: solve field + torque (Arkkio gap-band)
      var solved = stack.solve(state.theta, state.i);
      lastSolve = solved;

      // Step 7: mechanics (semi-implicit Euler)
      // α = (τ_em − b·ω − τ_load) / J
      // ω^{n+1} = ω^n + α·dt  (semi-implicit: ω used in damping from step n)
      var tau = solved.torque;
      var alpha = (tau - mechanical.damping * state.omega - mechanical.loadTorque) / mechanical.J;
      state.omega = state.omega + alpha * dt;
