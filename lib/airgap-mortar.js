// =============================================================================
//  LIB.AirgapMortar — moving-band air-gap coupling
// =============================================================================
//
//  Real-space FE air-gap band: a single layer of air triangles meshed between
//  the rotor gap ring (rotated by φ) and the stator gap ring. The two rings are
//  EXISTING body DOFs (no harmonic DOFs, no new nodes) — the band only adds
//  sparse triplets coupling rotor-ring ↔ stator-ring. This captures the full
//  gap field with NO Fourier truncation (cage / slot / cogging / high-tooth
//  content native).
//
//  Contract with motor-slice.js:
...
//  (the band is fixed across the Newton iters at a given θ). This re-analysis
//  is the moving-band cost; the factorize itself stays sparse and cheap.
// =============================================================================
(function () {
  "use strict";
  ...
  function build(rotorGap, statorGap, opts) {
    ...
    function stampInto(phi, outI, outJ, outV) {
      // Wrap φ to [0,2π). The angular merge-walk below compares rAng against the
      // wrapped start alignment r0 (and the mod-2π stator order); rAng must use the
      // same wrapped φ or, once the rotor passes one full revolution (φ≥2π), rAng's
      // base (thR+φ) no longer interleaves with the stator ring and the zipper
      // degenerates — collapsing the gap coupling. rPos uses cos/sin (already
      // periodic), so wrapping leaves the stamped geometry unchanged.
      ...
      // ps may legitimately reach Ngs when r0 falls in the final stator gap (r0 >
      // every node in [0,2π)): the first stator angle ≥ r0 is then stator[0] wrapped
      // to +2π, and sAng/sPos/psEnd handle ps ≥ Ngs. Wrapping ps back to 0 here would
      // start the strip at angle 0 against a rotor node near 2π, spanning the ring and
      // over-covering the band — corrupting the field on φ ∈ (−2π/Ngr, 0).
    ...
    // ---- axial-flux coupling vector over the gap band ----
    // For the global flux DOF Ψ (net radial flux carried via the annulus harmonic
    // 1-form ∇χ = θ̂/(2πr)), accumulate the gap-band part of c_a = ∫ν∇N_a·∇χ dA into
    // cOut (local gap-ring layout [rotor 0..Ngr-1 | stator Ngr..]) and return the
    // band's contribution to d = ∫ν|∇χ|² dA. The gap is air (ν=nuAir) and is the
    // dominant radial reluctance, so it must enter the flux DOF's field reluctance.
    // Zips the same φ-rotated T3 band as stampInto.
    function accTriFlux(xy, g, cOut) {
      const xi = xy[0], yi = xy[1], xj = xy[2], yj = xy[3], xk = xy[4], yk = xy[5];
      const det = (xj - xi) * (yk - yi) - (xk - xi) * (yj - yi);
      if (det === 0) return 0;
      const A2 = Math.abs(det), area = 0.5 * A2, sgn = det > 0 ? 1 : -1;
      const b0 = yj - yk, b1 = yk - yi, b2 = yi - yj;
      const c0 = xk - xj, c1 = xi - xk, c2 = xj - xi;
      const xc = (xi + xj + xk) / 3, yc = (yi + yj + yk) / 3, r2 = xc * xc + yc * yc;
      const gx = -yc / (2 * Math.PI * r2), gy = xc / (2 * Math.PI * r2);
      // c_a = nuAir·(∇N_a·∇χ)·area = nuAir·(b_a·gx + c_a·gy)·sgn/2  (∇N_a=(b_a,c_a)/det)
      cOut[g[0]] += nuAir * (b0 * gx + c0 * gy) * sgn * 0.5;
      cOut[g[1]] += nuAir * (b1 * gx + c1 * gy) * sgn * 0.5;
      cOut[g[2]] += nuAir * (b2 * gx + c2 * gy) * sgn * 0.5;
      return nuAir * (gx * gx + gy * gy) * area;
    }
    function fluxCouplingInto(phi, cOut) {
      phi = ((phi % TWO_PI) + TWO_PI) % TWO_PI;
      const rAng = (pr) => thR[permR[pr % Ngr]] + phi + (pr >= Ngr ? TWO_PI : 0);
      const sAng = (ps) => thS[permS[ps % Ngs]] + (ps >= Ngs ? TWO_PI : 0);
      const rPos = (pr) => { const idx = permR[pr % Ngr]; const a = thR[idx] + phi; return { g: idx, x: Rr * Math.cos(a), y: Rr * Math.sin(a) }; };
      const sPos = (ps) => { const idx = permS[ps % Ngs]; return { g: Ngr + idx, x: Rs * cosS[idx], y: Rs * sinS[idx] }; };
      let pr = 0, ps = 0, dAcc = 0;
      const r0 = ((thR[permR[0]] + phi) % TWO_PI + TWO_PI) % TWO_PI;
      while (ps < Ngs && (thS[permS[ps]] % TWO_PI) < r0) ps++;
      const prEnd = pr + Ngr, psEnd = ps + Ngs;
      let guard = 2 * (Ngr + Ngs) + 8;
      while ((pr < prEnd || ps < psEnd) && guard-- > 0) {
        ...
        if (pr < prEnd && (ps >= psEnd || rNextA <= sNextA)) {
          const rN = rPos(pr + 1);
          dAcc += accTriFlux([r0p.x, r0p.y, rN.x, rN.y, s0p.x, s0p.y], [r0p.g, rN.g, s0p.g], cOut);
          pr++;
        } else {
          const sN = sPos(ps + 1);
          dAcc += accTriFlux([r0p.x, r0p.y, s0p.x, s0p.y, sN.x, sN.y], [r0p.g, s0p.g, sN.g], cOut);
          ps++;
        }
      }
      return dAcc;
    ...
    //  extension of the two ring fields. The torque is evaluated by:
    ...
    //  generator's highest feature count is the densest fixture's 50 rotor teeth, but
    ...
    //  Measured at the worst (tightest) gap: at k·(Δr/r)=2, L=4→+5%, L=8→+1.5%; at
    ...
    const gR0u = new Float64Array(Nu);        // ∂T/∂(rotor-ring resampled) — adjoint scratch
    const gLu  = new Float64Array(Nu);        // ∂T/∂(stator-ring resampled)
    const gIntv = new Float64Array(nTot);     // ∂T/∂(interior rings)
    const Xadj = new Float64Array(nTot);      // adjoint reconstruction solution
    ...
    // Transpose of the periodic Catmull-Rom resample: scatter a gradient `gval`
    // at uniform angle `ang` back onto the four source ring nodes.
    function scatterCubic(gOut, perm, n, th0, step, ang, gval) {
      let u = (ang - th0) / step;
      u = u - Math.floor(u / n) * n;
      const i1 = Math.floor(u);
      const t = u - i1;
      ...
      const t2 = t * t, t3 = t2 * t;
      gOut[perm[i0]]     += 0.5 * (-t + 2 * t2 - t3) * gval;
      gOut[perm[i1 % n]] += 0.5 * (2 - 5 * t2 + 3 * t3) * gval;
      gOut[perm[i2]]     += 0.5 * (t + 4 * t2 - 3 * t3) * gval;
      gOut[perm[i3]]     += 0.5 * (-t2 + t3) * gval;
    }

    // ∂T/∂A on the gap rings: gRot[Ngr] = ∂T/∂ARotGap, gStat[Ngs] = ∂T/∂AStatGap.
    // T is a bilinear form in the ring-field stack [aR_u, Xint(interior), aS_u];
    // the gradient back-propagates the Maxwell band-average, through one adjoint
    // reconstruction solve (M is SPD ⇒ M⁻ᵀ=M⁻¹, reuse the factored recSolver),
    // and out through the resample transpose. ~one extra solveInto per call.
    function torqueGrad(ARotGap, AStatGap, phi, gRot, gStat) {
      // Forward (rings + reconstruction) — same as torque().
      for (let j = 0; j < Nu; j++) {
        const ang = j * hth;
        aS_u[j] = cubicRing(AStatGap, permS, Ngs, thS0, stepS, ang);
        aR_u[j] = cubicRing(ARotGap, permR, Ngr, thR0, stepR, ang - phi);
      ...
      for (let j = 0; j < Nu; j++) {
        RHS[j] += ar * aR_u[j];
        RHS[(nInt - 1) * Nu + j] += ar * aS_u[j];
      }
      recSolver.solveInto(RHS, Xint);

      // Backward through the Maxwell band-average.
      const inv12h = 1 / (12 * hth);
      const invh = 1 / hsub;
      const ringAt = (s, j) => (s === 0 ? aR_u[j] : s === L_SUB ? aS_u[j] : Xint[(s - 1) * Nu + j]);
      gR0u.fill(0); gLu.fill(0); gIntv.fill(0);
      const addG = (s, j, val) => {
        if (s === 0) gR0u[j] += val;
        else if (s === L_SUB) gLu[j] += val;
        else gIntv[(s - 1) * Nu + j] += val;
      };
      const scaleAll = ell / mu0 / L_SUB;
      for (let s = 0; s < L_SUB; s++) {
        const rin = Rr + s * hsub, rc = 0.5 * (rin + rin + hsub);
        for (let j = 0; j < Nu; j++) cmid[j] = 0.5 * (ringAt(s, j) + ringAt(s + 1, j));
        const sc = rc * hth * scaleAll;          // T contribution = sc·D4·Bt
        for (let j = 0; j < Nu; j++) {
          const jm2 = (j - 2 + Nu) % Nu, jm1 = (j - 1 + Nu) % Nu, jp1 = (j + 1) % Nu, jp2 = (j + 2) % Nu;
          const Bt = -(ringAt(s + 1, j) - ringAt(s, j)) * invh;
          const D4 = (-cmid[jp2] + 8 * cmid[jp1] - 8 * cmid[jm1] + cmid[jm2]) * inv12h;
          // ∂(sc·D4·Bt)/∂Bt · ∂Bt/∂Φ
          const cB = sc * D4;
          addG(s, j, cB * invh);
          addG(s + 1, j, -cB * invh);
          // ∂(sc·D4·Bt)/∂D4 · ∂D4/∂Φ  (D4 via cmid = ½(Φ[s]+Φ[s+1]))
          const w = sc * Bt * inv12h * 0.5;
          addG(s, jp2, -w); addG(s + 1, jp2, -w);
          addG(s, jp1, 8 * w); addG(s + 1, jp1, 8 * w);
          addG(s, jm1, -8 * w); addG(s + 1, jm1, -8 * w);
          addG(s, jm2, w); addG(s + 1, jm2, w);
        }
      }
      // Adjoint through the reconstruction: ∂T/∂RHS = M⁻¹·(∂T/∂Xint).
      recSolver.solveInto(gIntv, Xadj);
      for (let j = 0; j < Nu; j++) {
        gR0u[j] += ar * Xadj[j];                     // RHS ring 0   = ar·aR_u
        gLu[j]  += ar * Xadj[(nInt - 1) * Nu + j];   // RHS last ring = ar·aS_u
      }
      // Resample transpose → nodal gap-ring gradients.
      for (let k = 0; k < Ngr; k++) gRot[k] = 0;
      for (let k = 0; k < Ngs; k++) gStat[k] = 0;
      for (let j = 0; j < Nu; j++) {
        const ang = j * hth;
        scatterCubic(gStat, permS, Ngs, thS0, stepS, ang, gLu[j]);
        scatterCubic(gRot, permR, Ngr, thR0, stepR, ang - phi, gR0u[j]);
      }
    }
    ...
    return {
      ...
      fluxCouplingInto,      // fluxCouplingInto(phi, cOut[Ngr+Ngs]) → d_gap  (axial-flux DOF)
      tripletCapacity,       // upper bound on triplets per stamp
      torque,                // torque(A_rotorGap, A_statorGap, phi) → N·m (Maxwell stress)
      torqueGrad,            // torqueGrad(A_rotorGap, A_statorGap, phi, gRot, gStat) → ∂T/∂A on the gap rings
      ...
      // ---- Harmonic-shaped fields consumed by the slice; all zero/empty for
      //      mortar — it carries no harmonics ----
  ...
  // for this engine, evaluated BEFORE build() (mesh sizing). 12·max(slots,poles).
  LIB.AirgapMortar = {
    build: build,
