...
//  plus a stacked horizontal force-arrow renderer. Lifted from the legacy
//  pidapp renderer so the pidcontrol single-cart and three-cart lessons can
//  share one body.
//
//  draw(ctx, opts)
...
//    Paint a vertical stack of horizontal force arrows above the cart.
//    slots = [ { color, F, label } ] — drawn from index 0 = bottom-most slot
//    upward. Arrows shorter than 0.5 px are skipped.
...
(function () {
  ...
  function forceArrows(ctx, x, yTopBaseline, slots, opts) {
    ...
    function fxDx(F) {
      let dx = F * pxPerN;
      if (Math.abs(dx) > capPx) dx = Math.sign(dx) * capPx;
      return dx;
    }
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      const yy = yTopBaseline - slotGap * (i + 1);
      ctx.strokeStyle = s.color;
      ctx.fillStyle   = s.color;
      ctx.lineWidth   = Math.max(1.5, fsmall * 0.14);
      ctx.beginPath();
      ctx.moveTo(x, yy);
      ctx.lineTo(x + dx, yy);
      ctx.stroke();
      const head = Math.max(6, fsmall * 0.7) * Math.sign(dx);
      const wing = Math.max(3, fsmall * 0.35);
      ctx.beginPath();
      ctx.moveTo(x + dx, yy);
      ctx.lineTo(x + dx - head, yy - wing);
      ctx.lineTo(x + dx - head, yy + wing);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(`${s.label} ${Number(s.F).toFixed(0)}`,
                   x + dx + Math.sign(dx) * 6, yy);
    }
  }
