(function () {
  ...
  //
  //  Strategy:
  //    1. Within one sector [0, sectorAngle], collect feature-boundary theta edges.
  //    2. Subdivide each inter-edge band into `divsPerBand` equal sub-intervals.
  //       divsPerBand is chosen so the total N_ang is approximately targetTotalDivs.
  //    3. Tile P_body sectors.
  //
  //  Returns { thetaColumns: Float64Array(Ntheta), Ntheta: number,
  //            divsPerBand: number, nBandsPerSector: number }
  //
  //  Feature boundary angles are EXACT column positions, so no element ever
  ...
  function buildAngularColumns(features, member, P_body, targetTotalDivs) {
    ...
    const nBands      = thetaEdges.length - 1; // >= 1

    // Compute divsPerBand so total columns ≈ targetTotalDivs, minimum 1
    const divsPerBand = Math.max(1, Math.round(targetTotalDivs / (P_body * nBands)));
    const Ntheta = P_body * nBands * divsPerBand;

    // Build columns for one sector
    const sectorCols = [];
    for (let b = 0; b < nBands; b++) {
      const t0 = thetaEdges[b];
      const t1 = thetaEdges[b + 1];
      for (let k = 0; k < divsPerBand; k++) {
        sectorCols.push(t0 + (t1 - t0) * k / divsPerBand);
      }
    }
    // sectorCols.length === nBands * divsPerBand

    // Tile P_body times
    const thetaColumns = new Float64Array(Ntheta);
    for (let p = 0; p < P_body; p++) {
      const offset = p * sectorAngle;
      for (let j = 0; j < sectorCols.length; j++) {
        thetaColumns[p * sectorCols.length + j] = offset + sectorCols[j];
      }
    }

    return { thetaColumns, Ntheta, divsPerBand, nBandsPerSector: nBands };
  }
