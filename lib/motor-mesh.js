(function () {
  ...
  function buildAngularColumns(features, member, P_body, targetTotalDivs, divsPerBandOverride) {
  ...
  function buildBodyMesh(features, member, collarR, gapSide, opts) {
    ...
    if (gapMinNodes !== null && N_gap < gapMinNodes) {
      ...
      angResult = buildAngularColumns(features, member, P_body, 0, newDivsPerBand);
    ...
    if (dofBudget !== null) {
      ...
      if (N_gap > targetNtheta) {
        const reduced = buildAngularColumns(features, member, P_body, 0, targetDpb);
