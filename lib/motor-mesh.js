(function () {
  ...
  //
  //  Strategy: LCM-based uniform grid with overflow fallback.
  //
  //  1. Start with baseN = P_body.
  //  2. For every feature boundary angle θ_k that is a rational fraction of 2π
  //     (p/q with q ≤ LCM_CAP), extend baseN = lcm(baseN, q) — this places a
  //     grid line exactly on every feature boundary so no element straddles one.
  //  3. If any boundary would push baseN above LCM_CAP (irrational-fraction
  //     boundaries, e.g. brushed-dc-pm at 0.144 rad = non-rational × 2π), set
  //     baseN = nBandsPerSector × P_body instead — this gives manageable element
  //     counts at the cost of approximate (non-exact) boundary alignment.
  //  4. Ntheta = baseN × divsPerBand; thetaColumns is a strict uniform grid
  //     j × 2π/Ntheta, so gapTheta is automatically uniform to machine precision.
  //
  //  divsPerBand is driven by targetTotalDivs or an explicit override.
  //
  //  Returns { thetaColumns: Float64Array(Ntheta), Ntheta: number,
  //            divsPerBand: number, nBandsPerSector: number }
  // ---------------------------------------------------------------------------

  const LCM_CAP = 1000; // overflow threshold: fall back to nBandsPerSector × P_body

  function buildAngularColumns(features, member, P_body, targetTotalDivs, divsPerBandOverride, minDivsPerBand) {
    ...
    // Compute baseN via LCM of rational denominators of all feature boundary angles.
    let baseN = P_body;
    ...
    if (overflowed) {
      // Fall back: one column per feature-boundary band per period replica.
      // This is still a multiple of P_body so the grid tiles correctly.
    ...
    if (divsPerBandOverride !== undefined) {
      divsPerBand = Math.max(1, Math.round(divsPerBandOverride));
    } else {
      divsPerBand = Math.max(1, Math.round(targetTotalDivs / baseN));
    }

    // Apply minDivsPerBand from curvature refinement (localised features)
    if (minDivsPerBand !== undefined && divsPerBand < minDivsPerBand) {
      divsPerBand = minDivsPerBand;
    }

    // Uniform grid: Ntheta = baseN × divsPerBand
  ...
  function buildBodyMesh(features, member, collarR, gapSide, opts) {
    ...
    // refine scales the target; at refine=0.25 we allow as few as 8 divs (floored
    // by the feature count × P_body anyway, which is usually larger).
    const minTotalDivs = Math.max(8, Math.ceil(refine * 32));

    // Build angular columns (non-uniform, feature-boundary-aligned).
    // The per-band subdivision count is derived from minTotalDivs.
    let angResult = buildAngularColumns(features, member, P_body, minTotalDivs, undefined, minDivsPerBand);
    ...
    // Strategy:
    //   1. Find the largest divsPerBand such that Nr * Ntheta <= dofBudget + P_body,
    //      where Ntheta = nBandsPerSector * P_body * divsPerBand.
    //   2. If even divsPerBand=1 doesn't fit (N_gap at floor), reduce Nr instead.
    ...
    if (dofBudget !== null) {
      const gapFloor = gapMinNodes !== null
        ...
        : nBandsPerSector * P_body; // minimum possible Ntheta (divsPerBand=1)

      const budget_with_slack = dofBudget + P_body;

      // Compute the largest divsPerBand that keeps Nr * Ntheta <= budget + P_body.
      // Ntheta = nBandsPerSector * P_body * dpb
      // => dpb <= (budget + P_body) / (Nr * nBandsPerSector * P_body)
      const bandsTotal = nBandsPerSector * P_body;
      const maxDpb = Math.floor(budget_with_slack / Math.max(Nr, 1) / bandsTotal);
      let targetDpb = Math.max(1, maxDpb);

      // targetNtheta must respect gapFloor
      let targetNtheta = bandsTotal * targetDpb;
      if (targetNtheta < gapFloor) {
        targetNtheta = gapFloor;
        targetDpb = Math.ceil(gapFloor / bandsTotal);
      }
      ...
      // Rebuild angular columns with reduced divsPerBand if it helps
