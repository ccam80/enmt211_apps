(function () {
  ...
  //      poles: int }
  //
  //  Also extracts windings Map and poles.
  // ---------------------------------------------------------------------------
  ...
  function physicsFromConfig(config) {
    ...
    // Resolve a ring's kinematic role (rotor/stator) from its geometric member
    // (inner/outer) + config.motion. A direct rotor/stator member passes through.
    function roleOf(ring) {
      const m = ring.member;
      if (m === 'rotor' || m === 'stator') return m;
      const mode = (config.motion && config.motion[m]) ? config.motion[m]
        : (m === 'inner' ? 'rotating' : 'static');
      return mode === 'rotating' ? 'rotor' : 'stator';
    }

    function addWound(ri, s, member) {
      windingsMap.set(ri, {
        kind: 'wound',
        m: s.m != null ? s.m : 3,
        p: s.p != null ? s.p : 2,
      ...
      if (s.p && s.p > poles) poles = s.p;
    }
    function addCage(ri, c, member) {
      windingsMap.set(ri, {
        kind: 'cage',
        m: 1,
        p: c.p != null ? c.p : 2,
    ...
    for (let ri = 0; ri < rings.length; ri++) {
      const ring = rings[ri];
      if (!ring) continue;
      const member = roleOf(ring);
      if (Array.isArray(ring.components)) {
        for (const comp of ring.components) {
          if (comp.winding && comp.winding.standard) addWound(ri, comp.winding.standard, member);
          else if (comp.cage) addCage(ri, comp.cage, member);
        }
        continue;
      }
      const w = ring.winding;
      if (!w) continue;
      if (w.standard) addWound(ri, w.standard, member);
      else if (w.cage) addCage(ri, w.cage, member);
    }
  ...
  //
  //  Radial physics (skin depth, saturation).
  // ---------------------------------------------------------------------------
  ...
  function assembleMeshMultiBand(
    ...
    // Per-element conductor-FEATURE id (one id per emitted slot wedge), so the
    // turns area-weighting can normalise PER SLOT, not per circuit. A distributed
    // winding has N_slots conductor features sharing one srcId (circuit); grouping
    // the area-weight by srcId would split each slot's turn count by N_slots.
    const featOfElem = new Int32Array(Ne).fill(-1);
    const condFeatId = new Map();   // feature object → dense slot id
    let   condFeatCount = 0;
    ...
    function classifyElement(rCent, thCent, inCollar) {
      ...
      if (feat === null) {
        ...
        let fid = condFeatId.get(feat);
        if (fid === undefined) { fid = condFeatCount++; condFeatId.set(feat, fid); }
        featOfElem[eIdx] = fid;
      } else {
        matId[eIdx] = 0;
      }
    ...
    // Area-weighted turns: turns[e] = feat.turns * area_e / A_slot, where A_slot is
    // the summed area of the elements of ONE conductor feature (one slot wedge),
    // keyed by featOfElem — NOT by srcId. This makes Jz_e = current·turns[e]/area_e
    // = current·feat.turns/A_slot uniform across each slot's cross-section, and the
    // per-slot turn count sums to feat.turns. Grouping by srcId (the circuit) would
    // divide a distributed winding's per-slot turns by the slots-per-phase count,
    // collapsing the effective ampere-turns (and the inductance) by that factor.
    // At this point turns[e] still holds the raw feat.turns value from
    // classifyElement; we now rewrite it as the area-weighted per-slot share.
    const areaPerFeat = new Map();   // featOfElem (slot id) -> Σ area_e
    ...
    for (let e = 0; e < Ne; e++) {
      ...
      elemArea[e] = a;
      const fid = featOfElem[e];
      if (fid >= 0) {
        areaPerFeat.set(fid, (areaPerFeat.get(fid) || 0) + a);
      }
    }
    for (let e = 0; e < Ne; e++) {
      const fid = featOfElem[e];
      if (fid >= 0) {
        const Atot = areaPerFeat.get(fid);
        if (Atot > 0) {
  ...
  //  Includes nuMaxSlice and poles in the signature.
  // ---------------------------------------------------------------------------
