(function () {
  ...
  //
  //  Provides prepare/assembly + static-rotor Brauer Newton.
  //  Public API: solve(thetaR, currents) / coggingTorque / backEmfDensity / etc.
  //
  //  Hard contract:
  ...
  function create(section, opts) {
    ...
    // both bodies. gapMinNodes wins when the caller
    ...
    // radial flux = ℓ·Ψ. The slice only assembles its coupling column c_s here
    // (coupledFluxRhsInto); the DOF itself is condensed in MotorStack.solveCoupled,
    // which couples the slices through the axial flux loop. opts.fluxDof (set by the
    // stack from the axial netlist) flags this slice as carrying one. Absent ⇒ nCut=0
    // ⇒ no flux code runs ⇒ bit-identical.
    const nCut = opts.fluxDof ? 1 : 0;
    ...
    // ------------ Scratch buffers + per-body triplet sizes ------------
    //
    ...
    // Cached-kernel K·A. Mathematically out[a] = Σ_b ν·K_geom[a][b]·A[b]
    // — using the cached K_geom (built via the same 2×2 Gauss formula as the
    // per-Gauss-point accumulation) gives a numerically equivalent answer
    ...
    // Assemble body tangent triplets via cached K_geom + cached
    ...
    // ============================================================================
    //  G5 Schur condensation (LINEAR path)
    //
    ...
    function linearMagnetOnlySolve(thetaR) {
      // Schur via Lagrange-augmented K_b'. K_b' factor is built
    ...
    const __internals = {
      ...
      // Schur condensation diagnostic exports.
      solverKb: solverKb,
      kbTrip: kbTrip,
      W_dense: W_dense,
      BtW_dense: BtW_dense,
