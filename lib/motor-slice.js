(function () {
  ...
  function create(section, opts) {
    ...
    function backEmfDensity(thetaR, currents, omega, opts2) {
      ...
      // Same gap Parseval measure as LIB.MotorCircuit.backEmf — the per-conductor
      // back-EMF density is a work-rate quantity, so Σ e_elem matches the
      // circuit-level π·ω·(dLdth·i + dλpm/dθ) back-EMF exactly.
      const GAP_MEASURE = Math.PI;
      let h;
      if (opts2.derivStep != null) {
        h = opts2.derivStep;
      } else {
