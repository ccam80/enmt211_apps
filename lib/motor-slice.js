  ...
  function create(section, opts) {
    ...
    function solveStaticRotor(thetaR, currents) {
      ...
      for (let it = 0; !converged && it < maxIter; it++) {
        ...
        // ‖A‖∞ denominator (body-only A) makes deltaNorm stricter than the
        // residual: it can lag 1-2 orders behind on the same physical field
        // (residual clears 1e-6 at iter 4, deltaNorm only at iter 6).
        // Treat a residual ≥10× below tolerance as converged regardless of the
        // step norm.
    ...
    // mapAngle == null ⇒ mapAngle = solveAngle (operating-point behaviour:
    // brush map tracks the rotor → TOTAL dλ/dθ ≈ 0 for a commutated machine,
    // which is the design invariant, NOT the torque-producing term).
