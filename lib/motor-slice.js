(function () {
  ...
  function create(section, opts) {
    ...
    function coupledAssemble(combinedA, currents, thetaR, residualOut) {
      ...
      prepareSolve(thetaR, currents);
      buildInto();
      // ‖f‖∞ — the field RHS (magnet + winding load) magnitude. The natural
      // scale for the field residual R_field = K·A + K_h·A − f: a relative
      // convergence test ‖R_field‖/‖f‖ is machine/load-independent.
      ...
      solverSat.setValues(scratch.V_combined);
      solverSat.factorize();
      // buildInto already recovered scratch.fullA_* (recoverAndNuInto) at this same
      // A — reuse it for the gap-ring extraction instead of recovering again.
      gapNodalAInto(rotorMesh,  scratch.fullA_rotor,  scratch.gapNodal_R);
      gapNodalAInto(statorMesh, scratch.fullA_stator, scratch.gapNodal_S);
      return gap.torque(scratch.gapNodal_R, scratch.gapNodal_S, thetaR);
    }
