(function () {
  ...
  function create(section, opts) {
    ...
    // (pm-stepper zero current): the torque is a 0.6 N·m PHANTOM at residualTol=1e-6
    ...
    const schurScratch = {
      ...
      // per φ, recomputed in linearSchurPrepare); gammaDenom = Btpᵀ·y (scalar
    ...
    function linearSchurPrepare(phi) {
      ...
      // Bug #3 gauge correction: precompute the γ-elimination invariants for
      // this φ. With S_pd = −S factored above, the bordered γ system
      //   [ S_pd  −Btp ] [h]   [ Bᵀz ]
      //   [ Btpᵀ    0  ] [γ] = [ pᵀf ]
      // is solved by Schur-eliminating γ:
      //   h  = S_pd⁻¹(Bᵀz + Btp·γ) = A_harm0 + γ·yGamma,  yGamma = S_pd⁻¹·Btp
      //   γ  = (pᵀf − Btpᵀ·A_harm0) / (Btpᵀ·yGamma)
      // yGamma and gammaDenom = Btpᵀ·yGamma are φ-dependent (through S_pd) but
      // RHS-independent, so we compute them once per φ here (one extra
      // triangular solve), keeping the per-RHS path O(nHarm) extra work.
      const yG = schurScratch.yGamma;
      denseCholeskySolve(S, Btp, nHarm, yG);
      let denom = 0;
      for (let i = 0; i < nHarm; i++) denom += Btp[i] * yG[i];
      schurScratch.gammaDenom = denom;
    }
    ...
    function solveStaticRotor(thetaR, currents) {
      ...
      for (let it = 0; !converged && it < maxIter; it++) {
        ...
        // (‖ΔA‖∞/‖A‖∞) is a secondary "solution stopped moving" guard, but its
        // ‖A‖∞ denominator is gap-method-dependent: the harmonic engine carries
        // large harmonic-amplitude DOFs that inflate ‖A‖∞ and shrink its
        // deltaNorm, while mortar's body-only A gives a larger (stricter)
        // deltaNorm — so mortar burned an extra Newton iter chasing deltaNorm
        // AFTER the residual was already 1-2 orders below tolerance (measured:
        // pmsm residual clears 1e-6 at iter 4, deltaNorm only at iter 6, same
        // physical field). Resolve by treating a residual ≥10× below tolerance
        // as converged regardless of the method-dependent step norm.
    ...
    // caller. The motional-derivative probe (extractCoeffs) passes the center
    ...
    // angle — the dominant cost of sweepInductance/extractCoeffs under mortar).
    ...
    // time-stepper reuse the exact viz path stack.solve produces WITHOUT a second
    ...
    function solve(thetaR, currents) {
      ...
      // through stack.solve for the time-stepper.
    ...
    // (rotor/field geometry); mapAngle drives the BRUSH-MAP wiring. They are
    // distinct precisely so the motional derivative can rotate the field
    // (solveAngle = θ±h) while freezing the commutator topology (mapAngle = θ).
    // mapAngle == null ⇒ mapAngle = solveAngle (operating-point / legacy
    // behaviour: brush map tracks the rotor → TOTAL dλ/dθ ≈ 0 for a commutated
    // machine, which is the design invariant, NOT the torque-producing term).
    function evaluateAt(angle, mapAngle) {
      const phi = angle;
      if (mapAngle == null) mapAngle = angle;
      ...
      // Magnet RHS (magnetization curl) is θ-invariant in ASSEMBLY (magnets are
      // a stator field source, no brush map applies to them); but λ_pm linked by
      // a mechanically-commutated armature uses turns_eff(e,θ) in EXTRACTION, so
      // we pass `angle` so dλ_pm/dθ comes out nonzero/correct.
      const A_pm = schurScratch.A_magnet;
      linearSchurSolve(_magnetRhsCached, A_pm);
      const lambdaPm = fluxLinkagesFromCombined(A_pm, mapAngle);
      ...
      const brushOn = !!(brushCtx && brushCtx.any);
      for (let j = 0; j < m; j++) {
        // For a mechanically-commutated armature the unit-current RHS Jz uses
        // turns_eff(e,θ) which is θ-dependent — rebuild it at the probe angle
        // (the cached θ-invariant RHS is only correct for non-mechanical
        // circuits). Cheap: one circuit's conductor elements per probe.
        let rhs;
        if (brushOn && brushCtx.isMechanical[j]) {
          rhs = _buildLinearRhsGlobal(
            assembleUnitCurrentRhs(rotorMesh,  j, mapAngle),
            assembleUnitCurrentRhs(statorMesh, j, mapAngle)
          );
        } else {
          rhs = _unitCurrentRhsCached[j];
        }
        linearSchurSolve(rhs, A_j);
        fluxLinkagesFromCombinedInto(A_j, lam_j, mapAngle);
        for (let i = 0; i < m; i++) {
          L[i * m + j] = lam_j[i];
        }
      }
    ...
    function extractCoeffs(thetaR, opts2) {
      ...
      // The brush-map angle is FROZEN at thetaR (the center) for all three so
      // the central difference returns the MOTIONAL derivative: the field /
      // geometry rotates with solveAngle = thetaR±h while the commutator wiring
      // stays fixed at the operating point. For a commutated machine this is the
      // torque- and back-EMF-producing term; the moving-map TOTAL derivative is
      // ≈0 by design (motional + switching cancel — the brush holds the armature
      // current-sheet stationary in the stator frame). For NON-mechanical
      // circuits brushCtx carries no map, so mapAngle is inert and the result is
      // byte-identical to the previous single-angle probe.
      const minus  = evaluateAt(thetaR - h, thetaR);
      const center = evaluateAt(thetaR,     thetaR);
      const plus   = evaluateAt(thetaR + h, thetaR);
    ...
    //  OWN linear solves; solve/extractCoeffs/coenergyTorque are untouched).
    ...
    const __internals = {
      ...
      K: gap.K,
      get schurProf() { return _schurProf; },
      derivedSlots: slots,
      derivedPoles: poles,
    ...
    let _lastFNorm = 0;   // ‖f‖∞ from the last coupledAssemble (field-residual scale)
    ...
    // Full assemble + factorize (back-compat: standalone solves and the gates).
    function coupledAssemble(combinedA, currents, thetaR, residualOut) {
      const T = coupledAssembleNoFactor(combinedA, currents, thetaR, residualOut);
      coupledFactorize();
      return T;
    }
    ...
    // store f(θ−h) and the small matvec K_h(θ−h)·A. MUST precede coupledAssemble
    // (prepareSolve re-stamps the gap, invalidating the factor; coupledAssemble
    // then re-stamps + factorizes at θ).
    ...
    // K_h(θ)·A and f(θ) are in scratch.KAh / scratch.rhs from coupledAssemble's
    // buildInto), and ∂λ/∂θ from the center fullA (reused — only the brush map
    // carries θ) and ∂T/∂θ from the center gapNodal (only gap.torque's φ carries
    // θ). No refactor, no field re-assemble. Call AFTER coupledAssemble at θ.
    ...
    // Solve the field tangent factored by coupledAssemble against rhs.
    ...
    // ∂T/∂A in combined free-DOF at the state left by coupledAssemble. Adjoint
    ...
    return {
      ...
      coupledAssemble: coupledAssemble,
