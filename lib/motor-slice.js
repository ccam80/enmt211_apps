(function () {
  ...
  function create(section, opts) {
    ...
    function bodyKtimesAMeshInto(body, νPerElem, fullAbody, outMesh) {
      const NnBody = body.nodes.length / 2;
      ...
      for (let e = 0; e < Ne; e++) {
        ...
        if (n3 === -1) {
          ...
          const dNdx = g.dNdx, dNdy = g.dNdy;
          let dAdx = 0, dAdy = 0;
          dAdx += dNdx[0] * fullAbody[n0]; dAdx += dNdx[1] * fullAbody[n1]; dAdx += dNdx[2] * fullAbody[n2];
          dAdy += dNdy[0] * fullAbody[n0]; dAdy += dNdy[1] * fullAbody[n1]; dAdy += dNdy[2] * fullAbody[n2];
          const fac = ν * area;
          outMesh[n0] += fac * (dNdx[0] * dAdx + dNdy[0] * dAdy);
          outMesh[n1] += fac * (dNdx[1] * dAdx + dNdy[1] * dAdy);
          outMesh[n2] += fac * (dNdx[2] * dAdx + dNdy[2] * dAdy);
        } else {
          ...
          for (let gs = 0; gs < 2; gs++) {
            for (let gt = 0; gt < 2; gt++) {
              ...
              let dAdx = 0, dAdy = 0;
              dAdx += jx[0]*fullAbody[n0]; dAdx += jx[1]*fullAbody[n1]; dAdx += jx[2]*fullAbody[n2]; dAdx += jx[3]*fullAbody[n3];
              dAdy += jy[0]*fullAbody[n0]; dAdy += jy[1]*fullAbody[n1]; dAdy += jy[2]*fullAbody[n2]; dAdy += jy[3]*fullAbody[n3];
              outMesh[n0] += w * (jx[0]*dAdx + jy[0]*dAdy);
              outMesh[n1] += w * (jx[1]*dAdx + jy[1]*dAdy);
              outMesh[n2] += w * (jx[2]*dAdx + jy[2]*dAdy);
              outMesh[n3] += w * (jx[3]*dAdx + jy[3]*dAdy);
            }
          }
        }
      }
    ...
    // Assemble body tangent triplets (with rank-1 dν augment) directly into
    // preallocated typed-array buffers. Output (I, J, V) ordering identical to
    // assembleBodyTangentTriplets.
    function assembleBodyTangentTripletsInto(body, νPerElem, dνPerElem, fullAbody, outI, outJ, outV) {
      const Ne = body.elems.length / 4;
      const elems = body.elems;
      const nodes = body.nodes;
      const matId = body.matId;
      const materials = body.materials;
      let p = 0;
      // First pass: the K_lin (ν·K_geom) emission, matches assembleInteriorPatternAndValues.
      for (let e = 0; e < Ne; e++) {
        const ν = νPerElem[e];
        const n0 = elems[4*e];
        const n1 = elems[4*e+1];
        ...
        if (n3 === -1) {
          ...
          if (!(area > 0)) continue;
          const dNdx = g.dNdx, dNdy = g.dNdy;
          const nL0 = n0, nL1 = n1, nL2 = n2;
          for (let a = 0; a < 3; a++) {
            const na = (a === 0) ? nL0 : (a === 1) ? nL1 : nL2;
            for (let b = 0; b < 3; b++) {
              const nb = (b === 0) ? nL0 : (b === 1) ? nL1 : nL2;
              const k = ν * (dNdx[a]*dNdx[b] + dNdy[a]*dNdy[b]) * area;
              outI[p] = na; outJ[p] = nb; outV[p] = k;
              p++;
            }
          }
        } else {
          const xN = [nodes[2*n0], nodes[2*n1], nodes[2*n2], nodes[2*n3]];
          const yN = [nodes[2*n0+1], nodes[2*n1+1], nodes[2*n2+1], nodes[2*n3+1]];
          // Kloc: stack-local 16-element array
          const Kloc00=0,Kloc01=1,Kloc02=2,Kloc03=3,Kloc04=4,Kloc05=5,Kloc06=6,Kloc07=7,
                Kloc08=8,Kloc09=9,Kloc10=10,Kloc11=11,Kloc12=12,Kloc13=13,Kloc14=14,Kloc15=15;
          const Kloc = _kloc16;
          for (let i = 0; i < 16; i++) Kloc[i] = 0;
          for (let gs = 0; gs < 2; gs++) {
            for (let gt = 0; gt < 2; gt++) {
              const j = q4Jacobian(xN, yN, GP4_1D[gs], GP4_1D[gt]);
              if (!(j.detJ > 0)) continue;
              ...
              for (let a = 0; a < 4; a++) {
                for (let b = 0; b < 4; b++) {
                  Kloc[a*4 + b] += w * (jx[a]*jx[b] + jy[a]*jy[b]);
                }
              }
            }
          }
          const nL = [n0, n1, n2, n3];
          for (let a = 0; a < 4; a++) {
            for (let b = 0; b < 4; b++) {
              outI[p] = nL[a]; outJ[p] = nL[b]; outV[p] = Kloc[a*4 + b];
              p++;
            }
          }
        }
      }
      // Second pass: rank-1 dν·gA·gAᵀ augment per iron element (matches
      // assembleBodyTangentTriplets ordering: emitted for every iron element
      // regardless of dν value).
      if (dνPerElem) {
        for (let e = 0; e < Ne; e++) {
          if (materials[matId[e]].kind !== "iron") continue;
          const dν = dνPerElem[e];
          const n0 = elems[4*e];
          const n1 = elems[4*e+1];
          const n2 = elems[4*e+2];
          const n3 = elems[4*e+3];
          let nLocal0, nLocal1, nLocal2, nLocal3, nLen, dNdx, dNdy, area;
          if (n3 === -1) {
            const xN = [nodes[2*n0], nodes[2*n1], nodes[2*n2]];
            const yN = [nodes[2*n0+1], nodes[2*n1+1], nodes[2*n2+1]];
            const g = triGradients(xN, yN);
            ...
            dNdx = g.dNdx;
            dNdy = g.dNdy;
            nLocal0 = n0; nLocal1 = n1; nLocal2 = n2; nLocal3 = -1; nLen = 3;
          } else {
            const xN = [nodes[2*n0], nodes[2*n1], nodes[2*n2], nodes[2*n3]];
            const yN = [nodes[2*n0+1], nodes[2*n1+1], nodes[2*n2+1], nodes[2*n3+1]];
            const j = q4Jacobian(xN, yN, 0, 0);
            ...
            dNdx = j.dNdx;
            dNdy = j.dNdy;
            nLocal0 = n0; nLocal1 = n1; nLocal2 = n2; nLocal3 = n3; nLen = 4;
          }
          if (!(area > 0)) continue;
          let dAdx = 0, dAdy = 0;
          for (let i = 0; i < nLen; i++) {
            const ni = (i === 0) ? nLocal0 : (i === 1) ? nLocal1 : (i === 2) ? nLocal2 : nLocal3;
            dAdx += dNdx[i] * fullAbody[ni];
            dAdy += dNdy[i] * fullAbody[ni];
          }
          // gA_i = dNdx[i]*dAdx + dNdy[i]*dAdy
          const gA0 = dNdx[0]*dAdx + dNdy[0]*dAdy;
          const gA1 = dNdx[1]*dAdx + dNdy[1]*dAdy;
          const gA2 = dNdx[2]*dAdx + dNdy[2]*dAdy;
          const gA3 = (nLen === 4) ? (dNdx[3]*dAdx + dNdy[3]*dAdy) : 0;
          const coef = 2 * dν * area;
          for (let a = 0; a < nLen; a++) {
            const na = (a === 0) ? nLocal0 : (a === 1) ? nLocal1 : (a === 2) ? nLocal2 : nLocal3;
            const gAa = (a === 0) ? gA0 : (a === 1) ? gA1 : (a === 2) ? gA2 : gA3;
            for (let b = 0; b < nLen; b++) {
              const nb = (b === 0) ? nLocal0 : (b === 1) ? nLocal1 : (b === 2) ? nLocal2 : nLocal3;
              const gAb = (b === 0) ? gA0 : (b === 1) ? gA1 : (b === 2) ? gA2 : gA3;
              outI[p] = na; outJ[p] = nb; outV[p] = coef * gAa * gAb;
              p++;
            }
          }
        }
    ...
    const _kloc16 = new Float64Array(16);
    ...
    function solveStaticRotor(thetaR, currents) {
      ...
      if (!enabled) {
        ...
        bodyKtimesAMeshInto(rotorMesh,  scratch.nu_rotor,  scratch.fullA_rotor,  scratch.KAR_mesh);
        bodyKtimesAMeshInto(statorMesh, scratch.nu_stator, scratch.fullA_stator, scratch.KAS_mesh);
