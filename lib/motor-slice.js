(function () {
  ...
  //  The public solve(thetaR, currents) / coggingTorque / backEmfDensity / etc.
  ...
  //  has 1.
  // --------------------------------------------------------------------------
  // Largest conductor/iron feature group (count sharing one rRange) on `member`.
  // For a tiled tooth/slot structure this equals the tooth/slot count — i.e. the
  // finest angular structure that body presents, which the mesh must resolve.
  function maxFeatureGroup(section, member) {
    const features = (section && section.features) ? section.features : [];
    const counts = new Map();
    for (let i = 0; i < features.length; i++) {
      const f = features[i];
      if (!f || f.member !== member) continue;
  ...
  function derivedSlots(section) {
    return maxFeatureGroup(section, "stator");
  }
  ...
  // --------------------------------------------------------------------------
  //  Brush / commutator current-sheet model (mechanically-commutated armatures)
  //
  //  A mechanically-commutated DC armature lives in the ROTOR frame: its
  //  conductor ±turn pattern is frozen in the rotor at mesh-build time. Stationary
  //  brushes continuously remap which physical rotor conductors carry +i vs −i so
  //  the armature current-sheet (MMF) is held FIXED in the STATOR frame. Without
  //  this remap the rotor-frozen sheet rotates with the rotor against a stationary
  //  stator field → dλ_pm/dθ ≡ 0 → no linear torque.
  //
  //  Effective per-conductor turns:
  //    turns_eff(e, θ) = |turns(e)| · brushProfile( α_s(e,θ) − α_brush )
  //    α_s(e, θ)       = α_rotor(e) + θ          (conductor angle in the STATOR frame)
  //
  //  brushProfile(ψ) is a trapezoidal ±1 wave of (p/2)-pole-pair periodicity: the
  //  sign(cos((p/2)ψ)) square wave with its zero-crossings replaced by a linear
  //  ramp of MECHANICAL width δc centred on each brush (inter-pole crossing). The
  //  ramp models the finite linear-commutation reversal of a coil shorted under a
  //  brush; δc→0 recovers the ideal square wave.
  //
  //  This is purely spatial/geometric and agnostic — it dispatches on
  //  commutation.mode === "mechanical" only, never on machine identity.
  // --------------------------------------------------------------------------

  // brushProfile(psi, pp, deltaC) → number in [-1, 1]
  //   psi    : mechanical angle α_s − α_brush (rad)
  //   pp     : pole pairs = p/2
  //   deltaC : commutation-zone MECHANICAL width (rad); 0 ⇒ ideal square wave
  //
  //  Electrical angle φ = pp·psi. The square wave sign(cos φ) is +1 for
  //  φ ∈ (−π/2, +π/2) (mod 2π) and −1 otherwise; the zero-crossings sit at
  //  φ = ±π/2 (mod π). Inside a half-width-δφ/2 band around each crossing we ramp
  //  linearly through 0. δφ = pp·deltaC is the electrical commutation width.
  function brushProfile(psi, pp, deltaC) {
    const phi = pp * psi;
    // Distance from the nearest zero-crossing of cos(φ), measured along φ.
    // cos(φ) zeros at φ = π/2 + n·π. Map φ to the signed distance d from the
    // nearest such crossing, d ∈ [−π/2, π/2], and the sign of the plateau we are
    // ramping toward.
    // s = cos(φ) carries the plateau sign; |φ_off| = how far past the crossing.
    const c = Math.cos(phi);
    if (!(deltaC > 0)) {
      // Ideal square wave.
      return c >= 0 ? 1 : (c < 0 ? -1 : 0);
    }
    const half = 0.5 * pp * deltaC;  // half electrical ramp width
    if (!(half > 0)) return c >= 0 ? 1 : -1;
    // Signed angular distance from φ to the nearest cos-zero-crossing (π/2 mod π).
    // Crossings land on multiples of π after shifting by −π/2.
    const g = phi - Math.PI / 2;
    const gw = g - Math.PI * Math.round(g / Math.PI);  // gw ∈ [−π/2, π/2]
    const absg = Math.abs(gw);
    const plateau = c >= 0 ? 1 : -1;
    if (absg >= half) return plateau;
    // Inside the commutation band: linear ramp 0 (at crossing) → plateau (edge).
    return plateau * (absg / half);
  }

  // --------------------------------------------------------------------------
  //  statorFieldAxis(statorBody, pp) → mechanical angle (rad) of the +field
  //  FLUX d-axis (the pole the field winding / magnet array drives flux out of).
  //
  //  Agnostic derivation as the spatial phase of the (p/2)-pole-pair fundamental
  //  of the stator's field source, expressed as a FLUX d-axis. Two physical
  //  field sources, dispatched on element material kind only (no machine
  //  branching) — but they relate to the flux d-axis DIFFERENTLY:
  //
  //    • PM magnets : the radial magnetization M̂·r̂ IS the flux source; its
  //      spatial fundamental peaks ON the flux d-axis. Use it directly:
  //        phasor_pm = Σ (mrMag·M̂·r̂)·e^{−i·pp·θ}     (peak at flux d-axis)
  //
  //    • field winding : the signed per-conductor turns form a CURRENT SHEET
  //      J(θ). The MMF it drives is the spatial integral F(θ)=∫J dθ′ (Ampère),
  //      and the air-gap flux (∝ MMF for a smooth gap) peaks where F peaks. The
  //      fundamental of F is +90° ELECTRICAL ahead of the fundamental of J
  //      (∫cos = sin): the current-sheet axis given by atan2(Σturns·sin,
  //      Σturns·cos) sits a quarter electrical period BEFORE the flux d-axis.
  //      So the winding's flux-d-axis phasor is the current-sheet phasor rotated
  //      by +π/2 elec:  phasor_w = (Σturns·e^{−i·pp·θ})·e^{+iπ/2} = i·(…).
  //      [Verified: brushOffset torque sweep peaks at +(π/2)/pp mech for the
  //       wound machines and at 0 for PM — exactly this quarter-period offset.]
  //
  //  Both contributions are summed as flux-d-axis phasors so a machine with both
  //  sources (none in the current fixtures, but kept general) combines correctly:
  //    α_field = (1/pp)·atan2( Im(Σ phasor), Re(Σ phasor) ).
  //  Returns 0 if no field source is found on the stator.
  // --------------------------------------------------------------------------
  function statorFieldAxis(statorBody, pp) {
    const Ne = statorBody.elems.length / 4;
    const nodes = statorBody.nodes;
    const elems = statorBody.elems;
    const matId = statorBody.matId;
    const materials = statorBody.materials;
    const magDir = statorBody.magDir;
    const srcId = statorBody.srcId;
    const turns = statorBody.turns;
    // Flux-d-axis phasor accumulators (Re = cos comp, Im = sin comp).
    let dRe = 0, dIm = 0, mag = 0;
    for (let e = 0; e < Ne; e++) {
      const n0 = elems[4*e], n1 = elems[4*e+1], n2 = elems[4*e+2], n3 = elems[4*e+3];
      let xc, yc;
      if (n3 === -1) {
        xc = (nodes[2*n0] + nodes[2*n1] + nodes[2*n2]) / 3;
        yc = (nodes[2*n0+1] + nodes[2*n1+1] + nodes[2*n2+1]) / 3;
      } else {
        xc = (nodes[2*n0] + nodes[2*n1] + nodes[2*n2] + nodes[2*n3]) / 4;
        yc = (nodes[2*n0+1] + nodes[2*n1+1] + nodes[2*n2+1] + nodes[2*n3+1]) / 4;
      }
      const th = Math.atan2(yc, xc);
      const rr = Math.hypot(xc, yc);
      const cps = Math.cos(pp * th);
      const sps = Math.sin(pp * th);
      const mat = materials[matId[e]];
      if (mat && mat.kind === "magnet" && mat.mrMag > 0 && magDir) {
        // PM: radial magnetization peaks ON the flux d-axis (no quarter shift).
        const radial = (rr > 0)
          ? (magDir[2*e] * (xc / rr) + magDir[2*e+1] * (yc / rr))
          : 0;
        const w = mat.mrMag * radial;
        if (w === 0) continue;
        dRe += w * cps;
        dIm += w * sps;
        mag += Math.abs(w);
      } else if (mat && mat.kind === "conductor" && srcId && srcId[e] >= 0) {
        // Field winding: current-sheet phasor Σturns·(cps − i·sps); the flux
        // d-axis is +π/2 elec ahead (multiply phasor by e^{+iπ/2} = i). With the
        // sign convention here (accumulating cos→Re, sin→Im), rotating the
        // current-sheet axis atan2(Σt·sin, Σt·cos) forward by +π/2 elec maps
        //   (Re' , Im') = (−Σt·sin , +Σt·cos)
        // for the flux-d-axis phasor.
        const w = turns[e];
        if (w === 0) continue;
        dRe += -w * sps;
        dIm +=  w * cps;
    ...
    if (mag === 0 || (dRe === 0 && dIm === 0)) return 0;
    return Math.atan2(dIm, dRe) / pp;
  }

  // alphaRotorOf(body) → Float64Array(Ne) of conductor element centroid angles
  // (mechanical, rotor body frame). Non-conductor elements get 0 (unused).
  function alphaRotorOf(body) {
    const Ne = body.elems.length / 4;
    const out = new Float64Array(Ne);
    const nodes = body.nodes, elems = body.elems, srcId = body.srcId;
    for (let e = 0; e < Ne; e++) {
      if (!srcId || srcId[e] < 0) continue;
      const n0 = elems[4*e], n1 = elems[4*e+1], n2 = elems[4*e+2], n3 = elems[4*e+3];
      let xc, yc;
      if (n3 === -1) {
        xc = (nodes[2*n0] + nodes[2*n1] + nodes[2*n2]) / 3;
  ...
  // --------------------------------------------------------------------------
  //  Axial-flux DOF Ψ: the body part of the coupling vector c_a = ∫ ν ∇N_a·∇χ dA
  //  and the self term d = ∫ ν |∇χ|² dA, where ∇χ = θ̂/(2πr) = (−y, x)/(2πr²) is the
  //  annulus harmonic 1-form (net radial flux = ℓ·Ψ).
  //
  //  The geometry integrals ∫∇N_a·∇χ and ∫|∇χ|² are FIXED — the body mesh does not
  //  move (rotor rotation is the gap zip, not node motion) — so precompute them once
  //  (precomputeBodyFluxGeometry) and per iter just ν-scale and scatter
  //  (accumulateBodyFluxCouplingInto). Only ν changes between Newton iters, so this is
  //  exact, not an approximation — and turns a per-iter Gauss assembly over every
  //  element into a single multiply-add pass. Q4: 2×2 Gauss; tri: 1-point centroid.
  // --------------------------------------------------------------------------
  const _GP = 1 / Math.sqrt(3);
  const _gpST = [[-_GP, -_GP], [_GP, -_GP], [_GP, _GP], [-_GP, _GP]];
  // gNode[4e+a] = ∫∇N_a·∇χ for local node a of element e; gD[e] = ∫|∇χ|² (both ν-free).
  function precomputeBodyFluxGeometry(body) {
    const Ne = body.elems.length / 4;
    const INV2PI = 1 / (2 * Math.PI);
    const gNode = new Float64Array(Ne * 4), gD = new Float64Array(Ne);
    for (let e = 0; e < Ne; e++) {
      const n0 = body.elems[4 * e], n1 = body.elems[4 * e + 1],
            n2 = body.elems[4 * e + 2], n3 = body.elems[4 * e + 3];
      if (n3 === -1) {
        const x0 = body.nodes[2*n0], y0 = body.nodes[2*n0+1];
        const x1 = body.nodes[2*n1], y1 = body.nodes[2*n1+1];
        const x2 = body.nodes[2*n2], y2 = body.nodes[2*n2+1];
        const g = triGradients([x0, x1, x2], [y0, y1, y2]);
        const xc = (x0 + x1 + x2) / 3, yc = (y0 + y1 + y2) / 3, r2 = xc * xc + yc * yc;
        const gx = -yc * INV2PI / r2, gy = xc * INV2PI / r2;
        gNode[4*e]   = g.area * (g.dNdx[0] * gx + g.dNdy[0] * gy);
        gNode[4*e+1] = g.area * (g.dNdx[1] * gx + g.dNdy[1] * gy);
        gNode[4*e+2] = g.area * (g.dNdx[2] * gx + g.dNdy[2] * gy);
        gD[e] = g.area * (gx * gx + gy * gy);
      } else {
        const xN = [body.nodes[2*n0], body.nodes[2*n1], body.nodes[2*n2], body.nodes[2*n3]];
        const yN = [body.nodes[2*n0+1], body.nodes[2*n1+1], body.nodes[2*n2+1], body.nodes[2*n3+1]];
        for (let q = 0; q < 4; q++) {
          const s = _gpST[q][0], t = _gpST[q][1];
          const jac = q4Jacobian(xN, yN, s, t);
          const sh = q4ShapeAt(s, t);
          let xc = 0, yc = 0;
          for (let i = 0; i < 4; i++) { xc += sh.N[i] * xN[i]; yc += sh.N[i] * yN[i]; }
          const r2 = xc * xc + yc * yc;
          const gx = -yc * INV2PI / r2, gy = xc * INV2PI / r2;
          const dJ = jac.detJ;   // 2×2 Gauss weight = 1
          for (let i = 0; i < 4; i++) gNode[4*e+i] += dJ * (jac.dNdx[i] * gx + jac.dNdy[i] * gy);
          gD[e] += dJ * (gx * gx + gy * gy);
        }
      }
    }
    return { gNode: gNode, gD: gD };
  }
  function accumulateBodyFluxCouplingInto(body, nu, geom, cFull) {
    const Ne = body.elems.length / 4, gNode = geom.gNode, gD = geom.gD;
    let dAcc = 0;
    for (let e = 0; e < Ne; e++) {
      const nue = nu[e];
      const n0 = body.elems[4 * e], n1 = body.elems[4 * e + 1],
            n2 = body.elems[4 * e + 2], n3 = body.elems[4 * e + 3];
      cFull[n0] += nue * gNode[4*e];
      cFull[n1] += nue * gNode[4*e+1];
      cFull[n2] += nue * gNode[4*e+2];
      if (n3 !== -1) cFull[n3] += nue * gNode[4*e+3];
      dAcc += nue * gD[e];
    }
    return dAcc;
  }
  ...
  function assembleInteriorMagnetLoadAndJzInto(body, currents, out, brushCtx, thetaR) {
    const NnBody = body.nodes.length / 2;
    for (let i = 0; i < NnBody; i++) out[i] = 0;
    const Ne = body.elems.length / 4;
    const brushOn = !!(brushCtx && brushCtx.any);
    ...
    for (let e = 0; e < Ne; e++) {
      ...
      if (mat && mat.kind === "conductor") {
        ...
        if (sid >= 0) {
          ...
          let turns = body.turns[e];
          // Brush/commutator spatial remap (mechanically-commutated armatures
          // only): replace the rotor-frozen ±turns sign with the brush-locked
          // sign turns_eff(e,θ) = |turns(e)|·brushProfile(α_rotor(e)+θ − α_brush).
          if (brushOn && brushCtx.isMechanical[sid]) {
            let xc = 0, yc = 0;
            for (let i = 0; i < nLocal.length; i++) {
              xc += body.nodes[2*nLocal[i]];
              yc += body.nodes[2*nLocal[i]+1];
            }
            xc /= nLocal.length; yc /= nLocal.length;
            const alpha = Math.atan2(yc, xc);
            const psi = alpha + (thetaR || 0) - brushCtx.brushAxis[sid];
            turns = Math.abs(turns) * brushProfile(psi, brushCtx.ppArr[sid], brushCtx.deltaC[sid]);
          }
  ...
  function create(section, opts) {
    ...
    // residualTol = 1e-7: relative ‖K·A − f‖∞ / ‖f‖∞ at convergence.
    // History: 1e-9 → 1e-6 (2026-05-29, after the Brauer-overflow false-convergence
    // path was removed) → 1e-7 (2026-06-02). The 1e-6 reasoning ("mesh torque error
    // ~1e-2..1e-3 dominates, so a tighter residual just chases noise") holds for
    // HIGH-signal torque — but it is WRONG for LOW-signal operating points. Measured
    // (a zero-current cogging point): the torque is a 0.6 N·m PHANTOM at residualTol=1e-6
    // and only collapses to its true ~1e-5 value once residual ≤ 1e-7 (stable from
    // 1e-7 down to 1e-14). At low ‖f‖ (cogging/detent, unexcited rest) the residual-
    // level field error IS the dominant torque term, not mesh error — so 1e-6 injects
    // phantom torque (spurious detent, false self-start). 1e-7 is the knee: kills the
    // phantom while staying above the mesh floor. (Costs ~1 extra Newton iter.)
    const newtonOpts = Object.assign(
      { maxIter: 8, tol: 1e-6, residualTol: 1e-7 },
      opts.newton || {}
    );
    ...
    // ---- Air-gap coupling engine (the one swap point) ---------------------
    // The air-gap coupling decides how the rotor and stator meshes talk across
    // the gap. `gapEngine` is the SINGLE place that selects it; the rest of the
    // slice is engine-agnostic, using only the GapEngine interface
    // (requiredGapNodes / build / stamp / stampInto / torque / projectInto /
    // dofMap + capability flags). To swap or add an engine, change this line.
    const gapEngine = LIB.AirgapMortar;
    // Gap-ring node floor the mesh must satisfy for this engine (pre-build). The gap
    // field must resolve the finest tiled tooth/slot structure on EITHER body — rotor
    // teeth or per-winding stator teeth, whichever is finer — so size from the larger
    // feature-group count across rotor and stator, not the stator slot count alone.
    const gapFeatureGroups = Math.max(slots, maxFeatureGroup(section, "rotor"));
    const requiredGapNodes = gapEngine.requiredGapNodes(gapFeatureGroups, poles);
    ...
    const gap = gapEngine.build(rotorGap, statorGap, { ell });
    ...
    // perBody / maxK come from the gap (sparse kList → 2·|kList|+1, not 2K+1).
    const perBody = gap.dofMap.harmonics.perBody;
    ...
    // ------------ Axial-flux DOF (Ψ) — Schur-condensed, off by default ------------
    // The net-radial-flux DOF Ψ carries the annulus harmonic 1-form ∇χ=θ̂/(2πr); net
    // radial flux = ℓ·Ψ. It is condensed out of the sparse solve (one extra RHS
    // K·x₂=c against the existing factorization + a 1×1 Schur), so nGlobal and the
    // sparsity pattern are UNCHANGED. opts.axialFlux={Fpm,Raxial} enables it for this
    // slice (P1 single-slice stand-in; the multi-slice netlist supersedes the
    // per-slice circuit). Absent ⇒ nCut=0 ⇒ no flux code runs ⇒ bit-identical.
    const axialFlux = opts.axialFlux || null;
    // The flux DOF is allocated when this slice carries one: either the P1
    // single-slice circuit (opts.axialFlux) or, in the coupled multi-slice path,
    // when the stack flags participation (opts.fluxDof) and drives Ψ via the netlist.
    const nCut = (axialFlux || opts.fluxDof) ? 1 : 0;
    const _fluxCFullR = nCut ? new Float64Array(rotorMesh.nodes.length / 2) : null;
    const _fluxCFullS = nCut ? new Float64Array(statorMesh.nodes.length / 2) : null;
    const _fluxCRfree = nCut ? new Float64Array(nRotorFree) : null;
    const _fluxCSfree = nCut ? new Float64Array(nStatorFree) : null;
    const _fluxCGap   = nCut ? new Float64Array(Ngr + Ngs) : null;
    const _fluxC      = nCut ? new Float64Array(nGlobal) : null;
    const _fluxX2     = nCut ? new Float64Array(nGlobal) : null;
    // ν-free body geometry integrals for the flux coupling — precomputed once.
    const _fluxGeomR  = nCut ? precomputeBodyFluxGeometry(rotorMesh) : null;
    const _fluxGeomS  = nCut ? precomputeBodyFluxGeometry(statorMesh) : null;
    let lastFlux = { Psi: 0, Rfield: 0, d: 0, cA: 0 };
    ...
    // ---- Mortar per-θ pattern cache ----
    // The mortar band re-zips as φ rotates → its sparsity PATTERN is φ-dependent,
    // so the engine cannot keep the single create-time analyze(). Instead we cache
    // the (fixed) body pattern and, at the top of each solveStaticRotor(θ), rebuild
    // the combined pattern = [body | band(θ)] and re-analyze solverSat once. The
    // band triplet COUNT is fixed (= harmTripCount, the zip always emits Ngr+Ngs
    // triangles), so V_combined / setValues are byte-identical to the harmonic path
    // (only the (I,J) coordinates change). nBodyTripPat = body triplet count.
    const nBodyTripPat = combinedTripCount - harmTripCount;
    const mortarBodyPatI = patternTrip.I.slice(0, nBodyTripPat);
    const mortarBodyPatJ = patternTrip.J.slice(0, nBodyTripPat);
    const mortarCombPatI = new Int32Array(combinedTripCount);
    const mortarCombPatJ = new Int32Array(combinedTripCount);
    mortarCombPatI.set(mortarBodyPatI, 0);
    mortarCombPatJ.set(mortarBodyPatJ, 0);
    // Boundary-triggered re-analyze: the band sparsity is piecewise-constant in φ
    // (it only changes when a rotor gap node crosses a stator gap node). Within an
    // interval the (I,J) are identical (only the sheared-triangle VALUES change),
    // so we skip the expensive symbolic when the band pattern matches the last one
    // analyzed — per solver (solverSat for Newton, solverLin for the L/dLdθ probe).
    let _mortarBandCacheSat = null;   // { I, J, n }
    let _mortarBandCacheLin = null;
    function _mortarSameBand(cache, bandI, bandJ, n) {
      if (!cache || cache.n !== n) return false;
      const cI = cache.I, cJ = cache.J;
      for (let t = 0; t < n; t++) { if (cI[t] !== bandI[t] || cJ[t] !== bandJ[t]) return false; }
      return true;
    }
    function _mortarStoreBand(bandI, bandJ, n) {
      const I = new Int32Array(n), J = new Int32Array(n);
      for (let t = 0; t < n; t++) { I[t] = bandI[t]; J[t] = bandJ[t]; }
      return { I: I, J: J, n: n };
    }
    ...
    const scratch = {
      ...
      harmR_a: new Float64Array(gap.K + 1),
      harmR_b: new Float64Array(gap.K + 1),
      harmS_a: new Float64Array(gap.K + 1),
      harmS_b: new Float64Array(gap.K + 1),
    ...
    const _solveResult = { torque: 0, fluxLinkages: null, field: _solveResultField, iters: 0 };
    ...
    // outM/outW (optional): capture the FIXED expansion map — for each emitted
    // output triplet q, outM[q] = source input-triplet index t and outW[q] = wi·wj.
    // The map is geometry-only (depends solely on the slave/master/renumber tables
    // and the input (i,j)=element connectivity), so building it once lets the hot
    // path skip this whole expand+renumber loop: V_out[q] = outW[q]·inV[outM[q]].
    function applyConstraintsToTripletsInto(inI, inJ, inV, nIn, slaveTable, renumBody, outI, outJ, outV, outM, outW) {
      const isSlave = slaveTable.isSlave;
      const masterL = slaveTable.masterL;
      const masterR = slaveTable.masterR;
      const weightL = slaveTable.weightL;
    ...
    //
    //  Per call:
    //    linearSchurSolve(fBodyFree, outA): z = K_b⁻¹·f_b (1 back-sub),
    ...
    // -------- Bug #3 gauge-correction invariant: Btp = Bᵀ·p --------
    //
    // p is the rotor-uniform body mode: p[i]=1 for i in the rotor free-DOF
    // range [0, nRotorFree), 0 elsewhere. It spans null(K_b) (the rotor body
    // has no Dirichlet pin). The full bordered solve leaves K_b singular and
    // lets the harmonic k=0 coupling resolve this null mode; the Schur
    // condensation must reproduce that EXACT gauge, not a substituted
    // ΣA_rotor=0 gauge (which drops the null-mode amplitude γ and corrupts L).
    //
    // The condensed system that is algebraically identical to the full
    // bordered solve is the (nHarm+1) bordered system in [h; γ]:
    //
    //   S·h + (Bᵀp)·γ = −Bᵀ·z         (z = K_b⁺·f_body, S = D − BᵀK_b⁺B)
    //   (Bᵀp)ᵀ·h      = pᵀ·f_body
    //
    // and the body recovery gains the +γ·p term: A_b = z − W·h + γ·p.
    //
    // Btp is φ-invariant (B is φ-invariant; only D rotates with φ). It is the
    // k=0 rotor harmonic projection row of B, restricted to rotor body DOFs.
    // (Σ over B entries whose bodyIdx is a rotor free DOF, accumulated per
    // harmonic index.)
    const Btp = new Float64Array(nHarm);
    for (let t = 0; t < B.n; t++) {
      if (B.bodyIdx[t] < nRotorFree) Btp[B.harmIdx[t]] += B.value[t];
    }
    ...
    const schurScratch = {
      ...
      // Bug #3 gauge correction: y = S_pd⁻¹·Btp (one extra triangular solve
      // per φ); gammaDenom = Btpᵀ·y (scalar
      // Schur complement on the γ unknown).
      yGamma:   new Float64Array(nHarm),
      gammaDenom: 0,
      // linearSchurSolve output buffer for the magnet-only solve (coggingTorque).
    ...
    function linearSchurSolve(fBody, outA) {
      // Mortar: there is NO harmonic block to condense — the gap coupling is the
      // sparse band living in the body DOFs, already in solverLin's factored
      // system (nGlobal == nBody). The Schur path below would solve solverKb
      // (gap-coupling-FREE body stiffness) and add nothing (nHarm=0), returning
      // a φ-invariant rotor/stator-decoupled field → dL/dθ≡0. Route straight to
      // the band-aware plain factor instead (factored by _ensureLinearFactorAt).
      solverLin.solveInto(fBody, outA);
    }
    ...
    // ========================================================================
    //  Newton-path Schur condensation (opts.schur).
    //
    //  The saturated Newton inner solve factorizes the full bordered tangent
    //  [J_b B; Bᵀ D] every iteration; the dense gapN×perBody B block fills the
    //  sparse factor (≈6× the body nnz). This condenses it: factor the body
    //  tangent J_b alone (sparse, body-only) + a small dense perBody Schur
    //  complement S = D − BᵀJ_b⁻¹B. Exact reduction of the SAME tangent — no
    //  truncation; the converged A is identical to the combined path.
    //
    //  Unlike the LINEAR Schur path above (solverKb / W_dense / BtW_dense are
    //  built ONCE because K_b(ν₀) is fixed), the Newton tangent J_b changes
    //  every iteration (rank-1 dν saturation updates), so W = J_b⁺B and S are
    //  recomputed per iter on a fresh body-tangent factor (solverJb). B, Btp,
    //  B_colStart, B_order are REUSED — the gap coupling is ν- and φ-invariant
    //  (only D rotates with φ, and φ is fixed within a solve).
    //
    //  The body recovery includes the SAME Bug #3 null-mode γ correction as
    //  linearSchurSolve: J_b carries the rotor-uniform null space (no rotor
    //  Dirichlet pin), resolved via the harmonic k=0 coupling, NOT ΣA_rotor=0.
    //  The only generalization vs linearSchurSolve is a non-zero harmonic RHS
    //  f_h (the Newton residual's harmonic rows): g = Bᵀz − f_h.
    // ========================================================================
    let newtonSchurStep = null;
    ...
    function buildConductorIndex(body) {
      ...
      const alphaArr = new Float64Array(total);   // conductor centroid angle (rotor frame)
      const nodes = body.nodes, elems = body.elems;
      ...
      for (let e = 0; e < Ne; e++) {
        ...
        alphaArr[pos] = Math.atan2(yc, xc);
        cursor[sid]++;
      }
      return { start, elemIdx, turns: turnsArr, alpha: alphaArr };
    }
    const conductorIndexRotor  = buildConductorIndex(rotorMesh);
    const conductorIndexStator = buildConductorIndex(statorMesh);

    // ------------ Brush / commutator current-sheet context (agnostic) ------------
    //
    // For every circuit whose commutation.mode === "mechanical" (a physical
    // commutator + brushes on a rotor-frame armature) we build a spatial brush
    // map that remaps conductor current SIGN per rotor angle so the armature
    // current-sheet is brush-locked (stationary in the stator frame). Electronic
    // and sequencer modes are BRUSHLESS and are NOT remapped here — their
    // commutation is the stator-side drive, already modeled by the phase/sector
    // gate in lib/excitation.js. Dispatch is on commutation.mode only.
    //
    //   isMechanical[k] : 1 if circuit k is a mechanically-commutated armature
    //   brushAxis[k]    : α_brush (mechanical rad), fixed in the stator frame
    //   deltaC[k]       : commutation-zone mechanical width (rad)
    //   ppArr[k]        : pole pairs p/2 for circuit k
    // α_brush = α_field + brushOffset (ideal quadrature built into brushProfile).
    const brushCtx = (function buildBrushCtx() {
      const circuits = (opts.circuits && Array.isArray(opts.circuits)) ? opts.circuits : null;
      const isMechanical = new Uint8Array(nCircuits);
      let any = false;
      if (!circuits) return { any: false, isMechanical };
      const brushAxis = new Float64Array(nCircuits);
      const deltaC    = new Float64Array(nCircuits);
      const ppArr     = new Float64Array(nCircuits);
      // Default commutation zone ≈ one slot pitch (2π/Qslots) mechanical, from
      // the rotor armature's slot count. derivedSlots counts the stator; the
      // rotor slot count is the conductor-feature group size on the rotor.
      const rotorSlots = (function () {
        const counts = new Map();
        const feats = (section.features || []);
        for (const f of feats) {
          if (!f || f.member !== "rotor" || f.kind !== "conductor") continue;
          const key = (f.rRange ? f.rRange[0] + "|" + f.rRange[1] : "0|0");
          counts.set(key, (counts.get(key) || 0) + 1);
        }
        let s = 1;
        for (const v of counts.values()) if (v > s) s = v;
        return s;
      })();
      for (let k = 0; k < nCircuits && k < circuits.length; k++) {
        const c = circuits[k];
        const comm = c && c.commutation;
        if (!comm || comm.mode !== "mechanical") continue;
        isMechanical[k] = 1;
        any = true;
        const pk = (comm.poles != null && comm.poles >= 2) ? comm.poles : poles;
        const pp = pk / 2;
        ppArr[k] = pp;
        const brushOffset = (comm.brushOffset != null) ? comm.brushOffset : 0;
        const dC = (comm.deltaC != null && comm.deltaC > 0)
          ? comm.deltaC
          : (rotorSlots > 0 ? TWO_PI / rotorSlots : 0);
        deltaC[k] = dC;
        const alphaField = statorFieldAxis(statorMesh, pp);
        // Ideal quadrature, derived: brushProfile reverses the conductor sign at
        // α_brush, so the armature CURRENT-SHEET reversal (the brush neutral
        // axis) sits at α_brush and the armature MMF PEAK sits 90° elec away
        // from it. Torque ∝ sin(angle between armature-MMF axis and field axis),
        // maximised when the MMF peak is 90° elec from the field axis — i.e.
        // when the brush neutral axis is ALIGNED with the field axis. So ideal
        // quadrature is α_brush = α_field (the +90° elec is already built into
        // the brushProfile geometry; adding it again would double-count and give
        // ZERO average torque, as the offset sweep confirmed). brushOffset then
        // advances/retards the brushes about that ideal point.
        brushAxis[k] = alphaField + brushOffset;
      }
      return { any, isMechanical, brushAxis, deltaC, ppArr };
    })();
    ...
    // Prepare the field solve at (thetaR, currents): assemble the RHS into
    // scratch.rhs, stamp+remap the gap band into scratch.harmTrip_* (count
    // scratch.harmTrip_n), and re-analyze the mortar pattern when the band moved.
    // buildInto() + the Newton/bypass then run off this prepared scratch. Hoisted
    // so the coupled Newton can re-prepare per outer iteration (i and θ change).
    function prepareSolve(thetaR, currents) {
      const phi = thetaR;
      const f = scratch.rhs;
      for (let i = 0; i < nGlobal; i++) f[i] = 0;
      assembleInteriorMagnetLoadAndJzInto(rotorMesh,  currents, scratch.fR_mesh, brushCtx, phi);
      assembleInteriorMagnetLoadAndJzInto(statorMesh, currents, scratch.fS_mesh, brushCtx, phi);
      ...
      // Mortar: re-run the symbolic ONLY when the band pattern actually changed
      // (crossed an interval boundary); within an interval reuse the analyze.
      const hI = scratch.harmTrip_I, hJ = scratch.harmTrip_J;
      if (!_mortarSameBand(_mortarBandCacheSat, hI, hJ, harmN)) {
        for (let t = 0; t < harmN; t++) {
          mortarCombPatI[nBodyTripPat + t] = hI[t];
          mortarCombPatJ[nBodyTripPat + t] = hJ[t];
        }
        const patN = nBodyTripPat + harmN;
        solverSat.setPattern(nGlobal,
          (patN === combinedTripCount) ? mortarCombPatI : mortarCombPatI.subarray(0, patN),
          (patN === combinedTripCount) ? mortarCombPatJ : mortarCombPatJ.subarray(0, patN));
        solverSat.analyze();
        _mortarBandCacheSat = _mortarStoreBand(hI, hJ, harmN);
      }
    }

    // Per-Newton-iteration assemble (hoisted to slice scope so both
    // solveStaticRotor and the coupled Newton drive one field iteration). Reads
    // the current iterate scratch.A_iter, the prepared RHS scratch.rhs, and the
    // gap stamp scratch.harmTrip_* (count scratch.harmTrip_n); writes the tangent
    // scratch.V_combined and the residual scratch.residual.
    // Steps 1–2: recover full A per body + ν,dν per element. Shared by the full
    // tangent assemble (buildInto) and the residual-only path.
    function recoverAndNuInto() {
      recoverFullFromBigA_Into(scratch.A_iter, 0, nRotorFree, slaveTableR, renumR, NnR_full, scratch.fullA_rotor);
      recoverFullFromBigA_Into(scratch.A_iter, nRotorFree, nStatorFree, slaveTableS, renumS, NnS_full, scratch.fullA_stator);
    ...
    // Step 6: residual R_field = K_lin(ν)·A − f + K_h·A (secant K, not Newton
    // tangent). Reads scratch.{nu_*, fullA_*, A_iter, rhs, harmTrip_*}; writes
    // scratch.residual. Shared so the residual logic never drifts between the
    // full assemble and the residual-only path.
    function assembleFieldResidualInto() {
      const A_iter = scratch.A_iter, f = scratch.rhs, harmN = scratch.harmTrip_n;
      ...
      for (let i = 0; i < nRotorFree; i++) residual[i] = scratch.KAR_free[i] + KAh[i] - f[i];
      for (let i = 0; i < nStatorFree; i++) residual[nRotorFree + i] = scratch.KAS_free[i] + KAh[nRotorFree + i] - f[nRotorFree + i];
      for (let i = nRotorFree + nStatorFree; i < nGlobal; i++) residual[i] = KAh[i] - f[i];
    }

    // Fixed constraint-expansion map (source mesh-triplet index + weight per output
    // body-pattern slot), built once. The mesh→free-DOF expansion is geometry-only,
    // so the per-iter tangent collapses to V_combined[t] = w[t]·meshV[m[t]].
    let _xpandMapR = null, _xpandWR = null, _xpandNR = 0;
    let _xpandMapS = null, _xpandWS = null, _xpandNS = 0;
    function _buildExpansionMaps() {
      // Emit+constrain once on the probe state — only the (source,weight) map is
      // captured; the probe values are discarded. Structure (skips/order) matches
      // the hot path because it is the same two functions.
      const nR_mesh = assembleBodyTangentTripletsInto(rotorMesh, rotorKernels,
        _probeNuR, _probeDnuR, _probeAR,
      ...
      const nS_mesh = assembleBodyTangentTripletsInto(statorMesh, statorKernels,
        _probeNuS, _probeDnuS, _probeAS,
        scratch.tripS_mesh_I, scratch.tripS_mesh_J, scratch.tripS_mesh_V);
      const mR = new Int32Array(maxBodyTripR_constr), wR = new Float64Array(maxBodyTripR_constr);
      const mS = new Int32Array(maxBodyTripS_constr), wS = new Float64Array(maxBodyTripS_constr);
      _xpandNR = applyConstraintsToTripletsInto(
        scratch.tripR_mesh_I, scratch.tripR_mesh_J, scratch.tripR_mesh_V, nR_mesh,
        slaveTableR, renumR,
        scratch.tripR_constr_I, scratch.tripR_constr_J, scratch.tripR_constr_V, mR, wR);
      _xpandNS = applyConstraintsToTripletsInto(
        scratch.tripS_mesh_I, scratch.tripS_mesh_J, scratch.tripS_mesh_V, nS_mesh,
        slaveTableS, renumS,
        scratch.tripS_constr_I, scratch.tripS_constr_J, scratch.tripS_constr_V, mS, wS);
      _xpandMapR = mR; _xpandWR = wR;
      _xpandMapS = mS; _xpandWS = wS;
    }

    function buildInto() {
      if (_xpandMapR === null) _buildExpansionMaps();
      const harmN = scratch.harmTrip_n;
      // 1–2. Recover full A + ν,dν per element.
      recoverAndNuInto();

      // 3. Body tangent VALUES (mesh-indexed) into scratch — the (I,J) writes are
      // unused (the body pattern is fixed); only tripR/S_mesh_V feed the map below.
      assembleBodyTangentTripletsInto(rotorMesh, rotorKernels,
        scratch.nu_rotor, scratch.dnu_rotor, scratch.fullA_rotor,
        scratch.tripR_mesh_I, scratch.tripR_mesh_J, scratch.tripR_mesh_V);
      assembleBodyTangentTripletsInto(statorMesh, statorKernels,
        scratch.nu_stator, scratch.dnu_stator, scratch.fullA_stator,
        scratch.tripS_mesh_I, scratch.tripS_mesh_J, scratch.tripS_mesh_V);

      // 4–5. Constraint expansion + concat via the fixed map → V_combined directly:
      // each body slot t is w[t]·meshV[m[t]] (= applyConstraintsToTriplets' output,
      // same product) — the mesh→free-DOF expansion is geometry-only, built once.
      const Vc = scratch.V_combined;
      const mR = _xpandMapR, wR = _xpandWR, mvR = scratch.tripR_mesh_V, nR = _xpandNR;
      const mS = _xpandMapS, wS = _xpandWS, mvS = scratch.tripS_mesh_V, nS = _xpandNS;
      for (let t = 0; t < nR; t++) Vc[t]      = wR[t] * mvR[mR[t]];
      for (let t = 0; t < nS; t++) Vc[nR + t] = wS[t] * mvS[mS[t]];
      const nBody = nR + nS;
      for (let t = 0; t < harmN; t++) Vc[nBody + t] = scratch.harmTrip_V[t];

      // 6. Residual.
      assembleFieldResidualInto();
    }

    // Residual-only assemble: steps 1–2 + 6, SKIPPING the Newton tangent (steps
    // 3–5, the dominant cost). For the θ-border FD, which differences R_field
    // only — it never touches the tangent. Leaves scratch.residual valid.
    function buildResidualOnlyInto() {
      recoverAndNuInto();
      assembleFieldResidualInto();
    }

    // Assemble the axial-flux coupling c (nGlobal) and self term d for the current
    // element reluctivities (scratch.nu_rotor / nu_stator). c = ∫ν∇N·∇χ over rotor
    // body + stator body + gap band; d = ∫ν|∇χ|². Fills _fluxC, returns d.
    function assembleFluxCoupling(phi) {
      const c = _fluxC;
      for (let i = 0; i < nGlobal; i++) c[i] = 0;
      const NnR = rotorMesh.nodes.length / 2, NnS = statorMesh.nodes.length / 2;
      for (let i = 0; i < NnR; i++) _fluxCFullR[i] = 0;
      for (let i = 0; i < NnS; i++) _fluxCFullS[i] = 0;
      let d = accumulateBodyFluxCouplingInto(rotorMesh, scratch.nu_rotor, _fluxGeomR, _fluxCFullR);
      d += accumulateBodyFluxCouplingInto(statorMesh, scratch.nu_stator, _fluxGeomS, _fluxCFullS);
      applyConstraintsToFInto(_fluxCFullR, slaveTableR, renumR, NnR, nRotorFree, _fluxCRfree);
      applyConstraintsToFInto(_fluxCFullS, slaveTableS, renumS, NnS, nStatorFree, _fluxCSfree);
      for (let i = 0; i < nRotorFree; i++) c[i] += _fluxCRfree[i];
      for (let i = 0; i < nStatorFree; i++) c[nRotorFree + i] += _fluxCSfree[i];
      for (let i = 0; i < Ngr + Ngs; i++) _fluxCGap[i] = 0;
      d += gap.fluxCouplingInto(phi, _fluxCGap);
      for (let i = 0; i < Ngr; i++) { const g = gapLocalToGlobalRotor[i]; if (g >= 0) c[g] += _fluxCGap[i]; }
      for (let j = 0; j < Ngs; j++) { const g = gapLocalToGlobalStator[j]; if (g >= 0) c[g] += _fluxCGap[Ngr + j]; }
      return d;
    }

    // Schur-condense the flux DOF onto a converged field A (= K⁻¹f). One extra RHS
    // x₂=K⁻¹c against the live factorization, a 1×1 Schur for Ψ, then correct A.
    // axialFlux.Psi prescribes the flux (verification); else MMF-driven via the
    // slice-local R_axial+F_pm stand-in. R_field = d − cᵀK⁻¹c is the radial reluctance.
    function applyFluxDOF(phi, A) {
      const d = assembleFluxCoupling(phi);
      const c = _fluxC, x2 = _fluxX2;
      solverSat.solveInto(c, x2);                       // x2 = K⁻¹ c
      let cA = 0, cx2 = 0;
      for (let i = 0; i < nGlobal; i++) { cA += c[i] * A[i]; cx2 += c[i] * x2[i]; }
      const Rfield = d - cx2;
      let Psi;
      if (axialFlux.Psi != null) {
        Psi = axialFlux.Psi;                            // prescribed-flux mode
      } else {
        const Fpm = axialFlux.Fpm || 0, Rax = axialFlux.Raxial || 0;
        Psi = (Fpm - cA) / (Rfield + Rax);
      }
      for (let i = 0; i < nGlobal; i++) A[i] -= Psi * x2[i];
      lastFlux = { Psi: Psi, Rfield: Rfield, d: d, cA: cA };
    }
    ...
    function solveStaticRotor(thetaR, currents) {
      ...
      if (!enabled) {
        ...
        if (nCut > 0) applyFluxDOF(phi, A);

        const converged = residual < newtonOpts.residualTol;
        const summary = { iters: 1, deltaNorm: 0, residual, converged };
        lastNewton = summary;
        return { A, iters: 1, deltaNorm: 0, residual, converged };
      }
      ...
      // buildInto() is hoisted to slice scope (above) — it reads scratch.A_iter /
      // scratch.rhs / scratch.harmTrip_n, all populated by this function before
      // the Newton loop below, so behaviour is unchanged.
      ...
      // bounded ν the failure mode stays out of NaN, so this is a
      // convergence aid rather than a NaN guard — but both are needed:
      // bounded ν alone leaves Newton lurching back and forth across the
      // knee on cold-start; damping alone is wasted if ν blows up.
      //
      ...
      for (let it = 0; !converged && it < maxIter; it++) {
        ...
        // (Lagged-tangent chord disabled: it breaks convergence on stiff
        // machines. Always full Newton — refactor the tangent every iter.)

        // Relative step norm: ‖α·dA‖∞ over the larger of the pre- and post-step
        // iterate ‖A‖∞. Taking the max keeps the ratio well-defined when either
        // end is ~0 — notably a cold start (A_prev≡0), where dividing by the
        // pre-step norm alone collapses onto the eps floor and explodes; there
        // the post-step norm (A_iter = A_prev + α·dA) anchors it to ~1.
        let aMax = 0, dMax = 0;
        for (let i = 0; i < nGlobal; i++) {
          const ai = Math.abs(A_prev[i]);
          const an = Math.abs(A_iter[i]);
          const di = Math.abs(alpha * dA[i]);
          if (ai > aMax) aMax = ai;
          if (an > aMax) aMax = an;
          if (di > dMax) dMax = di;
        }
        ...
        // Convergence. The residual (‖K(ν)·A − f‖∞/‖f‖∞, the discrete PDE
        // force-balance) is the physical criterion — the pre-Newton warm-start
        // fast-path above converges on it alone. The deltaNorm step-criterion
        // (‖ΔA‖∞/‖A‖∞) is a secondary "solution stopped moving" guard that lags
        // the residual: it can sit 1-2 orders behind on the same physical field
        // (residual clears 1e-6 at iter 4, deltaNorm only at iter 6), and on a
        // 1-iter cold-start convergence it reads ~1 (the field moved its full
        // magnitude) while the residual is already at machine zero. So treat a
        // residual ≥10× below tolerance as converged regardless of the step norm.
        if (residualScaled < residualTol &&
            (deltaNorm < tol || residualScaled < residualTol * 0.1)) {
          converged = true;
          break;
        }
      }
      ...
      lastNewton = { iters, deltaNorm, residual: residualScaled, converged };
      return { A: A_out, iters, deltaNorm, residual: residualScaled, converged };
    }
    ...
    // mapAngle (5th param) decouples the BRUSH-MAP angle from the rotor/solve
    // angle (thetaR). The brush wiring is the ONLY thetaR-dependence in this
    // pickup, so default mapAngle = thetaR ⇒ byte-identical for every existing
    // caller. A motional probe passes the center
    // angle as mapAngle while the field/geometry is solved at thetaR±h, so the
    // wiring (commutator topology) is held FROZEN at the center: the resulting
    // dλ/dθ is the MOTIONAL term (conductors cutting flux at fixed wiring), not
    // the ≈0 TOTAL (motional + switching) that a moving brush map produces.
    function fluxLinkagesFromFullAInto(fullAR, fullAS, out, thetaR, mapAngle) {
      const brushOn = !!(brushCtx && brushCtx.any);
      const th = (mapAngle != null) ? mapAngle : (thetaR || 0);
      ...
      // Rotor body
      {
        ...
        const alpha = idx.alpha;
        const elems = rotorMesh.elems;
        for (let k = 0; k < nCircuits; k++) {
          const lo = start[k], hi = start[k + 1];
          const mech = brushOn && brushCtx.isMechanical[k];
          const axis = mech ? brushCtx.brushAxis[k] : 0;
          const pp   = mech ? brushCtx.ppArr[k]    : 0;
          const dC   = mech ? brushCtx.deltaC[k]   : 0;
          let acc = 0;
          for (let p = lo; p < hi; p++) {
            const e = elemIdx[p];
            const n0 = elems[4*e];
            ...
            const t = mech
              ? Math.abs(turns[p]) * brushProfile(alpha[p] + th - axis, pp, dC)
              : turns[p];
            acc += Aavg * t;
          }
          out[k] += acc;
      ...
      // Stokes/Ampère bridge): the physical webers, no π scaling. An independent
      // field-side computation λ = N·ell·(Ā_go − Ā_ret) matches this to ratio
      // 1.0; motor-run consumes it for back-EMF/circuit.
    ...
    //  only invalidator. linearMagnetOnlySolve and backEmfDensity reuse the
    //  factor for back-to-back calls at the same phi.
    //
    //  _magnetRhsCached — the combined nGlobal magnet-load RHS. It is phi-
    //  invariant (magnet vectors are in the rotor frame), so assemble it ONCE
    //  at create-time.
    // ----------------------------------------------------------------------
    ...
    // Mortar: the linear-probe body stiffness (ν₀) is φ-invariant — only the gap
    // band rotates. Cache the body V once so the per-angle linear factor restamps
    // only the band (avoids re-running the full body tangent assembly every probe
    // angle — the dominant cost of the linear probes under mortar).
    let _mortarLinBodyV = null;

    function _ensureLinearFactorAt(phi) {
      if (_phiLinearCached === phi) return;
      if (_linearNuRCached === null) _buildLinearRhsCaches();
      // Body V (ν₀) is φ-invariant → assemble once, reuse across all probe angles.
      if (_mortarLinBodyV === null) {
        const t0 = assembleCombinedTriplets_phi(0, _linearNuRCached, _linearNuSCached);
        _mortarLinBodyV = t0.V.slice(0, nBodyTripPat);
      }
      // Restamp only the band at φ → global triplets (scratch.harmTrip_*).
      const ntLocal = gap.stampInto(phi,
        scratch.gapStampLocal_I, scratch.gapStampLocal_J, scratch.gapStampLocal_V);
      const bandN = remapStampInto(
        scratch.gapStampLocal_I, scratch.gapStampLocal_J, scratch.gapStampLocal_V, ntLocal,
        scratch.harmTrip_I, scratch.harmTrip_J, scratch.harmTrip_V);
      // Combined V = [cached body | band(φ)].
      const V = scratch.V_combined;
      V.set(_mortarLinBodyV, 0);
      for (let t = 0; t < bandN; t++) V[nBodyTripPat + t] = scratch.harmTrip_V[t];
      // Boundary-triggered symbolic: only re-analyze when the band pattern moved.
      if (!_mortarSameBand(_mortarBandCacheLin, scratch.harmTrip_I, scratch.harmTrip_J, bandN)) {
        for (let t = 0; t < bandN; t++) {
          mortarCombPatI[nBodyTripPat + t] = scratch.harmTrip_I[t];
          mortarCombPatJ[nBodyTripPat + t] = scratch.harmTrip_J[t];
        }
        const patN = nBodyTripPat + bandN;
        solverLin.setPattern(nGlobal,
          (patN === combinedTripCount) ? mortarCombPatI : mortarCombPatI.subarray(0, patN),
          (patN === combinedTripCount) ? mortarCombPatJ : mortarCombPatJ.subarray(0, patN));
        solverLin.analyze();
        _mortarBandCacheLin = _mortarStoreBand(scratch.harmTrip_I, scratch.harmTrip_J, bandN);
      }
      const patN = nBodyTripPat + bandN;
      solverLin.setValues((patN === combinedTripCount) ? V : V.subarray(0, patN));
      solverLin.factorize();
      _phiLinearCached = phi;
    }

    function _ensureLinearSchurFactorAt(phi) {
      // Mortar: no harmonic block to condense — use the plain band-aware factor
      // (solverLin includes the gap band). _buildLinearRhsCaches also sets
      // _linearNuRCached, which _ensureLinearFactorAt needs.
      if (_linearNuRCached === null) _buildLinearRhsCaches();
      _ensureLinearFactorAt(phi);
    }
    ...
    // ---- buildFieldBundle(combinedA, thetaR) → _solveResult (field viz bundle) ----
    //
    // Post-process a CONVERGED combined-A (free DOF) into the render bundle: full
    // nodal A per body (Anode), element B arrays (Belem: mag/Bx/By), gap nodal A +
    // harmonic projections, the Maxwell torque, and circuit flux linkages. NO
    // solve / NO factorize — pure constraint recovery + ∇×A + projection. The
    // monolithic coupled Newton returns only combinedA per slice; this lets the
    // time-stepping loop reuse the exact viz path stack.solve produces WITHOUT a second
    // field solve. _solveResult is the slice's shared, scratch-aliased object.
    function buildFieldBundle(combinedA, thetaR) {
      ...
      fluxLinkagesFromFullAInto(fullAR, fullAS, _solveResultFluxLinkages, thetaR);
    ...
    function solve(thetaR, currents) {
      ...
      buildFieldBundle(r.A, thetaR);
      // Newton iterations this solve took — the cost signal for adaptive-dt
      // step control (warm-start near the solution ⇒ iters→1). Surfaced up
      // through stack.solve for the time-stepping loop.
      _solveResult.iters = r.iters;
      return _solveResult;
    }
    ...
    // --------------------------------------------------------------------------
    //  Linear-material probe helpers (magnet-only RHS, unit-current RHS, flux-
    //  linkage recovery) shared by coggingTorque and backEmfDensity. All use the
    //  linear material (no Brauer); the warm-start cache is not consulted.
    // --------------------------------------------------------------------------
    ...
    // mapAngle (4th param) decouples the BRUSH-MAP angle from the rotor/solve
    // angle (thetaR) — the brush weight is the only thetaR-dependence here, so
    // default mapAngle = thetaR is byte-identical for existing callers. The
    // motional-derivative probe freezes mapAngle at the center while solving the
    // field at thetaR±h (see fluxLinkagesFromFullAInto for the rationale).
    function assembleUnitCurrentRhs(body, j, thetaR, mapAngle) {
      const NnBody = body.nodes.length / 2;
      const f = new Float64Array(NnBody);
      const Ne = body.elems.length / 4;
      const mech = !!(brushCtx && brushCtx.any && brushCtx.isMechanical[j]);
      const axis = mech ? brushCtx.brushAxis[j] : 0;
      const pp   = mech ? brushCtx.ppArr[j]    : 0;
      const dC   = mech ? brushCtx.deltaC[j]   : 0;
      const th   = (mapAngle != null) ? mapAngle : (thetaR || 0);
      for (let e = 0; e < Ne; e++) {
        const sid = body.srcId[e];
        if (sid !== j) continue;
        ...
        let turns = body.turns[e];
        if (mech) {
          // Brush/commutator spatial remap at probe angle thetaR.
          let xc = 0, yc = 0;
          for (let i = 0; i < nLocal.length; i++) {
            xc += body.nodes[2*nLocal[i]];
            yc += body.nodes[2*nLocal[i]+1];
          ...
          xc /= nLocal.length; yc /= nLocal.length;
          const alpha = Math.atan2(yc, xc);
          turns = Math.abs(turns) * brushProfile(alpha + th - axis, pp, dC);
        }
    ...
    function fluxLinkagesFromCombined(combinedA, thetaR, mapAngle) {
      bodyFullNodalAFromCombinedInto(combinedA);
      const out = new Float64Array(nCircuits);
      return fluxLinkagesFromFullAInto(scratch.fullA_rotor, scratch.fullA_stator, out, thetaR, mapAngle);
    }

    // Into variant: writes into caller's out (length nCircuits). No alloc.
    function fluxLinkagesFromCombinedInto(combinedA, out, thetaR, mapAngle) {
      bodyFullNodalAFromCombinedInto(combinedA);
      return fluxLinkagesFromFullAInto(scratch.fullA_rotor, scratch.fullA_stator, out, thetaR, mapAngle);
    }

    // linearFluxLinkages(thetaR, currents) → Float64Array(nCircuits). One
    // saturation-disabled field solve at the given currents, returning the
    // circuit flux linkages. MotorRun calls this at setup to size the
    // integrator's per-state tolerances: the linear material keeps the
    // cold-start solve cheap and robust, where the saturating product solve can
    // stall at a high cold-start current.
    function linearFluxLinkages(thetaR, currents) {
      _ensureLinearSchurFactorAt(thetaR);
      const rhs = _buildLinearRhsGlobal(
        assembleInteriorMagnetLoadAndJz(rotorMesh,  currents),
        assembleInteriorMagnetLoadAndJz(statorMesh, currents)
      );
      const A_out = schurScratch.A_magnet;
      linearSchurSolve(rhs, A_out);
      return fluxLinkagesFromCombined(A_out, thetaR);
    }

    // ----------------------------------------------------------------------
    //  backEmfDensity(thetaR, currents, omega, opts2)
    //    → { rotor: [{ e, alpha, turnsEff, dAdth, x, y }...],
    //        stator: [...], total }
    //
    //  READ-ONLY pedagogical accessor (NOT in the physics path — it does its
    //  OWN linear solves; solve() is untouched).
    //  Returns the per-conductor MOTIONAL back-EMF density
    //      e_elem = omega · turns_eff(e, θ) · (∂A_e/∂θ)
    //  element-wise, BEFORE summation, for visualization. ∂A_e/∂θ is the
    //  motional derivative of the element-averaged vector potential: the
    //  field/geometry rotates (solveAngle = θ±h) while the brush wiring is
    //  FROZEN at the center θ — exactly the decomposition the circuit-level
    //  dLdth·i + dλpm/dθ back-EMF sums. Σ e_elem over a circuit equals the
    //  circuit-level back-EMF term (both carry the gap Parseval measure π).
    //
    //  This is the spatial visualization of where conductors cut flux: a +1/−1
    //  brush sign times the local speed-EMF ω·∂A/∂θ.
    //
    //  Implementation: solve the full linear field (magnet + the supplied
    //  circuit currents) at θ−h and θ+h with the brush map frozen at θ, recover
    //  per-element area-averaged A, central-difference, and weight by the frozen
    //  turns_eff(e,θ). One pair of dedicated solves; nothing cached or mutated.
    // ----------------------------------------------------------------------
    function _fullLinearSolveInto(solveAngle, mapAngle, currents, A_out) {
      // Combined RHS = magnet load + Σ_j currents[j]·(unit-current RHS for j),
      // the per-circuit RHS rebuilt with the brush map frozen at mapAngle for
      // mechanically-commutated circuits (matches assembleUnitCurrentRhs).
      _ensureLinearSchurFactorAt(solveAngle);
      const fR = assembleMagnetRhs(rotorMesh);   // mesh-node magnet load (rotor)
      const fS = assembleMagnetRhs(statorMesh);  // (stator)
      const m = nCircuits;
      for (let j = 0; j < m; j++) {
        const ij = currents && currents[j] ? currents[j] : 0;
        if (ij === 0) continue;
        const jR = assembleUnitCurrentRhs(rotorMesh,  j, mapAngle);
        const jS = assembleUnitCurrentRhs(statorMesh, j, mapAngle);
        for (let n = 0; n < jR.length; n++) fR[n] += ij * jR[n];
        for (let n = 0; n < jS.length; n++) fS[n] += ij * jS[n];
      }
      const rhs = _buildLinearRhsGlobal(fR, fS);
      linearSchurSolve(rhs, A_out);
    }

    // Area-averaged element A for a conductor element (matches the λ bridge).
    function _elemAavg(body, fullAbody, e) {
      const elems = body.elems;
      const n0 = elems[4*e], n1 = elems[4*e+1], n2 = elems[4*e+2], n3 = elems[4*e+3];
      return (n3 === -1)
        ? (fullAbody[n0] + fullAbody[n1] + fullAbody[n2]) / 3
        : (fullAbody[n0] + fullAbody[n1] + fullAbody[n2] + fullAbody[n3]) / 4;
    }

    function backEmfDensity(thetaR, currents, omega, opts2) {
      opts2 = opts2 || {};
      const om = (omega != null) ? omega : 1;
      // Same measure as LIB.MotorCircuit.backEmf. The back-EMF is a CIRCUIT
      // VOLTAGE e = dλ/dt, and λ (the conductor loop integral ∮A·dl) is physical
      // webers with NO gap Parseval π — so the per-conductor back-EMF density is
      // measure-1 too, and Σ e_elem matches the circuit-level ω·(dLdth·i + dλpm/dθ)
      // back-EMF. (The Parseval π lives only in the gap-energy TORQUE, not in any
      // λ-derived voltage; pinned independently by the cage synchronous speed.)
      const GAP_MEASURE = 1;
      let h;
      ...
      // Two dedicated full solves: field at θ±h, wiring frozen at θ.
      const A_minus = new Float64Array(nGlobal);
      const A_plus  = new Float64Array(nGlobal);
      _fullLinearSolveInto(thetaR - h, thetaR, currents, A_minus);
      _fullLinearSolveInto(thetaR + h, thetaR, currents, A_plus);

      function bodyDensity(body, conductorIndex) {
        const out = [];
        const start = conductorIndex.start;
        const elemIdx = conductorIndex.elemIdx;
        const turnsArr = conductorIndex.turns;
        const alphaArr = conductorIndex.alpha;
        const nodes = body.nodes, elems = body.elems;
        // Recover per-body full nodal A for each probe.
        bodyFullNodalAFromCombinedInto(A_minus);
        const fullA_minus_R = scratch.fullA_rotor, fullA_minus_S = scratch.fullA_stator;
        const isRotor = (body === rotorMesh);
        const fullA_minus = isRotor ? fullA_minus_R.slice() : fullA_minus_S.slice();
        bodyFullNodalAFromCombinedInto(A_plus);
        const fullA_plus = isRotor ? scratch.fullA_rotor : scratch.fullA_stator;
        for (let k = 0; k < nCircuits; k++) {
          const mech = brushOn && brushCtx.isMechanical[k];
          const axis = mech ? brushCtx.brushAxis[k] : 0;
          const pp   = mech ? brushCtx.ppArr[k]    : 0;
          ...
          for (let p = lo; p < hi; p++) {
            const e = elemIdx[p];
            const Am = _elemAavg(body, fullA_minus, e);
            const Ap = _elemAavg(body, fullA_plus,  e);
            const dAdth = (Ap - Am) * inv2h;
            const turnsEff = mech
              ? Math.abs(turnsArr[p]) * brushProfile(alphaArr[p] + thetaR - axis, pp, dC)
              : turnsArr[p];
            // centroid position (rotor frame) for placement
            const n0 = elems[4*e], n1 = elems[4*e+1], n2 = elems[4*e+2], n3 = elems[4*e+3];
            let xc, yc;
            if (n3 === -1) {
              xc = (nodes[2*n0]+nodes[2*n1]+nodes[2*n2])/3;
            ...
            out.push({
              circuit: k,
              e: GAP_MEASURE * om * turnsEff * dAdth * ell,
              alpha: alphaArr[p],
              turnsEff: turnsEff,
              dAdth: dAdth * ell,
              x: xc, y: yc,
      ...
      return {
        rotor:  bodyDensity(rotorMesh,  conductorIndexRotor),
        stator: bodyDensity(statorMesh, conductorIndexStator),
        omega:  om,
      };
    }
    ...
    const __internals = {
      ...
      fluxState: function () { return lastFlux; },   // axial-flux DOF: {Psi, Rfield, d, cA}
      ...
      derivedSlots: slots,
      derivedPoles: poles,
      ell: ell,                 // axial length — circuit border ∂λ_k/∂A = ell·uRHS_k
    ...
    // ======================================================================
    //  Monolithic field-circuit-motion Newton support.
    //
    //  The stack-level coupled Newton drives ONE field-Newton iteration per
    //  outer iteration at an externally-supplied (combinedA, currents, θ), then
    //  condenses the field block via the borders exposed here. Everything reuses
    //  the SAME assemble path (prepareSolve + buildInto) as the standalone solve.
    // ======================================================================

    // ∂T/∂A scratch (gap-ring gradients + full-body scatter buffers).
    const _dTgapR = new Float64Array(Ngr);
    const _dTgapS = new Float64Array(Ngs);
    const _dTfullR = new Float64Array(NnR_full);
    const _dTfullS = new Float64Array(NnS_full);
    // θ-border one-sided FD scratch. ∂R_field/∂θ = ∂(K_h·A)/∂θ − ∂f/∂θ (the
    // K(ν)·A term is θ-independent at fixed A and cancels), so we store only the
    // θ-varying pieces at θ−h: the gap-stamp matvec K_h(θ−h)·A and the RHS f(θ−h).
    const _KAhThetaM = new Float64Array(nGlobal);
    const _fThetaM   = new Float64Array(nGlobal);
    const _lamThetaM = new Float64Array(nCircuits);
    let _lastFNorm = 0;   // ‖f‖∞ from the last coupledAssembleNoFactor (field-residual scale)

    // Transpose of recoverFullFromBigA_Into: full-body gradient gFull → big
    // free-DOF (accumulated into outBig at baseOffset). Forward recovers
    // non-slaves from Abig then slaves from masters; the adjoint reverses —
    // push each slave's gradient onto its (non-slave) masters first, then push
    // non-slave gradients to the big vector. gFull is mutated (master accum).
    function recoverFullFromBigA_Adjoint(gFull, baseOffset, len, slaveTable, renumBody, NnBody, outBig) {
      const isSlave = slaveTable.isSlave;
      const masterL = slaveTable.masterL, masterR = slaveTable.masterR;
      const weightL = slaveTable.weightL, weightR = slaveTable.weightR;
      for (let i = 0; i < NnBody; i++) {
        if (!isSlave[i]) continue;
        gFull[masterL[i]] += weightL[i] * gFull[i];
        gFull[masterR[i]] += weightR[i] * gFull[i];
      }
      for (let i = 0; i < NnBody; i++) {
        if (isSlave[i]) continue;
        const fi = renumBody[i];
        if (fi >= 0) outBig[baseOffset + fi] += gFull[i];
      }
    }

    // Assemble + factor the field tangent ∂R_field/∂A at (combinedA, currents,
    // θ). Leaves scratch.residual = R_field, solverSat factored, and
    // scratch.fullA_*/gapNodal_* populated (for the torque borders).
    // Assemble the field tangent + residual at (combinedA, currents, θ), and the
    // torque, but DO NOT factorize. Leaves scratch.{V_combined, residual, KAh, rhs,
    // fullA_*, gapNodal_*} ready, _lastFNorm = ‖f‖∞, residualOut = R_field, and
    // returns T(θ). The Newton can check convergence on these BEFORE paying the
    // factorize (deferred to coupledFactorize) — a converged iter skips it.
    function coupledAssembleNoFactor(combinedA, currents, thetaR, residualOut) {
      for (let i = 0; i < nGlobal; i++) scratch.A_iter[i] = combinedA[i];
      prepareSolve(thetaR, currents);
      buildInto();
      // ‖f‖∞ — the field RHS magnitude; scale for the relative field-residual test.
      let fn = 0; for (let i = 0; i < nGlobal; i++) { const a = Math.abs(scratch.rhs[i]); if (a > fn) fn = a; }
      _lastFNorm = fn;
      if (residualOut) for (let i = 0; i < nGlobal; i++) residualOut[i] = scratch.residual[i];
      // buildInto already recovered scratch.fullA_* — reuse for the gap-ring extraction.
      gapNodalAInto(rotorMesh,  scratch.fullA_rotor,  scratch.gapNodal_R);
      gapNodalAInto(statorMesh, scratch.fullA_stator, scratch.gapNodal_S);
      return gap.torque(scratch.gapNodal_R, scratch.gapNodal_S, thetaR);
    }

    // Numeric factorize of the tangent assembled by coupledAssembleNoFactor. Must
    // follow it (uses scratch.V_combined) with no intervening gap re-stamp.
    function coupledFactorize() {
      solverSat.setValues(scratch.V_combined);
      solverSat.factorize();
    }

    // θ-border one-sided FD, split into a pre-eval (before the factorize) and a
    // post-diff (after). ∂R_field/∂θ touches NO field assembly: since
    //   R_field = K(ν)·A − f(i,θ) + K_h(θ)·A   and K(ν)·A is θ-independent at fixed
    // A (ν depends on B=∇A, not θ), the FD reduces to
    //   ∂R_field/∂θ = (ΔK_h·A − Δf)/h     [the K(ν)·A matvec cancels EXACTLY].
    // So we only re-evaluate the θ-varying pieces — the gap stamp K_h(θ−h)·A and
    // the RHS f(θ−h) — both cheap, no recover/ν/K(ν)·A over the 5664 elements.
    //
    // coupledThetaPreEval: prepareSolve at θ−h gives f(θ−h) and the gap stamp;
    // store f(θ−h) and the small matvec K_h(θ−h)·A. MUST precede
    // coupledAssembleNoFactor (prepareSolve re-stamps the gap; the assemble then
    // re-stamps at θ and coupledFactorize factorizes).
    function coupledThetaPreEval(combinedA, currents, thetaR) {
      const h = Math.PI / (poles * 1e5);
      for (let i = 0; i < nGlobal; i++) scratch.A_iter[i] = combinedA[i];
      prepareSolve(thetaR - h, currents);
      for (let i = 0; i < nGlobal; i++) _fThetaM[i] = scratch.rhs[i];
      for (let i = 0; i < nGlobal; i++) _KAhThetaM[i] = 0;
      const hI = scratch.harmTrip_I, hJ = scratch.harmTrip_J, hV = scratch.harmTrip_V, harmN = scratch.harmTrip_n;
      for (let t = 0; t < harmN; t++) _KAhThetaM[hI[t]] += hV[t] * scratch.A_iter[hJ[t]];
    }

    // coupledThetaPostDiff: form ∂R_field/∂θ from the θ-varying terms (center
    // K_h(θ)·A and f(θ) are in scratch.KAh / scratch.rhs from
    // coupledAssembleNoFactor's buildInto), and ∂λ/∂θ from the center fullA
    // (reused — only the brush map carries θ) and ∂T/∂θ from the center gapNodal
    // (only gap.torque's φ carries θ). No refactor, no field re-assemble. Call
    // AFTER coupledAssembleNoFactor at θ.
    function coupledThetaPostDiff(thetaR, lamCenter, Tcenter, dRdthOut, dLamdthOut) {
      const h = Math.PI / (poles * 1e5);
      const invh = 1 / h;
      const KAhC = scratch.KAh, fC = scratch.rhs;
      for (let i = 0; i < nGlobal; i++) dRdthOut[i] = ((KAhC[i] - _KAhThetaM[i]) - (fC[i] - _fThetaM[i])) * invh;
      fluxLinkagesFromFullAInto(scratch.fullA_rotor, scratch.fullA_stator, _lamThetaM, thetaR - h);
      for (let k = 0; k < nCircuits; k++) dLamdthOut[k] = (lamCenter[k] - _lamThetaM[k]) * invh;
      const Tm = gap.torque(scratch.gapNodal_R, scratch.gapNodal_S, thetaR - h);
      return (Tcenter - Tm) * invh;
    }

    // Circuit flux linkages λ_k(combinedA, θ) into out (nCircuits). The circuit
    // border ∂R_ckt/∂A = ag0·∂λ_k/∂A; ∂λ_k/∂A is FD-related to the unit-current
    // RHS (the flux-pickup factor is measured against coupledUnitRhsInto).
    function coupledFluxInto(combinedA, thetaR, out) {
      return fluxLinkagesFromCombinedInto(combinedA, out, thetaR);
    }

    // R_field at (combinedA, currents, θ) WITHOUT refactoring — for the θ-border
    // finite difference (writes nGlobal residual into out).
    function coupledFieldResidualInto(combinedA, currents, thetaR, out) {
      for (let i = 0; i < nGlobal; i++) scratch.A_iter[i] = combinedA[i];
      prepareSolve(thetaR, currents);
      buildResidualOnlyInto();   // residual only — the θ-FD never uses the tangent
      for (let i = 0; i < nGlobal; i++) out[i] = scratch.residual[i];
    }

    // Solve the field tangent factored by coupledFactorize against rhs.
    function coupledSolveAgainst(rhs, out) { solverSat.solveInto(rhs, out); }

    // Multi-RHS variant: B/X are column-major (nGlobal × ncols). One Eigen
    // solve(matrix) — L streamed once for all columns. Used by the Schur
    // condensation to batch its nf+2 field back-solves per Newton iter.
    function coupledSolveMultiAgainst(B, ncols, X) { solverSat.solveMultiInto(B, ncols, X); }

    // Constrained unit-current RHS for circuit k in combined free-DOF layout.
    //   ∂R_field/∂i_k = −out.   ∂λ_k/∂A = ell · out   (reciprocity λ_k = ell·u_kᵀA;
    //   the factor is exactly ell, FD-verified to 3e-15 spread).
    function coupledUnitRhsInto(k, thetaR, out) {
      const fR = assembleUnitCurrentRhs(rotorMesh,  k, thetaR);
      const fS = assembleUnitCurrentRhs(statorMesh, k, thetaR);
      applyConstraintsToFInto(fR, slaveTableR, renumR, NnR_full, nRotorFree, scratch.fR_free);
      applyConstraintsToFInto(fS, slaveTableS, renumS, NnS_full, nStatorFree, scratch.fS_free);
      for (let i = 0; i < nRotorFree; i++) out[i] = scratch.fR_free[i];
      for (let i = 0; i < nStatorFree; i++) out[nRotorFree + i] = scratch.fS_free[i];
      for (let i = nRotorFree + nStatorFree; i < nGlobal; i++) out[i] = 0;
    }

    // ∂T/∂A in combined free-DOF at the state left by coupledAssembleNoFactor. Adjoint
    // of: combinedA → fullA (recover) → gapNodal (gapLoop) → gap.torque.
    function coupledDTdAInto(thetaR, out) {
      gap.torqueGrad(scratch.gapNodal_R, scratch.gapNodal_S, thetaR, _dTgapR, _dTgapS);
      for (let i = 0; i < NnR_full; i++) _dTfullR[i] = 0;
      for (let i = 0; i < NnS_full; i++) _dTfullS[i] = 0;
      const loopR = rotorMesh.gapLoop, loopS = statorMesh.gapLoop;
      for (let i = 0; i < loopR.length; i++) _dTfullR[loopR[i]] += _dTgapR[i];
      for (let i = 0; i < loopS.length; i++) _dTfullS[loopS[i]] += _dTgapS[i];
      for (let i = 0; i < nGlobal; i++) out[i] = 0;
      recoverFullFromBigA_Adjoint(_dTfullR, 0,          nRotorFree,  slaveTableR, renumR, NnR_full, out);
      recoverFullFromBigA_Adjoint(_dTfullS, nRotorFree, nStatorFree, slaveTableS, renumS, NnS_full, out);
    }

    // Axial-flux DOF column for the coupled Schur: fill out (nGlobal) with the
    // flux-coupling vector c_s = ∫ν∇N·∇χ (rotor+stator+gap) at the current iterate
    // (uses scratch.nu_* from the preceding coupledAssembleNoFactor), return the
    // self term d_s. The stack condenses c_s like the θ-column and forms the loop
    // (Φ) reduced rows from c_sᵀA_s + d_sΨ_s. Returns 0 / leaves out untouched when
    // this slice has no flux DOF.
    function coupledFluxRhsInto(thetaR, out) {
      if (nCut === 0) { for (let i = 0; i < nGlobal; i++) out[i] = 0; return 0; }
      const d = assembleFluxCoupling(thetaR);
      for (let i = 0; i < nGlobal; i++) out[i] = _fluxC[i];
      return d;
    }

    // Build linear-path RHS caches at create-time. All RHS used by
    // coggingTorque/backEmfDensity are phi-invariant.
    ...
    return {
      ...
      // Monolithic-Newton borders (see block above).
      coupledFieldResidualInto: coupledFieldResidualInto,
      coupledSolveAgainst: coupledSolveAgainst,
      coupledSolveMultiAgainst: coupledSolveMultiAgainst,
      coupledUnitRhsInto: coupledUnitRhsInto,
      coupledFluxRhsInto: coupledFluxRhsInto,
      coupledDTdAInto: coupledDTdAInto,
      coupledFluxInto: coupledFluxInto,
      coupledThetaPreEval: coupledThetaPreEval,
      coupledThetaPostDiff: coupledThetaPostDiff,
      coupledAssembleNoFactor: coupledAssembleNoFactor,
      coupledFactorize: coupledFactorize,
      coupledRhsNorm: function () { return _lastFNorm; },
      buildFieldBundle: buildFieldBundle,
