(function () {
  ...
  // --------------------------------------------------------------------------
  //  assembleBodyFluxCouplingInto(body, nu, cFull) → d_body
  //
  ...
  //  annulus harmonic 1-form (net radial flux = ℓ·Ψ). cFull is FULL nodal indexing
  //  (caller zeroes + applies constraints, like the magnet RHS). nu is per-element
  //  reluctivity (the same array the tangent uses, so saturation enters c too).
  //  Q4: 2×2 Gauss; tri: 1-point centroid — matching the body stiffness quadrature.
  // --------------------------------------------------------------------------
  const _GP = 1 / Math.sqrt(3);
  const _gpST = [[-_GP, -_GP], [_GP, -_GP], [_GP, _GP], [-_GP, _GP]];
  function assembleBodyFluxCouplingInto(body, nu, cFull) {
    const Ne = body.elems.length / 4;
    ...
    for (let e = 0; e < Ne; e++) {
      ...
      if (n3 === -1) {
        ...
        const w = nue * g.area;
        cFull[n0] += w * (g.dNdx[0] * gx + g.dNdy[0] * gy);
        cFull[n1] += w * (g.dNdx[1] * gx + g.dNdy[1] * gy);
        cFull[n2] += w * (g.dNdx[2] * gx + g.dNdy[2] * gy);
        dAcc += w * (gx * gx + gy * gy);
      } else {
        const xN = [body.nodes[2*n0], body.nodes[2*n1], body.nodes[2*n2], body.nodes[2*n3]];
        const yN = [body.nodes[2*n0+1], body.nodes[2*n1+1], body.nodes[2*n2+1], body.nodes[2*n3+1]];
        const nn = [n0, n1, n2, n3];
        for (let q = 0; q < 4; q++) {
          ...
          const gx = -yc * INV2PI / r2, gy = xc * INV2PI / r2;
          const w = nue * jac.detJ;   // 2×2 Gauss weight = 1
          for (let i = 0; i < 4; i++) cFull[nn[i]] += w * (jac.dNdx[i] * gx + jac.dNdy[i] * gy);
          dAcc += w * (gx * gx + gy * gy);
        }
      }
    }
  ...
  function create(section, opts) {
    ...
    function assembleFluxCoupling(phi) {
      ...
      let d = assembleBodyFluxCouplingInto(rotorMesh, scratch.nu_rotor, _fluxCFullR);
      d += assembleBodyFluxCouplingInto(statorMesh, scratch.nu_stator, _fluxCFullS);
