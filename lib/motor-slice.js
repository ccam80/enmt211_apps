(function () {
  ...
  function create(section, opts) {
    ...
    // θ-border FD scratch.
    const _rThetaP = new Float64Array(nGlobal);
    const _rThetaM = new Float64Array(nGlobal);
    const _lamThetaP = new Float64Array(nCircuits);
    ...
    // θ-borders by central difference at frozen (A, currents): the field residual
    // ∂R_field/∂θ (→ dRdthOut, length nGlobal), the flux ∂λ_k/∂θ (→ dLamdthOut,
    // length nCircuits), and dT/dθ (returned). MUST be called BEFORE the final
    // coupledAssemble of a Newton iteration: coupledFieldResidualInto re-preps the
    // gap stamp at θ±h, so the factor used by coupledSolveAgainst must be
    // (re)established at θ afterwards. h is the same machine-aware step as extractCoeffs.
    function coupledThetaDerivs(combinedA, currents, thetaR, dRdthOut, dLamdthOut) {
      const h = Math.PI / (poles * 1e5);
      const inv2h = 1 / (2 * h);
      coupledFieldResidualInto(combinedA, currents, thetaR + h, _rThetaP);
      coupledFieldResidualInto(combinedA, currents, thetaR - h, _rThetaM);
      for (let i = 0; i < nGlobal; i++) dRdthOut[i] = (_rThetaP[i] - _rThetaM[i]) * inv2h;
      fluxLinkagesFromCombinedInto(combinedA, _lamThetaP, thetaR + h);
      fluxLinkagesFromCombinedInto(combinedA, _lamThetaM, thetaR - h);
      for (let k = 0; k < nCircuits; k++) dLamdthOut[k] = (_lamThetaP[k] - _lamThetaM[k]) * inv2h;
      // dT/dθ at frozen A: gapNodal is θ-independent (from fullA); only gap.torque's
      // φ argument carries θ. Recompute gapNodal from combinedA, then central-diff φ.
      bodyFullNodalAFromCombinedInto(combinedA);
      gapNodalAInto(rotorMesh,  scratch.fullA_rotor,  scratch.gapNodal_R);
      gapNodalAInto(statorMesh, scratch.fullA_stator, scratch.gapNodal_S);
      const Tp = gap.torque(scratch.gapNodal_R, scratch.gapNodal_S, thetaR + h);
    ...
    return {
      ...
      coupledThetaDerivs: coupledThetaDerivs,
