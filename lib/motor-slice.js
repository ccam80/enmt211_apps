(function () {
  ...
  function create(section, opts) {
    ...
    // θ-border one-sided FD scratch (θ−h residual + flux, evaluated before the
    // factorize so it survives into the post-diff).
    const _rThetaM = new Float64Array(nGlobal);
    ...
    function coupledAssemble(combinedA, currents, thetaR, residualOut) {
      ...
      solverSat.setValues(scratch.V_combined);
      solverSat.factorize();
      bodyFullNodalAFromCombinedInto(scratch.A_iter);
      gapNodalAInto(rotorMesh,  scratch.fullA_rotor,  scratch.gapNodal_R);
      gapNodalAInto(statorMesh, scratch.fullA_stator, scratch.gapNodal_S);
      return gap.torque(scratch.gapNodal_R, scratch.gapNodal_S, thetaR);
    }

    // θ-border one-sided FD, split into a pre-eval and a post-diff so the single
    // off-center evaluation happens BEFORE coupledAssemble's factorize (which
    // re-establishes the solver at θ) — coupledFieldResidualInto re-preps the gap
    // at θ−h, which invalidates the Cholesky factor, so it MUST precede the
    // factorize, exactly as the old central-diff did. The diff then reuses the
    // centers coupledAssemble produced. One residual+flux eval instead of two.
    //
    // coupledThetaPreEval: R(θ−h) → _rThetaM, λ(θ−h) → _lamThetaM. Call BEFORE
    // coupledAssemble (the next coupledAssemble re-stamps + factorizes at θ).
    function coupledThetaPreEval(combinedA, currents, thetaR) {
      const h = Math.PI / (poles * 1e5);
      coupledFieldResidualInto(combinedA, currents, thetaR - h, _rThetaM);
      fluxLinkagesFromCombinedInto(combinedA, _lamThetaM, thetaR - h);
    }

    // coupledThetaPostDiff: backward one-sided diff using the cached centers
    // (Rcenter = coupledAssemble's residualOut, lamCenter = coupledFluxInto,
    // Tcenter = coupledAssemble's return). gapNodal_* still hold the θ (center-A)
    // ring field — only gap.torque's φ carries θ, so T(θ−h) needs no re-stamp and
    // no refactor. Call AFTER coupledAssemble at the same θ. h-truncation O(h) ≈
    // 2e-5 relative is negligible for a Jacobian column.
    function coupledThetaPostDiff(thetaR, Rcenter, lamCenter, Tcenter, dRdthOut, dLamdthOut) {
      const h = Math.PI / (poles * 1e5);
      const invh = 1 / h;
      for (let i = 0; i < nGlobal; i++) dRdthOut[i] = (Rcenter[i] - _rThetaM[i]) * invh;
