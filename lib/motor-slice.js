(function () {
  ...
  function create(section, opts) {
    ...
    // Sparse basis: opts.kList (explicit harmonic orders) overrides the dense K.
    // The gap-node floor is set by the HIGHEST harmonic carried (Nyquist), not
    // the harmonic COUNT — a sparse list with a high slot-band order still needs
    // the ring resolved to 4·maxK.
    const _maxK = (opts.kList != null && opts.kList.length)
      ? opts.kList.reduce((a, b) => (b > a ? b : a), 1)
      : K;
    const requiredGapNodes = 4 * _maxK;
    // Air-gap coupling method (opts.gapMethod): "harmonic" (default, the
    // Dirichlet-to-Neumann harmonic gap) or "mortar" (real-space FE air-gap
    // band — sparse ring↔ring coupling, no harmonic DOFs, no Fourier
    // truncation). The mesh is still sized by K (4·maxK gap-node floor) so a
    // mortar-vs-harmonic A/B runs on byte-identical bodies — only the coupling
    // differs. mortar re-analyzes per solve(θ) (band pattern rotates).
    const gapMethod = (opts.gapMethod === "mortar") ? "mortar" : "harmonic";
    // Schur-condense the saturated Newton inner solve (opts.schur). OFF by
    // default — the combined solverSat path is byte-for-byte unchanged. When on,
    // each Newton iteration factors the body tangent J_b alone (sparse, no
    // gapN×perBody dense fill) and condenses the small perBody harmonic block
    // densely. Exact reduction of the SAME tangent system: no truncation.
    // The harmonic-Schur path condenses the dense harmonic block; it does not
    // apply to mortar (no harmonic block — mortar's substructure is the separate
    // ring-interface condensation). Gate it off for mortar.
    const useSchur = (opts.schur === true) && (gapMethod !== "mortar");
    ...
    const gap = (gapMethod === "mortar")
      ? LIB.AirgapMortar.build(rotorGap, statorGap, { ell })
      : LIB.AirgapHarmonic.build(rotorGap, statorGap,
          (opts.kList != null) ? { kList: opts.kList, ell } : { K, ell });
    ...
    const mortarBodyPatI = (gapMethod === "mortar") ? patternTrip.I.slice(0, nBodyTripPat) : null;
    const mortarBodyPatJ = (gapMethod === "mortar") ? patternTrip.J.slice(0, nBodyTripPat) : null;
    const mortarCombPatI = (gapMethod === "mortar") ? new Int32Array(combinedTripCount) : null;
    const mortarCombPatJ = (gapMethod === "mortar") ? new Int32Array(combinedTripCount) : null;
    if (gapMethod === "mortar") {
    ...
    function linearSchurSolve(fBody, outA) {
      ...
      _schurSolveCount++;
      // Pad fBody (length nBody) into the augmented RHS (length nKbAug) with
      // the multiplier RHS = 0. The Lagrange row g=p makes the augmented solve
      // return the Moore–Penrose pseudo-inverse z = K_b⁺·f (the unique
      // p-orthogonal solution of K_b z = f_⊥); ΣA_rotor=0 is an INTERNAL
      // device of the pseudo-inverse, NOT the final gauge — the full-bordered-
      // solve gauge is restored below by the γ·p null-mode correction.
      const fAug = schurScratch.fBodyAug;
      for (let i = 0; i < nBody; i++) fAug[i] = fBody[i];
      fAug[nBody] = 0;
      // z = K_b⁺·fBody (= z'[0:nBody]; z'[nBody] is the multiplier — discard).
      // solveInto avoids the per-call Float64Array(nKbAug) alloc solve() does.
      ...
      // A_harm0 = S_pd⁻¹·(Bᵀz). This is the ΣA_rotor=0-gauge harmonic solve
      // (what the pre-Bug-#3 path returned). The full-bordered-solve gauge
      // adds the null-mode amplitude γ on top.
      denseCholeskySolve(schurScratch.S, g, nHarm, A_harm);

      // -------- Bug #3 gauge correction --------
      // The full bordered solve resolves K_b's rotor-uniform null mode through
      // the harmonic k=0 coupling, NOT via ΣA_rotor=0. Recover the null-mode
      // amplitude γ that closes the bordered γ-row:
      //   γ = (pᵀf − Btpᵀ·A_harm0) / gammaDenom
      // with pᵀf = Σ over rotor free DOFs of f_body, and gammaDenom = Btpᵀ·yGamma
      // (precomputed in linearSchurPrepare). When gammaDenom is degenerate
      // (no rotor↔k0 coupling, e.g. a pinned/empty rotor), fall back to γ=0
      // (the ΣA_rotor=0 gauge), which is the correct limit.
      let ptf = 0;
      for (let i = 0; i < nRotorFree; i++) ptf += fBody[i];
      const yG = schurScratch.yGamma;
      let btpAh0 = 0;
      for (let j = 0; j < nHarm; j++) btpAh0 += Btp[j] * A_harm[j];
      const gammaDenom = schurScratch.gammaDenom;
      const gamma = (Math.abs(gammaDenom) > 1e-300)
        ? (ptf - btpAh0) / gammaDenom
        : 0;
      // h = A_harm0 + γ·yGamma
      for (let j = 0; j < nHarm; j++) A_harm[j] += gamma * yG[j];

      // Body recovery: A_b = z − W·h + γ·p. p is 1 on rotor free DOFs, 0 on
      // stator, so the +γ·p term shifts only the rotor body block.
      ...
      for (let i = 0; i < nRotorFree; i++) outA[i] += gamma;
    ...
    let _schurProf = null;
    if (useSchur) {
      const nBodyTrip = combinedTripCount - harmTripCount;

      // solverJb pattern = body prefix of the combined pattern (rotor + stator,
      // stator offset already baked in, rank-1 positions emitted) + the SAME
      // Lagrange gauge augmentation as buildKbTriplets (zero-average rotor A).
      const nJb = nBodyTrip + 2 * nRotorFree + 1;
      const jbI = new Int32Array(nJb);
      const jbJ = new Int32Array(nJb);
      const jbVpat = new Float64Array(nJb);
      for (let t = 0; t < nBodyTrip; t++) {
        jbI[t] = patternTrip.I[t];
        jbJ[t] = patternTrip.J[t];
        jbVpat[t] = patternTrip.V[t];
      }
      let pp = nBodyTrip;
      for (let i = 0; i < nRotorFree; i++) { jbI[pp] = i;     jbJ[pp] = nBody; jbVpat[pp] = 1; pp++; }
      for (let i = 0; i < nRotorFree; i++) { jbI[pp] = nBody; jbJ[pp] = i;     jbVpat[pp] = 1; pp++; }
      jbI[pp] = nBody; jbJ[pp] = nBody; jbVpat[pp] = 0; pp++;

      const solverJb = LIB.FeaSolver.create();
      solverJb.setPattern(nKbAug, jbI, jbJ);
      solverJb.setValues(jbVpat);
      solverJb.analyze();

      // Per-iter scratch (allocated once at create-time).
      const jbV    = new Float64Array(nJb);
      const W_nt   = new Float64Array(nBody * nHarm);
      const BtW_nt = new Float64Array(nHarm * nHarm);
      const S_nt   = new Float64Array(nHarm * nHarm);
      const rhsCol = new Float64Array(nKbAug);
      const colSol = new Float64Array(nKbAug);
      const yG_nt  = new Float64Array(nHarm);
      const z_nt   = new Float64Array(nKbAug);
      const g_nt   = new Float64Array(nHarm);
      const Ah_nt  = new Float64Array(nHarm);

      // The Lagrange-aug tail of jbV is constant across iters; set it once.
      for (let i = 0; i < 2 * nRotorFree; i++) jbV[nBodyTrip + i] = 1;
      jbV[nBodyTrip + 2 * nRotorFree] = 0;

      // Optional per-phase profiling (SCHUR_PROF). Accumulates ms per phase so
      // we can see whether the cost is the per-iter W back-solves (inherent),
      // the JS dense factor (fixable via WASM), or the body factorize.
      const PROF = (typeof process !== "undefined" && process.env && process.env.SCHUR_PROF) ? true : false;
      const prof = { factor: 0, W: 0, BtW: 0, dense: 0, solve: 0, calls: 0, refreshes: 0 };
      _schurProf = prof;
      const pnow = PROF ? (() => Number(process.hrtime.bigint()) / 1e6) : (() => 0);

      // Lagged-tangent (modified-Newton) state. The condensation factors —
      // frozen J_b factor (in solverJb), W_nt, the S_nt Cholesky and its γ
      // invariants — are the EXPENSIVE part (forming BᵀJ_b⁻¹B, ~95% of the
      // step). They depend only on the body tangent J_b(ν) and the φ-dependent
      // D. When `refresh` is false the step REUSES them and only re-solves with
      // the current RHS — a chord/modified-Newton step. The root is unchanged
      // (residual=0 is independent of the tangent driving the step); the loop's
      // residual check + line search guard convergence and trigger a refresh
      // when the frozen tangent goes stale (see the call site).
      let schurGammaDenom = 0;

      // newtonSchurStep(neg, outDA, refresh): when refresh, re-factor the body
      // tangent and rebuild W / BtW / S (the costly condensation); always solve
      // J·outDA = neg (neg = −residual) using the current/cached factors.
      // neg layout: [f_body (nBody) ; f_h (nHarm)].
      newtonSchurStep = function (neg, outDA, refresh) {
        let _t = pnow();
        if (refresh) {
          // (1) Body tangent → solverJb. The body prefix of V_combined is the
          // current rotor+stator tangent in pattern order (buildInto wrote it).
          const Vc = scratch.V_combined;
          for (let t = 0; t < nBodyTrip; t++) jbV[t] = Vc[t];
          solverJb.setValues(jbV);
          solverJb.factorize();
          if (PROF) { prof.factor += pnow() - _t; _t = pnow(); }

          // (2) W = J_b⁺·B — one back-sub per harmonic DOF column. RHS is the
          // augmented body vector (multiplier slot 0); only body rows enter W.
          for (let j = 0; j < nHarm; j++) {
            for (let i = 0; i < nKbAug; i++) rhsCol[i] = 0;
            const lo = B_colStart[j], hi = B_colStart[j + 1];
            for (let k = lo; k < hi; k++) {
              const t = B_order[k];
              rhsCol[B.bodyIdx[t]] += B.value[t];
            }
            solverJb.solveInto(rhsCol, colSol);
            for (let i = 0; i < nBody; i++) W_nt[i * nHarm + j] = colSol[i];
          }
          if (PROF) { prof.W += pnow() - _t; _t = pnow(); }

          // (3) BtW = Bᵀ·W.
          for (let i = 0; i < nHarm * nHarm; i++) BtW_nt[i] = 0;
          for (let t = 0; t < B.n; t++) {
            const bi = B.bodyIdx[t], hi = B.harmIdx[t], v = B.value[t];
            const wRowOff = bi * nHarm, outOff = hi * nHarm;
            for (let j = 0; j < nHarm; j++) BtW_nt[outOff + j] += v * W_nt[wRowOff + j];
          }
          if (PROF) { prof.BtW += pnow() - _t; _t = pnow(); }

          // (4) S_pd = BtW − D. The harm-harm block D is the gap stamp's −M⁻¹
          // (fixed per solve), already in scratch.harmTrip_*; subtracting the
          // negative values yields S_pd = BᵀW + M⁻¹ (SPD). Factor in place.
          for (let i = 0; i < nHarm * nHarm; i++) S_nt[i] = BtW_nt[i];
          const harmBase = harmonicBaseGlobal;
          const hI = scratch.harmTrip_I, hJ = scratch.harmTrip_J, hV = scratch.harmTrip_V;
          const hn = scratch.harmTrip_n;
          for (let t = 0; t < hn; t++) {
            const i = hI[t], j = hJ[t];
            if (i >= harmBase && j >= harmBase) {
              S_nt[(i - harmBase) * nHarm + (j - harmBase)] -= hV[t];
            }
          }
          denseCholeskyFactor(S_nt, nHarm);
          // γ-elimination invariants (φ + ν dependent; cached with the factor).
          denseCholeskySolve(S_nt, Btp, nHarm, yG_nt);
          schurGammaDenom = 0;
          for (let i = 0; i < nHarm; i++) schurGammaDenom += Btp[i] * yG_nt[i];
          if (PROF) { prof.dense += pnow() - _t; _t = pnow(); prof.refreshes++; }
        }
        const gammaDenom = schurGammaDenom;

        // (5) Solve. z = J_b⁺·f_body (augmented; multiplier slot RHS = 0).
        for (let i = 0; i < nBody; i++) rhsCol[i] = neg[i];
        rhsCol[nBody] = 0;
        solverJb.solveInto(rhsCol, z_nt);
        // g = Bᵀz − f_h  (S_pd = −S sign convention: see linearSchurSolve).
        for (let j = 0; j < nHarm; j++) g_nt[j] = 0;
        for (let t = 0; t < B.n; t++) g_nt[B.harmIdx[t]] += B.value[t] * z_nt[B.bodyIdx[t]];
        for (let j = 0; j < nHarm; j++) g_nt[j] -= neg[nBody + j];
        denseCholeskySolve(S_nt, g_nt, nHarm, Ah_nt);
        // Bug #3 γ null-mode gauge correction.
        let ptf = 0;
        for (let i = 0; i < nRotorFree; i++) ptf += neg[i];
        let btpAh0 = 0;
        for (let j = 0; j < nHarm; j++) btpAh0 += Btp[j] * Ah_nt[j];
        const gamma = (Math.abs(gammaDenom) > 1e-300) ? (ptf - btpAh0) / gammaDenom : 0;
        for (let j = 0; j < nHarm; j++) Ah_nt[j] += gamma * yG_nt[j];
        // Body recovery: A_b = z − W·h (+ γ on rotor free DOFs).
        for (let i = 0; i < nBody; i++) {
          let dot = 0; const off = i * nHarm;
          for (let j = 0; j < nHarm; j++) dot += W_nt[off + j] * Ah_nt[j];
          outDA[i] = z_nt[i] - dot;
        }
        for (let i = 0; i < nRotorFree; i++) outDA[i] += gamma;
        for (let j = 0; j < nHarm; j++) outDA[nBody + j] = Ah_nt[j];
        if (PROF) { prof.solve += pnow() - _t; prof.calls++; }
      };
    }
    ...
    function solveStaticRotor(thetaR, currents) {
      ...
      // -------- mortar: re-analyze for THIS φ (band pattern rotated) --------
      // The body pattern is fixed (cached); append this φ's band (I,J) and
      // re-run the symbolic factorization once per solve. The Newton iters below
      // then reuse it (setValues + factorize per iter), exactly like harmonic.
      if (gapMethod === "mortar") {
        const hI = scratch.harmTrip_I, hJ = scratch.harmTrip_J;
        // Re-run the symbolic ONLY when the band pattern actually changed
      ...
      // Schur refresh policy. A lagged-tangent (modified-Newton chord) — reuse
      // W/BtW/S across iters and only refresh on backtrack — was implemented
      // and MEASURED: it breaks convergence on stiff saturated machines
      // (induction/sync-rel hit maxIter at 30-47% torque error) because the
      // frozen tangent gives only linear convergence. The per-iter reform of
      // BᵀJ_b⁻¹B is the price of full Newton's quadratic convergence, NOT lazy
      // caching — so we always refresh (exact full-Newton Schur). The flag is
      // retained so a future cross-solve reuse (hot-path warm-start, where ~1
      // iter converges) can be tried without restructuring.
      let schurRefresh = true;
      for (let it = 0; !converged && it < maxIter; it++) {
        ...
        const dA = scratch.dA;
        if (newtonSchurStep) {
          // Schur-condensed inner solve (opts.schur): factor the body tangent
          // alone + dense perBody Schur complement, no gapN×perBody fill. The
          // body prefix of scratch.V_combined holds the current tangent; D is
          // in scratch.harmTrip_*. Exact reduction of the SAME tangent system;
          // refresh=false reuses the cached factors (modified-Newton chord).
          newtonSchurStep(neg, dA, schurRefresh);
        } else {
        ...
        // (Lagged-tangent chord disabled — see schurRefresh comment above: it
        // breaks convergence on stiff machines. Always refresh = full Newton.)
    ...
    function _ensureLinearFactorAt(phi) {
      ...
      // Harmonic: pattern is φ-invariant (only values change) — analyze stays amortized.
    ...
    function _ensureLinearSchurFactorAt(phi) {
      ...
      // _linearNuRCached, which _ensureLinearFactorAt needs; linearSchurPrepare
      // would normally set them, but we bypass it here.
      if (gapMethod === "mortar") {
        if (_linearNuRCached === null) _buildLinearRhsCaches();
    ...
    function solve(thetaR, currents) {
      ...
      // the result object). Pass thetaR so the brush/commutator spatial remap
      // matches the one used in the RHS Jz assembly (same θ).
