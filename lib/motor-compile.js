(function () {
  ...
  function compile(section) {
    ...
    // Fix A: expand rotorMask to cover every cell in every radial ring that has
    // at least one rotor cell.  setRotorAngle can only write rotated template
    // values into cells already in the rotor mask; if the mask only covers the
    // iron/magnet feature sectors, the inter-tooth air cells are never updated
    // and the saliency is frozen.  Marking the full band (all theta for each
    // rotor ring row) lets the iron/air pattern genuinely rotate.
    for (let i = 0; i < Nr; i++) {
      let hasRotor = false;
      for (let j = 0; j < Ntheta; j++) {
      ...
      if (hasRotor) {
        for (let j = 0; j < Ntheta; j++) rotorMask[i * Ntheta + j] = 1;
      }
    }
