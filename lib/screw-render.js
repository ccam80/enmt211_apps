...
(function () {
  ...
  function drawShaft(ctx, L, p, theta, x0Px, x1Px, yMid, shaftHpx, phaseShiftPx) {
    ...
    ctx.lineTo(x0Px, top);
    ctx.closePath();
    // Brighter cylindrical gradient so the shaft edges stand out from the
    // dark canvas. Mid-tone is a light steel grey; the top/bottom edges are
    // lifted out of near-black so they're visible against the panel.
    const grad = ctx.createLinearGradient(0, top, 0, bot);
    grad.addColorStop(0,   "#4a5462");
    grad.addColorStop(0.5, "#b6c0ce");
    grad.addColorStop(1,   "#2c333e");
    ctx.fillStyle = grad;
    ctx.fill();
  ...
  function drawLeadNut(ctx, L, p, state, opts) {
    ...
    // The collar must always be wide enough that at least two helical teeth
    // are visible inside it — otherwise the kinematic story is hidden when
    // the screw is rendered with a coarse pitch (single tooth on screen).
    // 2·pitchPx + a small margin guarantees centred + adjacent teeth fit.
    const nutW = Math.max(110, 2 * pitchPx + 24);
    const nutH = shaftHpx + 30;
  ...
  // The shell now reserves marginLeftPx for the motor disc, so the usable
  // track width shrinks by ~80 px on lessons that paint a motor; pass
  // `motorMarginPx` through so the slider floor matches.
  function minRenderablePitch(canvas, starts, xLimit, floorMin, motorMarginPx) {
    if (!canvas) return floorMin;
    const r = canvas.getBoundingClientRect();
    const padX = 60;
    const margin = +motorMarginPx || 0;
    if (r.width < 2 * padX + margin + 8) return floorMin;
    const usableW = r.width - 2 * padX - margin;
