...
//    cartGround(ctx, L, opts?)
//      Floor under a horizontal-track lesson: ground rectangle from
//      L.trackY (or opts.groundY) down, a thin horizontal rule at the
//      ground line, and metre ticks with labels at integer x positions.
//    wall(ctx, sx, yTop, yBot, opts?)
//      Vertical wall block + diagonal hatch shading on the outer side.
//      opts.side ∈ {"left","right"} (default "left", i.e. the wall sits
//      to the LEFT of sx and sx is its inside face).
//    refMarker(ctx, sx, yTop, yBot, opts?)
//      Dashed vertical reference line + downward triangle + label, the
//      shared "this is where the controller wants the load" affordance.
//      opts: { label, color, dash, font, triHeight, triHalfW, apexGap,
//              labelGap }.
...
(function () {
  ...
  // Pull the live canvas-text scale set by the shell each frame. Each
  // primitive uses these as its default font size; an explicit opts.fontSize
  // / opts.font on the call site still overrides. Token names mirror
  // lib/canvas-type.js: sm, md, lg, title, arrow, plotTitle, plotTick.
  function tok(name, base) {
    const T = LIB.Type && LIB.Type.current;
    return T && T[name] ? T[name] : base;
  }
  ...
  LIB.Draw = {
    ...
    trapezoidWeight(ctx, cx, cyTop, mass, opts) {
      ...
      if (label) {
        const labelFs = +opts.labelFontSize || tok("md", 12);
        ctx.fillStyle = opts.labelColor || "#e6e9ef";
        ctx.font = opts.labelFont || `600 ${labelFs}px ui-sans-serif`;
    ...
    jReflectedCallout(ctx, cx, yTop, J, opts) {
      opts = opts || {};
      // Bumped to title/md from md/sm — these labels carry the lesson's key
      // formula and were getting lost at typical canvas sizes.
      const titleFs = +opts.titleFontSize || tok("title", 18);
      const subFs   = +opts.subFontSize   || tok("md", 14);
      ctx.textAlign  = "center";
      ctx.textBaseline = "bottom";
      // Both lines stack ABOVE yTop so they never overlap the load they
      // describe — title sits highest, subtitle just above yTop.
      ctx.fillStyle = opts.subColor || "#8a93a3";
      ctx.font = opts.subFont || `${subFs}px ui-sans-serif`;
      ctx.fillText("(reflected to motor)", cx, yTop - 4);
      ctx.fillStyle = opts.color || "#66bb6a";
      ctx.font = opts.titleFont || `700 ${titleFs}px ui-sans-serif`;
      ctx.fillText(`J_load,refl = ${J.toFixed(4)} kg·m²`,
                   cx, yTop - 4 - subFs - 2);
    },
    ...
    track(ctx, opts) {
      ...
      if (opts.posLabel != null) {
        const posFs = +opts.posLabelFontSize || tok("sm", 11);
        ctx.fillStyle = labelColor;
        ctx.font = `${posFs}px ui-sans-serif`;
      ...
      if (opts.target != null && Number.isFinite(opts.target)) {
        ...
        ctx.stroke();
        if (opts.targetLabel) {
          const tgtFs = +opts.targetLabelFontSize || tok("sm", 11);
          ctx.fillStyle = targetColor;
          ctx.font = `${tgtFs}px ui-sans-serif`;
    ...
    arrow(ctx, x0, y0, x1, y1, opts) {
      ...
      if (opts.label) {
        const fontSize = +opts.fontSize || tok("arrow", 13);
    ...
    holdBadge(ctx, W, H, hold, opts) {
      ...
      const fs        = +opts.fontSize || tok("md", 13);
      const font      = opts.font      || `${fs}px ui-sans-serif`;
      ...
      if (hold && hold.controlled) {
        ...
        if (typeof opts.subtitle === "string") {
          ctx.save();
          ctx.font = `${tok("sm", 11)}px ui-sans-serif`;
          ctx.fillStyle = muted;
          ctx.textAlign = "left"; ctx.textBaseline = "top";
    ...
    cartGround(ctx, L, opts) {
      // Floor + integer-metre ticks for any horizontal linearTrack lesson.
      // Reads geometry from L; opts only override colours, label format, and
      // the tick range/density.
      opts = opts || {};
      const groundY    = (opts.groundY != null) ? +opts.groundY : L.trackY;
      const fillColor  = opts.fillColor  || "#1a1f27";
      const ruleColor  = opts.ruleColor  || "#2a313c";
      const tickColor  = opts.tickColor  || "#1e252f";
      const labelColor = opts.labelColor || "#55606f";
      const tickFrom   = (opts.tickFrom != null) ? +opts.tickFrom : Math.ceil(L.xMin);
      const tickTo     = (opts.tickTo   != null) ? +opts.tickTo   : Math.floor(L.xMax);
      const tickEvery  = (opts.tickEvery!= null) ? +opts.tickEvery: 1;
      const unit       = (opts.unit     != null) ? opts.unit      : "m";
      const fsmall     = +opts.fsmall || tok("sm", 11);
      const font       = opts.font   || (fsmall + "px ui-sans-serif");
      const tickLen    = +opts.tickLen   || 6;
      const labelGap   = +opts.labelGap  || 8;

      // Ground fill + top rule.
      ctx.fillStyle = fillColor;
      ctx.fillRect(0, groundY, L.W, L.H - groundY);
      ctx.strokeStyle = ruleColor;
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, groundY); ctx.lineTo(L.W, groundY);
      ctx.stroke();

      // Integer-metre ticks + labels.
      ctx.fillStyle    = labelColor;
      ctx.font         = font;
      ctx.textAlign    = "center";
      ctx.textBaseline = "top";
      ctx.strokeStyle  = tickColor;
      for (let xm = tickFrom; xm <= tickTo + 1e-9; xm += tickEvery) {
        const sx = L.xToPx(xm);
        ctx.beginPath();
        ctx.moveTo(sx, groundY);
        ctx.lineTo(sx, groundY + tickLen);
        ctx.stroke();
        const txt = (unit ? (xm + " " + unit) : String(xm));
        ctx.fillText(txt, sx, groundY + labelGap);
      }
    },

    wall(ctx, sx, yTop, yBot, opts) {
      // Vertical wall block (thickness px) on one side of sx, with diagonal
      // hatching extending past the wall on the outer side. Default side
      // "left": the wall sits at [sx − thickness, sx] and the hatches run
      // further left. side: "right" mirrors everything across sx.
      opts = opts || {};
      const side       = (opts.side === "right") ? "right" : "left";
      const thickness  = +opts.thickness  || 16;
      const hatchExt   = +opts.hatchExtend || 12;
      const hatchStep  = +opts.hatchStep  || 12;
      const fillColor  = opts.fillColor   || "#2a313c";
      const hatchColor = opts.hatchColor  || "#3a4453";
      const lineW      = +opts.lineW      || 1;
      const wallH      = yBot - yTop;
      const dir        = (side === "right") ? 1 : -1;

      // Wall rectangle.
      ctx.fillStyle = fillColor;
      const rx0 = (side === "left") ? (sx - thickness) : sx;
      ctx.fillRect(rx0, yTop, thickness, wallH);

      // Diagonal hatches.
      ctx.strokeStyle = hatchColor;
      ctx.lineWidth   = lineW;
      const hatchN = Math.max(2, Math.round(wallH / hatchStep));
      const innerX = (side === "left") ? (sx - thickness) : (sx + thickness);
      for (let i = 0; i < hatchN; i++) {
        const yy = yTop + (i * wallH) / hatchN;
        ctx.beginPath();
        ctx.moveTo(innerX, yy);
        ctx.lineTo(innerX + dir * hatchExt,
                   yy + (wallH / hatchN) * 0.8);
        ctx.stroke();
      }
    },
    ...
    refMarker(ctx, sx, yTop, yBot, opts) {
      // Dashed vertical reference + downward triangle (apex at yTop) + label
      // above. Shared by every "this is the target" overlay (PID-cart, car
      // centerline, future control lessons).
      opts = opts || {};
      const color     = opts.color || "#f6c945";
      const dash      = opts.dash  || [4, 4];
      const lineW     = +opts.lineW    || 1;
      const triHeight = +opts.triHeight|| 10;
      const triHalfW  = +opts.triHalfW || 6;
      const apexGap   = +opts.apexGap  || 2;
      const labelGap  = +opts.labelGap || 4;
      const fs        = +opts.fontSize || tok("sm", 11);
      const font      = opts.font || `${fs}px ui-sans-serif`;
      ...
      const apexY = yTop - apexGap;
      const baseY = apexY - triHeight;
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.moveTo(sx, apexY);
      ctx.lineTo(sx - triHalfW, baseY);
      ctx.lineTo(sx + triHalfW, baseY);
      ctx.closePath();
      ctx.fill();
      ...
      if (opts.label) {
        ...
        ctx.fillText(String(opts.label), sx, baseY - labelGap);
      }
    },
    ...
    dimensionLine(ctx, x0, y, x1, label, opts) {
      opts = opts || {};
      const color    = opts.color || "#8a93a3";
      const fontSize = +opts.fontSize || tok("sm", 11);
