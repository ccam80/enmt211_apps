  ...
  function backend({ factor = 3, fillet = { strength: 1 }, mg = {} } = {}) {
    return {
      prepare(section) {
        ...
        op._refineMg   = mg;
        // Build initial hierarchy (rotor at angle 0 from prepare).
        op._refineHierarchy = buildHierarchy(op, refined.grid, mg);
        return { op, compiled };
      },

      solveSaturated(op, b, o) {
        // Rebuild Galerkin hierarchy from the current fine operator (rotor angle
        // may have changed since prepare; stale coarse levels diverge).
        const hierarchy = buildHierarchy(op, op._refineGrid, op._refineMg);
      ...
      linearSolve(op, b, o) {
        // Rebuild Galerkin hierarchy from the current fine operator.
        const hierarchy = buildHierarchy(op, op._refineGrid, op._refineMg);
