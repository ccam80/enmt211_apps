(function () {
  ...
  const DEFAULTS = {
    spacing:    0.013,   // world-units between dots along a path
    speedScale: 0.005,   // path-units per amp per second of sim time
    mode:       "linear",
  ...
  // ---------------------------------------------------------------------------
  //  autoScale(currents, prevPeak, opts) → { scale, peak }
  //
  //  Normalises dot speed by the live current envelope so the strongest circuit
  //  always moves at ~`target` path-units/sec and weak circuits (e.g. a lightly
  //  loaded induction stator) still move visibly. The envelope is a slow-decay
  //  running peak so an AC current crossing zero doesn't blow the scale up.
  // ---------------------------------------------------------------------------
  function autoScale(currents, prevPeak, opts) {
    opts = opts || {};
    const target = opts.target != null ? opts.target : 0.02;
    const decay  = opts.decay  != null ? opts.decay  : 0.98;
    const floor  = opts.floor  != null ? opts.floor  : 0.05;
    let peak = floor;
    for (let k = 0; k < currents.length; k++) {
      const a = Math.abs(currents[k]);
      if (a > peak) peak = a;
    }
    const env = Math.max(peak, (prevPeak || floor) * decay);
    return { scale: target / env, peak: env };
  }

  LIB.CurrentDots = { step, placeDots, autoScale, DEFAULTS };
})();
