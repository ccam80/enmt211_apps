(function () {
  ...
  // ---------------------------------------------------------------------------
  //  Variable-Δθ (Galerkin) projection machinery
  //
  //  The DFT projection above (a[k]=(2/N)Σcos…) is only exact on UNIFORM nodes,
  //  where the cos/sin basis is discretely orthogonal. To let the gap circle sit
  //  on a per-feature (non-uniform Δθ) node ring, project by least-squares against
  //  the basis under the node quadrature: solve the Gram system  G·c = cᵂ  with
  //    cᵂ[I] = Σ_i w_i·A_i·ψ_I(θ_i),   G[I,J] = Σ_i w_i·ψ_I(θ_i)·ψ_J(θ_i),
  //  basis ψ = [1, cos θ, sin θ, …, cos Kθ, sin Kθ], w_i = node arc weight.
  //  On uniform nodes G = diag(2π, π, π, …) and this reduces to the DFT exactly,
  //  so existing behaviour is the special case (the regression gate).
  // ---------------------------------------------------------------------------

  // Periodic node arc weights (Σ w_i = 2π). For uniform nodes w_i = 2π/N.
  function nodeWeights(theta) {
    const N = theta.length;
    const w = new Float64Array(N);
    let s = 0;
    for (let i = 0; i < N; i++) {
      const prev = theta[(i - 1 + N) % N];
      const next = theta[(i + 1) % N];
      let d = next - prev;
      d = ((d % TWO_PI) + TWO_PI) % TWO_PI;   // arc spanned, wrapped to [0,2π)
      w[i] = 0.5 * d;
      s += w[i];
    }
    const scale = TWO_PI / s;                  // guard wrap accumulation
    for (let i = 0; i < N; i++) w[i] *= scale;
    return w;
  }

  // In-place Cholesky: A (n×n, row-major) → lower L with A = L·Lᵀ (L in lower tri).
  function cholFactor(A, n) {
    for (let j = 0; j < n; j++) {
      let d = A[j * n + j];
      for (let k = 0; k < j; k++) d -= A[j * n + k] * A[j * n + k];
      if (d <= 0) throw new Error("AirgapHarmonic: gap-node Gram not SPD (col " + j + ", pivot " + d + ") — degenerate/duplicate gap nodes?");
      d = Math.sqrt(d);
      A[j * n + j] = d;
      for (let i = j + 1; i < n; i++) {
        let sum = A[i * n + j];
  ...
  // Solve A·x = b given the in-place Cholesky factor L (lower) of A.
  ...
  // Precompute the projector for a fixed node set: weights + factored Gram.
  // Returns { w, Lchol, P } reusable across many projections on the same nodes.
  function makeProjector(theta, kList) {
    const nH = kList.length, N = theta.length, P = 2 * nH + 1;
    const w = nodeWeights(theta);
    const G = new Float64Array(P * P);
    const psi = new Float64Array(P);
    for (let i = 0; i < N; i++) {
      const th = theta[i], wi = w[i];
      psi[0] = 1;
      for (let h = 0; h < nH; h++) { const kt = kList[h] * th; psi[2 * h + 1] = Math.cos(kt); psi[2 * h + 2] = Math.sin(kt); }
      for (let I = 0; I < P; I++) {
        const wpsiI = wi * psi[I];
        for (let J = I; J < P; J++) G[I * P + J] += wpsiI * psi[J];
      }
    }
    for (let I = 0; I < P; I++) for (let J = 0; J < I; J++) G[I * P + J] = G[J * P + I];
    cholFactor(G, P);
    // Dense projection matrix Pmat (P × N), Pmat[h,i] = (G⁻¹·Bᵂᵀ)[h,i]:
    //   amplitudes  c = Pmat · A_nodal ,  and the bordered coupling is PmatᵀMPmat.
    // Column i is G⁻¹·(w_i·ψ(θ_i)). On uniform nodes Pmat[h,i] reduces to the DFT
    // coefficients (1/N, (2/N)coskθ, (2/N)sinkθ).
    const Pmat = new Float64Array(P * N);
    const rhs = new Float64Array(P), col = new Float64Array(P), psi2 = new Float64Array(P);
    for (let i = 0; i < N; i++) {
      const th = theta[i], wi = w[i];
      psi2[0] = 1;
      for (let h = 0; h < nH; h++) { const kt = kList[h] * th; psi2[2 * h + 1] = Math.cos(kt); psi2[2 * h + 2] = Math.sin(kt); }
      for (let h = 0; h < P; h++) rhs[h] = wi * psi2[h];
      cholSolve(G, P, rhs, col);
      for (let h = 0; h < P; h++) Pmat[h * N + i] = col[h];
    }
    return { w, Lchol: G, P, kList, nH, N, Pmat };
  }

  // Galerkin projection of nodal A onto {a[k], b[k]} via the prepared Pmat.
  // Output amplitude arrays are VALUE-indexed (a[k], b[k], size maxK+1) with
  // zeros at orders absent from kList. Pmat row h maps to the harmonic at
  // position (h-1)>>1 in kList: odd h → cos amplitude, even h → sin amplitude.
  function projectWith(proj, theta, Anodal) {
    const { Pmat, P, kList, nH, N } = proj;
    const maxK = nH ? kList[nH - 1] : 0;
    const a = new Float64Array(maxK + 1), b = new Float64Array(maxK + 1);
    for (let h = 0; h < P; h++) {
      let s = 0;
      const off = h * N;
      for (let i = 0; i < N; i++) s += Pmat[off + i] * Anodal[i];
      if (h === 0) { a[0] = s; continue; }
      const k = kList[(h - 1) >> 1];
      if (h & 1) a[k] = s;
      else b[k] = s;
    }
  ...
  function computeMk(r1, r2, k, mu0) {
    ...
    // Reformulated to avoid float64 underflow at high harmonic order: for large
    // k, Math.pow(r,k) of either radius can underflow to 0, driving E = b²−a²
    // to 0 and c = k/(mu0·E) to Infinity → NaN.  Divide numerator and
    // denominator by b² and work in the bounded ratio rho = (r1/r2)^k ∈ (0,1)
    // (single pow of a ratio<1, so the difference 1−rho² never underflows):
    //   c·(a²+b²)/(b²−a²)  →  (k/mu0)·(1+rho²)/(1−rho²)
    //   c·(2ab)/(b²−a²)    →  (k/mu0)·(2rho)/(1−rho²)
    // Algebraically identical wherever the original was finite; preserves the
    // existing sign convention [[m00, m01],[m10, m11]].
    const rho = Math.pow(r1 / r2, k);
    const rho2 = rho * rho;
    const denom = 1 - rho2;
    const kmu = k / mu0;
    const m00 = kmu * (1 + rho2) / denom;
    const m01 = -kmu * (2 * rho) / denom;
    return [
      [m00, m01],
      [m01, m00],
    ];
  ...
  function build(rotorGap, statorGap, opts) {
    // Harmonic basis: an explicit list of positive integer orders `kList`
    // (k=0, the constant/log mean mode, is always present and handled
    // separately). `opts.K` is the back-compat dense form → kList = [1..K],
    // which must reproduce the legacy behaviour bit-for-bit (the regression
    // gate). A sparse kList carries only the physically significant harmonics
    // (pole/MMF orders + slot bands), shrinking perBody = 2·|kList|+1 and with
    // it the O(|kList|²) bordered coupling that dominates the assembly.
    let kList;
    if (opts && Array.isArray(opts.kList)) {
      // Validate: positive integers, sorted ascending, unique.
      const src = opts.kList.slice().sort((x, y) => x - y);
      kList = [];
      for (let i = 0; i < src.length; i++) {
        const k = src[i];
        if (!Number.isInteger(k) || k < 1) {
          throw new Error("AirgapHarmonic.build: opts.kList entries must be positive integers (got " + k + ")");
        }
        if (i > 0 && k === src[i - 1]) continue; // drop duplicates
        kList.push(k);
      }
      if (kList.length < 1) {
        throw new Error("AirgapHarmonic.build: opts.kList must contain at least one positive integer");
      }
    } else if (opts && typeof opts.K === "number" && Number.isInteger(opts.K) && opts.K >= 1) {
      kList = [];
      for (let k = 1; k <= opts.K; k++) kList.push(k);
    } else {
      throw new Error("AirgapHarmonic.build: opts.K (positive integer) or opts.kList (array of positive integers) is required");
    }
    if (!opts || typeof opts.ell !== "number" || opts.ell <= 0) {
      throw new Error("AirgapHarmonic.build: opts.ell must be a positive number");
    }

    const nHarm = kList.length;          // number of (cos,sin) harmonic pairs
    const maxK = kList[nHarm - 1];        // highest harmonic order (sets Nyquist floor)
    const K = maxK;                       // back-compat export (== opts.K for the dense form)
    const ell = opts.ell;
    ...
    // Node floor is set by the HIGHEST harmonic carried (Nyquist on maxK), not
    // the count — a sparse kList with a high slot-band harmonic still needs the
    // ring resolved to 4·maxK.
    if (Ngr < 4 * maxK) {
      throw new Error(
        `AirgapHarmonic.build: rotor N_gap (${Ngr}) < 4·maxK (${4 * maxK}). ` +
        `Increase gapMinNodes to at least ${4 * maxK} for maxK=${maxK}.`
      );
    }
    if (Ngs < 4 * maxK) {
      throw new Error(
        `AirgapHarmonic.build: stator N_gap (${Ngs}) < 4·maxK (${4 * maxK}). ` +
        `Increase gapMinNodes to at least ${4 * maxK} for maxK=${maxK}.`
      );
    }
    ...
    // Build M_k matrices, value-indexed (sparse array, size maxK+1): k=0 plus
    // every order in kList. Absent orders are `undefined` and never indexed.
    const Mk_cache = new Array(maxK + 1);
    Mk_cache[0] = computeMk(r_mr, r_ms, 0, mu0);
    ...
    // DOF layout (D3):
    //   [gapRotor (Ngr) | gapStator (Ngs) | harmRotor (perBody) | harmStator (perBody)]
    // harmonics layout per body, by POSITION: [a0, a(k0), b(k0), a(k1), b(k1), …]
    // — position 2i+1 = cos amplitude of kList[i], position 2i+2 = sin. The
    // amplitude arrays returned by project()/surfaceFlux remain VALUE-indexed
    // (a[k], b[k], size maxK+1) so consumers and reconstruct() are unchanged.
    const perBody = 2 * nHarm + 1;       // number of harmonic DOFs per body
    const nHarmonicDofs = 2 * perBody;

    const layout = ["a0"];
    for (let i = 0; i < nHarm; i++) {
      layout.push("a" + kList[i], "b" + kList[i]);
    }
    ...
    // -------------------------------------------------------------------------
    // Per-body Galerkin projectors (weights + factored Gram), precomputed once.
    // On uniform nodes these reduce to the DFT; on per-feature (variable-Δθ)
    // gap rings they give the exact least-squares projection.
    const projRotor  = makeProjector(rotorTheta,  kList);
    const projStator = makeProjector(statorTheta, kList);

    function projectBound(gapTheta, Anodal) {
      // Fast path: caller passed one of the body rings (by reference). Otherwise
      // build a projector on the fly so arbitrary node sets still project.
      if (gapTheta === rotorTheta)  return projectWith(projRotor,  gapTheta, Anodal);
      if (gapTheta === statorTheta) return projectWith(projStator, gapTheta, Anodal);
      return projectWith(makeProjector(gapTheta, kList), gapTheta, Anodal);
    }
    function projectIntoBound(gapTheta, Anodal, a, b) {
      const r = projectBound(gapTheta, Anodal);
      a.set(r.a); b.set(r.b);
    }
    ...
    function surfaceFlux(ArotorGapNodal, AstatorGapNodal, phi) {
      const R = projectWith(projRotor,  rotorTheta,  ArotorGapNodal);
      const S = projectWith(projStator, statorTheta, AstatorGapNodal);
      ...
      // flux_rotor and flux_stator coefficients (value-indexed, zeros at orders
      // absent from kList — reconstruct skips them).
      const qRa = new Float64Array(maxK + 1); // cosine flux amplitude on rotor
      const qRb = new Float64Array(maxK + 1); // sine flux amplitude on rotor
      const qSa = new Float64Array(maxK + 1); // cosine flux amplitude on stator
      const qSb = new Float64Array(maxK + 1); // sine flux amplitude on stator
      ...
      // each carried harmonic order
      for (let idx = 0; idx < nHarm; idx++) {
        const k = kList[idx];
        const M = Mk_cache[k];

        // Apply rotor phase rotation R_k(+φ): forward-rotate the rotor
        // harmonic pair into the common frame so M_k can be applied.
        let arkR = R.a[k];
        let brkR = R.b[k];
        let ck = 1, sk = 0;
        if (phi !== 0) {
        ...
        // Cosine channel: [arkR; S.a[k]] → [qRa_F; qSa[k]]
        // Sine channel:   [brkR; S.b[k]] → [qRb_F; qSb[k]]
        // The rotor flux (qRa_F, qRb_F) is produced in the COMMON frame.
        const qRa_F = M[0][0] * arkR + M[0][1] * S.a[k];
        qSa[k]      = M[1][0] * arkR + M[1][1] * S.a[k];
        const qRb_F = M[0][0] * brkR + M[0][1] * S.b[k];
        qSb[k]      = M[1][0] * brkR + M[1][1] * S.b[k];

        // Back-rotate the rotor flux to the ROTOR frame by R_k(-φ) before it
        // is reconstructed onto rotorTheta. Without this the discrete
        // A↦flux operator is non-self-adjoint for φ≠0 (reciprocity defect).
        // Reuse the cos(kφ)/sin(kφ) computed above. The stator channel is in
        // the identity frame and is left unchanged.
        qRa[k] =  qRa_F * ck + qRb_F * sk;
        qRb[k] = -qRa_F * sk + qRb_F * ck;
      }
    ...
    //   G k≥1: 12 per harmonic  (4 diagonal addSym (i==j) + 8 cross-direction raw)
    // Total: 2(Ngr+Ngs)(1+2·nHarm) + 4 + 12·nHarm   (nHarm = |kList|)
    const tripletCapacity = 2 * (Ngr + Ngs) * (1 + 2 * nHarm) + 4 + 12 * nHarm;
    ...
    function stampInto(phi, outI, outJ, outV) {
      ...
      // B_r: Galerkin projection coupling (gap-rotor nodal DOFs ↔ rotor harmonic
      // DOFs). Pr[h,i] = (G_r⁻¹·Bᵂᵀ)[h,i] is the projection coefficient; the
      // bordered coupling Schur-condenses to PrᵀMPr. On uniform nodes Pr equals
      // the DFT values (1/Ngr, (2/Ngr)cos kθ, (2/Ngr)sin kθ) — the regression case.
      // Loop by harmonic POSITION h (0..nHarm-1): DOF cos = 2h+1, sin = 2h+2,
      // matching the position-indexed Pmat rows. For kList=[1..K] these reduce
      // to the legacy 2k-1/2k offsets exactly.
      const Pr = projRotor.Pmat;
      for (let i = 0; i < Ngr; i++) {
        addSym(i, baseHR + 0, Pr[i]);                                  // h=0 (a0)
        for (let h = 0; h < nHarm; h++) {
          const pa = 2 * h + 1, pb = 2 * h + 2;
          addSym(i, baseHR + pa, Pr[pa * Ngr + i]);                    // a(kList[h])
          addSym(i, baseHR + pb, Pr[pb * Ngr + i]);                    // b(kList[h])
        }
      }

      // B_s: same, stator body.
      const Ps = projStator.Pmat;
      for (let i = 0; i < Ngs; i++) {
        const iAbs = Ngr + i;
        addSym(iAbs, baseHS + 0, Ps[i]);                               // h=0 (a0)
        for (let h = 0; h < nHarm; h++) {
          const pa = 2 * h + 1, pb = 2 * h + 2;
          addSym(iAbs, baseHS + pa, Ps[pa * Ngs + i]);                 // a(kList[h])
          addSym(iAbs, baseHS + pb, Ps[pb * Ngs + i]);                 // b(kList[h])
        }
      }
      ...
      // Each carried order: 4×4 block with φ-dependent cross terms, using M_k^{-1}.
      // Position h sets the DOF indices (2h+1 cos, 2h+2 sin); value k=kList[h]
      // sets M_k and the cos(kφ)/sin(kφ) rotation.
      for (let h = 0; h < nHarm; h++) {
        const k = kList[h];
        const M = Mk_cache[k];
        ...
        const hrC = baseHR + (2 * h + 1);  // rotor cosine DOF for kList[h]
        const hrS = baseHR + (2 * h + 2);   // rotor sine DOF
        const hsC = baseHS + (2 * h + 1);   // stator cosine DOF
        const hsS = baseHS + (2 * h + 2);    // stator sine DOF
    ...
    return {
      K,                              // == maxK (highest carried order)
      kList:         kList.slice(),   // explicit harmonic basis
      nHarm,                          // == kList.length
      maxK,
      nGapRotor:     Ngr,
      nGapStator:    Ngs,
