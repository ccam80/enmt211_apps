(function () {
  ...
  function makeProjector(theta, K) {
    const N = theta.length, P = 2 * K + 1;
    ...
    for (let i = 0; i < N; i++) {
      ...
      psi[0] = 1;
      for (let k = 1; k <= K; k++) { psi[2 * k - 1] = Math.cos(k * th); psi[2 * k] = Math.sin(k * th); }
      for (let I = 0; I < P; I++) {
  ...
  function build(rotorGap, statorGap, opts) {
    ...
    const projRotor  = makeProjector(rotorTheta,  K);
    const projStator = makeProjector(statorTheta, K);
    ...
    function projectBound(gapTheta, Anodal) {
      ...
      return projectWith(makeProjector(gapTheta, K), gapTheta, Anodal);
    }
    ...
    function stampInto(phi, outI, outJ, outV) {
      ...
      for (let i = 0; i < Ngr; i++) {
        ...
        for (let k = 1; k <= K; k++) {
          addSym(i, baseHR + (2 * k - 1), Pr[(2 * k - 1) * Ngr + i]);  // a_k
          addSym(i, baseHR + (2 * k),     Pr[(2 * k) * Ngr + i]);      // b_k
        }
      }
      ...
      for (let i = 0; i < Ngs; i++) {
        ...
        for (let k = 1; k <= K; k++) {
          addSym(iAbs, baseHS + (2 * k - 1), Ps[(2 * k - 1) * Ngs + i]); // a_k
          addSym(iAbs, baseHS + (2 * k),     Ps[(2 * k) * Ngs + i]);     // b_k
        }
      }
