(function () {
  ...
  function build(rotorGap, statorGap, opts) {
    ...
    function stamp(phi) {
      ...
      // Count the triplets first, then fill:
      // For each harmonic index hIdx (0..perBody-1):
      //   - B_r coupling: Ngr × 2 (both directions) triplets
      //   - B_s coupling: Ngs × 2 triplets
      //   - G block: 4 entries × 2 (symmetric) for the 2×2 rotor-stator block
      //     (but self-terms and cross-terms separately)
      // Plus the G_rr and G_ss self-block diagonal (1 entry × 2 for off-diag, 1 for diag)
      //
      // Simpler: enumerate all triplets explicitly.

      // We'll collect (i, j, v) tuples and emit them.
      // Pre-allocate generously; we'll track the count.

      // Per harmonic DOF hIdx ∈ [0, perBody):
      //   B_r: Ngr pairs of (gapRotorNode, harmRotorDOF)
      //   B_s: Ngs pairs of (gapStatorNode, harmStatorDOF)
      // G block per k (k=0..K, corresponding to hIdxes 0 / (2k-1,2k)):
      //   4 entries in the 2x2 block (diagonal: G_rr[h,h], G_ss[h,h]; off-diag: G_rs[h,h], G_sr[h,h])
      //   Plus symmetry for off-diag
      // For k=0: single harmonic index h0=0, 2x2 block: G_rr[0,0], G_rs[0,0], G_sr[0,0], G_ss[0,0]
      // For k=1..K: two harmonic indices h1=2k-1 (cosine), h2=2k (sine)
      //   Each k contributes a 2x2 (cos,sin) sub-block for G_rr/G_rs/G_sr/G_ss,
      //   but since M_k is diagonal in the cos/sin space (cos-cos and sin-sin decouple
      //   because the Laplace equation respects cos/sin parity), the G block for
      //   harmonic k has the same scalar value for both the cosine and sine DOFs.
      //
      // Wait — this is important. M_k acts on the amplitude (a_r, a_s) for a single
      // harmonic ORDER k, and separately on (b_r, b_s). The M_k matrix is 2x2
      // (rotor-in vs stator-in), not 4x4 (cos+sin). So the harmonic DOF indexing is:
      //   For k=0: one DOF on each body (a0), total 2 DOFs in G block (1x1 per body → 2x2 cross)
      ...
      // But we also need to add the B entries for k=0 only separately... no, B applies
      // to all perBody harmonic indices.

      // Let's just build an array of triplets.
      const triplets = [];

      // Helper to add symmetric pair
      function addSym(i, j, v) {
        triplets.push([i, j, v]);
        if (i !== j) triplets.push([j, i, v]);
      }
      ...
      for (let k = 1; k <= K; k++) {
        ...
        triplets.push([hrC, hsC, -minv01 * ck]);
        triplets.push([hsC, hrC, -minv01 * ck]);

        triplets.push([hrC, hsS, -minv01 * sk]);
        triplets.push([hsS, hrC, -minv01 * sk]);

        triplets.push([hrS, hsC,  minv01 * sk]);
        triplets.push([hsC, hrS,  minv01 * sk]);

        triplets.push([hrS, hsS, -minv01 * ck]);
        triplets.push([hsS, hrS, -minv01 * ck]);
      }

      // Convert to typed arrays
      const nt = triplets.length;
    ...
    function torque(ArotorGapNodal, AstatorGapNodal, phi) {
      ...
      for (let k = 1; k <= K; k++) {
        ...
        const dT = (Math.PI * r_eval * r_eval * ell / mu0) *
                   k * (Rrot_a * S.b[k] - Rrot_b * S.a[k]);
        T += dT;
      }
