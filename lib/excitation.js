...
(function () {
  ...
  function evalTerminal(circuit, ctx) {
    ...
    // Step 4: mechanical
    //
    // A mechanically-commutated armature now has its commutation modeled
    // SPATIALLY in the slice (the brush/commutator current-sheet remap
    // turns_eff(e,θ) in lib/motor-slice.js): the brush map holds the armature
    // current-sheet brush-locked in the stator frame by flipping individual
    // conductor SIGNS per rotor angle. The spatial map IS the commutation, so
    // the old scalar ±1 sectorGate (a crude 1-D stand-in) must NOT also be
    // applied here — doing so would double-count / re-invert the already-correct
    // spatial sheet. The electrical layer for these circuits supplies only the
    // un-gated terminal current; the slice owns the commutation. (This pins the
    // hand-off from feature-brush-commutator.md "Reconcile with the existing
    // scalar gate".)
    if (mode === "mechanical") {
      const raw = supplyValue(terminal, t);
      if (type === "CURRENT") return { kind: "current", I: raw };
      return { kind: "voltage", V: raw };
    }
  }
