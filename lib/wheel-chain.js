...
(function () {
  ...
  const K_RIGID = 2.0e5;
  const C_RIGID = 1.0e3;
  // Default mass picked so J = ½·m·r² gives J=0.05 at r=0.5 m. Mass is the
  // first-class field; J is derived.
  ...
  // ---------------------------------------------------------------------------
  //  Layout / kinematics (operate on a wheels-array)
  // ---------------------------------------------------------------------------
  ...
  function makeWheelView(state, i, init) {
    ...
    // Mass is the primary field and J is derived. Callers that pass `J` instead
    // get their mass back-derived (so a slider on r alone keeps m fixed and
    // recomputes J).
    if (init && init.m != null) {
      w.m = +init.m;
      w.J = massToJ(w.r, w.m);
