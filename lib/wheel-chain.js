...
//        }
//
//        meshH = 0 collapses to plain rigid coupling at ψ=0 (the gear /
//        belt / whole-system case). meshH > 0 reads state.meshEngaged[i] for
//        each mesh's engagement state — caller is responsible for updating
//        that array via LIB.Lash.latchEngagement in preStep.
//
...
(function () {
  ...
  const K_RIGID = 2.0e5;
  const C_RIGID = 1.0e3;
  const WHEEL_DEFAULTS = { r: 0.5, r1: 0.5, r2: 0.5, J: 0.05 };
  ...
  function makeWheelView(state, i, init) {
    const w = {
      ...
      J:  init && init.J  != null ? +init.J  : WHEEL_DEFAULTS.J,
      lastTau: 0,
    };
  ...
  function contactDxdt(state, params, t, opts) {
    ...
    for (let i = 0; i < N - 1; i++) {
      ...
      const eng = (state.meshEngaged && state.meshEngaged[i] != null)
        ? state.meshEngaged[i] : +1;          // default: always engaged (rigid)
      const F = LIB.Lash.contactForce(psi, psiDot, eng, H, K, C);
      const wA = state.wheels[i];
      const wB = state.wheels[i + 1];
      const rA = rightR(wA, mode, i);
      const rB = leftR (wB, mode, i + 1);
  ...
  function contactJacobian(state, params, t, opts) {
    ...
    // Contact contributions per engaged mesh.
    for (let i = 0; i < N - 1; i++) {
      const eng = (state.meshEngaged && state.meshEngaged[i] != null)
        ? state.meshEngaged[i] : +1;
      if (eng === 0) continue;
      const wA = state.wheels[i];
      const wB = state.wheels[i + 1];
      const rA = rightR(wA, mode, i);
