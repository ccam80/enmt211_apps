...
(function () {
  ...
  function create({ Nr = 12, Ntheta = 256, rInner, rOuter, ell = 1 }) {
    ...
    // Rotor winding (conductor) current maps. A wound rotor's conductor pattern
    // must rotate rigidly with thetaR, exactly like the magnetization, so that
    // flux linkage, Jz, and the mutual-/λpm-gradients reflect the moved
    // conductors. coilMasksBase holds the unrotated per-circuit masks supplied
    // at compile; coilMaskRotorTemplate[k] holds the rotor-cell portion of mask
    // k at thetaR=0; coilMasksRot[k] is the current rotated mask (stator cells
    // = base, rotor cells = rotated). Only circuits with at least one rotor cell
    // are rotated; pure-stator circuits keep their base mask by reference.
    let coilMasksBase = null;          // Array<Float64Array> length nMasks, or null
    let coilMaskRotorTemplate = null;  // Array<Float64Array> (rotor-cell snapshot), or null
    let coilMasksRot = null;           // Array<Float64Array> current rotated masks, or null
    let coilMaskHasRotor = null;       // Array<bool> — true if mask k touches a rotor cell
    ...
    const op = {
      ...
      // ---- setRotorCoilMasks ----
      // Register the per-circuit conductor current maps so that rotor-member
      // windings rotate rigidly with thetaR (analogous to the magnetization
      // rotation). Snapshots the rotor-cell portion of every mask that touches a
      // rotor cell as its thetaR=0 template. Masks that touch no rotor cell are
      // kept by reference (stator windings never move). Must be called after
      // setRotorRegion (it needs rotorMask). Re-call replaces the registration.
      setRotorCoilMasks(masks) {
        if (!masks || masks.length === 0 || !rotorMask) {
          coilMasksBase = null;
          coilMaskRotorTemplate = null;
          coilMasksRot = null;
          coilMaskHasRotor = null;
          return;
        }
        const nMasks = masks.length;
        coilMasksBase = masks;
        coilMaskRotorTemplate = new Array(nMasks);
        coilMasksRot = new Array(nMasks);
        coilMaskHasRotor = new Array(nMasks);

        for (let k = 0; k < nMasks; k++) {
          const mk = masks[k];
          // Does this circuit place any conductor in a rotor cell?
          let touchesRotor = false;
          for (let idx = 0; idx < N; idx++) {
            if (rotorMask[idx] && mk[idx] !== 0) { touchesRotor = true; break; }
          }
          coilMaskHasRotor[k] = touchesRotor;
          if (touchesRotor) {
            // Template: rotor-cell values at thetaR=0 (non-rotor cells stay 0).
            const tmpl = new Float64Array(N);
            for (let idx = 0; idx < N; idx++) {
              if (rotorMask[idx]) tmpl[idx] = mk[idx];
            }
            coilMaskRotorTemplate[k] = tmpl;
            // Current rotated mask starts as a copy of the base (thetaR=0).
            coilMasksRot[k] = mk.slice();
          } else {
            coilMaskRotorTemplate[k] = null;
            coilMasksRot[k] = mk;   // by reference — never mutated
          }
        }
      },

      // ---- rotatedCoilMasks ----
      // Returns the per-circuit conductor maps reflecting the current rotor
      // angle: rotor-member windings rotated by the last setRotorAngle, stator
      // windings unchanged. Returns null if no coil masks were registered. The
      // returned array and its rotated entries are owned by the operator and are
      // overwritten on the next setRotorAngle — callers must consume immediately.
      rotatedCoilMasks() {
        return coilMasksRot;
      },
      ...
      setRotorAngle(thetaR) {
        ...
        // Rotor-winding (conductor) rotation. For each registered mask that
        // touches a rotor cell, rotate the rotor-cell portion by the same
        // interpolation as nu/magnetization, leaving stator cells untouched.
        const hasCoil = coilMaskRotorTemplate !== null;
        ...
        for (let i = 0; i < Nr; i++) {
          ...
          for (let j = 0; j < Ntheta; j++) {
            ...
            const idx0 = i * Ntheta + j0;
            const idx1 = i * Ntheta + j1;

            const v0 = rotorNuTemplate[idx0];
            const v1 = rotorNuTemplate[idx1];
            nuNew[idx] = v0 * (1 - frac) + v1 * frac;

            if (hasMag) {
              const m0r = rotorMrTemplate[idx0];
              const m1r = rotorMrTemplate[idx1];
              MrNew[idx] = m0r * (1 - frac) + m1r * frac;

              const m0t = rotorMtTemplate[idx0];
              const m1t = rotorMtTemplate[idx1];
              MtNew[idx] = m0t * (1 - frac) + m1t * frac;
            }

            if (hasCoil) {
              for (let k = 0; k < coilMaskRotorTemplate.length; k++) {
                const tmpl = coilMaskRotorTemplate[k];
                if (tmpl === null) continue;
                coilMasksRot[k][idx] = tmpl[idx0] * (1 - frac) + tmpl[idx1] * frac;
              }
            }
          }
        }
