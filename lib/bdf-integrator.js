...
(function () {
  ...
  function create(opts) {
    ...
    // Step-size dead band: when the proposed dt-change ratio lands in
    // [growDbLo, growDbHi], HOLD dt unchanged. Constant dt keeps the variable-step
    // Gear coefficients and the divided-difference LTE estimate clean, avoiding the
    // step-size churn that triggers spurious rejects. Default [1,1] = disabled.
    const growDbLo = opts.growDbLo != null ? opts.growDbLo : 1;
    const growDbHi = opts.growDbHi != null ? opts.growDbHi : 1;
    ...
    self.step = function (stepSolve) {
      ...
      for (;;) {
        ...
        if (worst <= 1 || dt <= dtMin) {
          ...
          const _ratio = dtNext / dt;
          if (_ratio >= growDbLo && _ratio <= growDbHi) dtNext = dt;   // dead band: hold dt
          self.dt = dtNext;
