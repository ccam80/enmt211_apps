(function () {
  ...
  function draw(ctx, bodyMesh, opts) {
    ...
    for (let e = 0; e < Ne; e++) {
      ...
      ctx.fillStyle = color;
      ctx.fill();
      // Stroke each element with a semi-transparent white edge so cell boundaries
      // are visible against any fill color regardless of the caller's scale transform.
      // lineWidth is left to the caller (set to 1/scale in world space so it stays
      // ~1 screen pixel under any zoom).
