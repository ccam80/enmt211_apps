...
- **Cross-lesson state.** `runTabs` does not preserve slider values when
  switching tabs (each spec re-runs its `state()` factory on mount). When
  that becomes desirable the shell can persist by `${spec.id}.${key}` to
  localStorage; currently it does not.
- **Drag flavour usage**: `point2d` is in place but currently unused
  (rover/ball-mill will exercise it). No `tangential` flavour for "drag
  along a moving belt's tangent" — would be a thin extension if a lesson
  needs it.

## Reference: the linear-transmission family

`lessons/linear_transmission/{lead-screw,ball-screw,conveyor,whole-system}.{html,js}`
are the canonical worked examples. They exercise:

- All declarative-physics hooks (`dof`, `dxdt`, `jacobian`,
  `integrator: "implicitEuler"`, `preStep`, `postStep`).
- `LIB.HeaderButtons.driveToggle()` for the Drive ON/OFF button (writes
  to `state.driveOn`); pause + reset + pointer-dispatch are all the
  shell's job.
- Static and mode-aware (`(state) => …`) sliders / plots / readouts.
- Two and three plot panes, with mixed reference + signal series.
- `LIB.Layout.linearTrack` for world↔px transforms.
- `LIB.ScrewRender.{drawShaft, drawLeadNut, drawBallNut}` and
  `LIB.BeltRender.drawScene` — shared device renderers, so the lesson
  files only contain physics + spec wiring (no inlined drawing code).
- `LIB.Drag.hbar` (load), `LIB.Drag.vbar` with anchor (screw shaft),
  `LIB.Drag.angular` (drive + idler pulleys), composed with `LIB.Drag.mux`.
- `LIB.Lash.{contactForce, latchEngagement}` — lead-screw's lash uses
  exactly this; backlash (rotational) uses the same primitive per mesh.
- `spec.modes` (whole-system swaps lead/ball/conveyor sub-modes while
  preserving motor θ, load x, PID integrator, and motor temperature).
- `LIB.App.runTabs` (the `index.html` hub mounts all four).

The unified-stiff-spring formulation is the key idea — the original
motionapp's lead-screw had three physics regimes (free / engaged / drag)
spelled out as separate manual implicit-Euler steps. Migrating to the
shell collapsed them into one set of branched stiff terms in `dxdt`, with
an implicit-Euler step driven by the library Jacobian. Pointer drag,
contact springs, and end-stop bumpers all live as dxdt branches gated on
state. Ball-screw and conveyor share the same shape minus the lash window
(rigid coupling spring at ψ = 0).

## Reference: the rotational-transmission family

`lessons/rotational_transmission/{gears,backlash,belt-and-pulley,whole-system}.{html,js}`
ride on the same declarative-physics surface as the linear lessons — no
bespoke `physics.step` anywhere. The chain itself lives in the library:

- `LIB.WheelChain.makeState(N, opts?)` → state object with flat DOF keys
  (`theta_0`, `omega_0`, `theta_1`, …) plus a `state.wheels[]` view of
  proxies that read/write through to those keys. Renderers/sliders use
  the proxies; physics callbacks read flat keys.
- `LIB.WheelChain.dof(state)` — pass it as `physics.dof` (function form)
  so the integrator's view tracks add/remove-wheel.
- `LIB.WheelChain.contactDxdt(state, params, t, opts)` and
  `LIB.WheelChain.contactJacobian(state, params, t, opts)` — viscous +
  contact-spring + (optional) drive contributions for an N-wheel chain.
  `meshH = 0` collapses to plain rigid coupling at ψ=0 (gears, belts,
  whole-system); `meshH > 0` reads `state.meshEngaged[i]` per mesh
  (backlash) — the lesson's preStep updates that array via
  `LIB.Lash.latchEngagement`.
- `LIB.WheelChainView.{buildView, hitTestWheel, pointerAngleFor, color}`
  — the shared per-frame chain layout + pointer hit-test.
- `LIB.GearRender.{drawWheel, drawGearShape, drawDriveArrow}` —
  rendering primitives.
- `LIB.Drag.chainWheel` — pointer-rotate any wheel via penalty spring;
  the integrator handles propagation through the chain naturally.
- `LIB.HeaderButtons.driveToggle()` — same Drive ON/OFF as the linear
  family.

The result: each rotational lesson's physics is `{ dof, dxdt, jacobian,
integrator: "implicitEuler", preStep, postStep }` — exactly the same shape
as lead-screw's. backlash just adds an engagement-latch step to preStep
and passes `meshH = lash/2` into `contactDxdt` so the contact springs gate
on `state.meshEngaged`.
